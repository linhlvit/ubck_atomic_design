...
| 2 | `compliance_report_data.row_data` là dạng JSON blob — không có cấu trúc cột tường minh. Có cần thiết kế entity Silver không? | Nếu row_data hoàn toàn không có schema ổn định → cân nhắc đưa vào Bronze only, không thiết kế Silver. Đề xuất: **giữ Silver entity** với row_data dạng Text (raw JSON), tránh parse. Cần xác nhận với business. |
| 3 | ~~`analysis_account_group_member.account_group_id` FK suy luận → account_group (T1) nhưng không có FK tường minh. Có cần FK này trên Silver?~~ **✅ RESOLVED:** FK suy luận được xác nhận đúng → thêm `account_investor_group_id FK` + `account_investor_group_code` (denormalized) vào `Market Surveillance Account Group Member Analysis Result`. | — |
