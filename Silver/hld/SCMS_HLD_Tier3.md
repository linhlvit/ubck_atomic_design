...
| 4 | `BC_BAO_CAO_GT` có giá trị Silver không? | ✅ **Có.** Thiết kế `Member Report Indicator Value` ở Tier 4 (FK đến `Member Periodic Report`). `BC_GT_*` và `BC_KHAI_THAC*` → ngoài scope. |
...
| Audit Log nguồn | CTCK_HS_LICH_SU | Danh sách hồ sơ lịch sử thay đổi | Audit Log nguồn — BANG_LIEN_KET + CHI_TIET_ID ghi lịch sử thay đổi dạng generic cho nhiều bảng khác nhau, không phải entity nghiệp vụ Silver |
