...
| — | — | — | BC_CANH_BAO | Cảnh báo trong quá trình thao tác | **Ngoài scope** | Thông tin cảnh báo phát sinh khi thao tác nhập liệu — không có ý nghĩa nghiệp vụ Silver. |
...
| # | Câu hỏi | Quyết định |
|---|---|---|
| 1 | `BC_KHAI_THAC` + `BC_KHAI_THAC_GT` — Gold pre-compute hay Fact Append? | ✅ **Ngoài scope.** Dữ liệu khai thác tổng hợp nội bộ hệ thống, không có giá trị nghiệp vụ Silver. |
| 2 | `BC_GT_HDR` + `BC_GT_DTL` (draft) — có cần lên Silver không? | ✅ **Ngoài scope.** Dữ liệu nháp trung gian, chưa phải submission chính thức. |
| 3 | `BC_BAO_CAO_GT` — có giá trị Silver không? | ✅ **Có.** Thiết kế entity `Member Report Indicator Value` (Tier 4). FK đến `Member Periodic Report`. |
| 4 | `CTCK_CD_DAI_DIEN` và `CTCK_CD_MOI_QUAN_HE` — Silver entity riêng hay denormalize? | ✅ **Entity riêng.** `Securities Company Shareholder Representative` và `Securities Company Shareholder Related Party`. FK đến `SecuritiesCompanyShareholder`. |
...
| System log | BC_CANH_BAO | Cảnh báo trong quá trình thao tác nhập liệu | Thông tin cảnh báo phát sinh khi thao tác — không có ý nghĩa nghiệp vụ Silver. |
| Gold derivative | BC_KHAI_THAC | Dữ liệu khai thác phục vụ báo cáo tổng hợp | Pre-aggregated nội bộ hệ thống SCMS — không có giá trị nghiệp vụ Silver. |
