...
| Tier | BCV Core Object | BCV Concept | Category | Source Table | Mô tả bảng nguồn | Silver Entity |
|---|---|---|---|---|---|---|
...
| DM_QUOC_TICH | Danh mục quốc tịch | [Location] Geographic Area | → **Silver entity `Geographic Area`** (shared). Bổ sung source_table. FK inbound: CTCK_CO_DONG, CTCK_NHAN_SU_CAO_CAP, CTCK_THONG_TIN, CTCK_VP_DAI_DIEN_NN. |
...
| DM_CANH_BAO | Danh sách cảnh báo báo cáo | Classification Value hoặc Silver entity | `SCMS_REPORT_WARNING_RULE` *(cần xác nhận)* |
...
|---|---|---|---|
| 1 | T1 | `DM_CANH_BAO` — Silver entity hay out of scope? | ✅ **Ngoài scope.** Rule nhập liệu hệ thống, không phải dữ liệu được nhập. Chỉ `BC_CANH_BAO` (kết quả thực tế) mới lên Silver. |
...
| 7 | T2 | `BM_BAO_CAO_LS` — Audit Log nguồn hay ETL Status History? | ✅ **Audit Log nguồn, ngoài scope Silver.** |
...
| 10 | T4 | `BC_KHAI_THAC` + `BC_KHAI_THAC_GT` — Gold hay Silver? | ✅ **Ngoài scope.** Dữ liệu khai thác tổng hợp nội bộ hệ thống. |
| 11 | T4 | `BC_GT_HDR` + `BC_GT_DTL` (draft) — cần Silver không? | ✅ **Ngoài scope.** Dữ liệu nháp trung gian. |
...
| Audit Log nguồn | BC_THANH_VIEN_LS | Lịch sử các lần gửi báo cáo | Audit Log nguồn — snapshot từng phiên bản gửi, không phải entity nghiệp vụ Silver |
