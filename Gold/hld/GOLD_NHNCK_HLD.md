...
|---|---|---|---|---|
| K_NHNCK_2 | Chứng chỉ cấp mới (YTD) | CCHN | Base | COUNT(DISTINCT License Certificate Document Code) WHERE Applications.IssueDate BETWEEN 01/01/Y AND today (Y hiện tại) hoặc 31/12/Y (Y quá khứ) |
...
> **✅ Đã giải quyết (BA v2 + Screenshot xác nhận):** Dashboard Mạng lưới map toàn bộ về nguồn NHNCK READY:
> - Đơn vị công tác → `ProfessionalHistories.Workplace` / `OrganizationReports.Workplace` → `Practitioner Employment History` (Nhóm 8) — READY
...
> **✅ Đã giải quyết (BA v2 xác nhận):** "Mạng lưới người có liên quan" map toàn bộ về `NHNCK.ProfessionalRelationships` — nguồn NHNCK READY, không cần SGDCK. Đã thiết kế đầy đủ tại **Nhóm 12** (`Practitioner Related Party Profile`). O_NHNCK_11 → **Closed**.
