...
| K_PTTT_99 | MarketCap_i (Vốn hóa từng mã) | — | Phái sinh | TBD — chờ BA | **Lý do pending:** [Nhóm 1 - BA chưa mapping xong]: BA chuyển Pending 2026-09-23 (BA dòng 165, 192). **Atomic cần bổ sung:** Đã có đủ. **Mart dự kiến:** giữ thiết kế trước — `security_trading_snapshot.close_price × K_PTTT_98` | PENDING |
| K_PTTT_100 | TotalCap_Sector (Tổng vốn hóa ngành) | Tỷ VND | Phái sinh | TBD — chờ BA | **Lý do pending:** [Nhóm 1 - BA chưa mapping xong]: BA chuyển Pending 2026-09-23 (BA dòng 164, 191). **Atomic cần bổ sung:** Đã có đủ. **Mart dự kiến:** giữ thiết kế trước — `SUM(K_PTTT_99)` GROUP BY `public_company.business_line_level_1_code`, `trading_dt` | PENDING |
| K_PTTT_101 | wᵢ — Trọng số vốn hóa per mã trong ngành | — | Phái sinh | TBD — chờ BA | **Lý do pending:** [Nhóm 1 - BA chưa mapping xong]: BA chuyển Pending 2026-09-23 (BA dòng 163). **Atomic cần bổ sung:** Đã có đủ. **Mart dự kiến:** giữ thiết kế trước — `K_PTTT_99 / K_PTTT_100` | PENDING |
| K_PTTT_102 | StressScoreSector — Chỉ số căng thẳng ngành tổng hợp | Điểm (0–100) | Phái sinh | TBD — chờ BA | **Lý do pending:** [Nhóm 1 - BA chưa mapping xong]: BA chuyển Pending 2026-09-23 (BA dòng 162, 199). **Atomic cần bổ sung:** Đã có đủ. **Mart dự kiến:** giữ thiết kế trước — `SUM(K_PTTT_95 × K_PTTT_101)` GROUP BY `public_company.business_line_level_1_code`, `trading_dt` | PENDING |
| K_PTTT_103 | Sector Liquid Score (TotalValue / TotalCap) | % | Phái sinh | `K_PTTT_96 / K_PTTT_100` | [SỬA 2026-09-23 — BA PTTT 14:49, Nhóm 7 34 Done / 8 Pending] BA dòng 186; phụ thuộc K_PTTT_100 (BA Pending) — cột vật lý giữ nguyên | READY |
| K_PTTT_104 | Sector Stress Score kỳ trước | Điểm (0–100) | Phái sinh | TBD — chờ BA | **Lý do pending:** [Nhóm 1 - BA chưa mapping xong]: BA chuyển Pending 2026-09-23 (BA dòng 200). **Atomic cần bổ sung:** Đã có đủ. **Mart dự kiến:** giữ thiết kế trước — `LAG(K_PTTT_102)` OVER (PARTITION BY `industry_dim_id` ORDER BY `cdr_dt_dim.cdr_dt`) | PENDING |
| K_PTTT_105 | Biến động áp lực (Stress Score kỳ này − kỳ trước) | Điểm | Phái sinh | `K_PTTT_102 − K_PTTT_104` | [SỬA 2026-09-23 — BA PTTT 14:49, Nhóm 7 34 Done / 8 Pending] BA dòng 198; phụ thuộc K_PTTT_102/104 (BA Pending) — cột vật lý giữ nguyên | READY |
| K_PTTT_106 | Xếp hạng ngành (Rất thấp/Thấp/Trung bình/Cao/Rất cao) | Text | Phái sinh | `LOOKUP status_threshold_config ON status_threshold_config.index_code = 'STRESS_SCORE' AND fct_sector_risk_snpst.stress_score_sector BETWEEN status_threshold_config.from_value AND status_threshold_config.to_value → status_threshold_config.status` | [SỬA 2026-09-23 — BA PTTT 14:49, Nhóm 7 34 Done / 8 Pending] BA dòng 201; phụ thuộc K_PTTT_102 (BA Pending) — cột vật lý giữ nguyên | READY |
...
| K_PTTT_233 | EPS | VND | Phái sinh | TBD — chờ Atomic | **Lý do pending:** [Nhóm 3 - Thiếu nguồn dữ liệu] nguồn `IDS.DATA/REPORT_CATALOG/RROW/RCOL/COMPANY_DATA` — chỉ có `REPORT_CATALOG` (metadata) draft trong Atomic, không có entity lưu giá trị báo cáo tài chính. **Atomic cần bổ sung:** entity chuẩn hóa giá trị báo cáo tài chính CTĐC (xem [[project_financial_report_value_eav_deprecated]]). **Mart dự kiến:** `Fact Market Statistics Snapshot` | PENDING |
| K_PTTT_234 | LNST | VND | Cơ sở | TBD — chờ Atomic | **Lý do pending:** [Nhóm 3 - Thiếu nguồn dữ liệu] cùng gap EAV báo cáo tài chính, xem K_PTTT_233 (BA Loại dữ liệu có `Dữ liệu động`). **Atomic cần bổ sung:** như trên. **Mart dự kiến:** `Fact Market Statistics Snapshot` | PENDING |
| K_PTTT_235 | P/E thị trường | Lần | Phái sinh | `K_PTTT_230 / NULLIF(K_PTTT_233, 0)` | **Lý do pending:** [Nhóm 3 - Thiếu nguồn dữ liệu] phụ thuộc K_PTTT_233 (EPS). **Atomic cần bổ sung:** như K_PTTT_233. **Mart dự kiến:** `Fact Market Statistics Snapshot` | PENDING |
...
```mermaid
flowchart LR
    fct_market_statistics_snpst["Fact Market Statistics Snapshot"] --> rpt_nh32["Data Explorer — Nhóm 32: K_PTTT_227-232,236-238"]
...
| Tên KPI | Bảng nguồn (BA) | Atomic entity dự kiến | Atomic table dự kiến |
|---|---|---|---|
| K_PTTT_233, 234, 235 | IDS.DATA / REPORT_CATALOG / RROW / RCOL / COMPANY_DATA | Entity chuẩn hóa giá trị báo cáo tài chính CTĐC (chưa thiết kế) | TBD |
...
| K_PTTT_244 | P/E theo ngành | Lần | Phái sinh | TBD — chờ Atomic | **Lý do pending:** [Nhóm 3 - Thiếu nguồn dữ liệu] phụ thuộc EPS/LNST (gap EAV báo cáo tài chính, xem Nhóm 32 K_PTTT_233). **Atomic cần bổ sung:** như Nhóm 32. **Mart dự kiến:** `Fact Sector Risk Snapshot` | PENDING |
...
| K_PTTT_234 | LNST theo ngành | VND | Cơ sở | TBD — chờ Atomic | **Lý do pending:** [Nhóm 3 - Thiếu nguồn dữ liệu] cùng gap EAV báo cáo tài chính CTĐC, xem Nhóm 32 K_PTTT_234. **Atomic cần bổ sung:** như Nhóm 32. **Mart dự kiến:** `Fact Sector Risk Snapshot` | PENDING |
...
```mermaid
flowchart LR
    fct_sector_risk_snpst["Fact Sector Risk Snapshot"] --> rpt_nh33["Data Explorer — Nhóm 33: K_PTTT_241-243, 262, 263"]
    industry_dim["Industry Dimension"] --> fct_sector_risk_snpst
    cdr_dt_dim["Calendar Date Dimension"] --> fct_sector_risk_snpst
...
| Tên KPI | Bảng nguồn (BA) | Atomic entity dự kiến | Atomic table dự kiến |
|---|---|---|---|
| K_PTTT_234, 244 | IDS.DATA/REPORT_CATALOG/RROW/RCOL/COMPANY_DATA | Entity chuẩn hóa giá trị báo cáo tài chính CTĐC (chưa thiết kế) | TBD — xem Nhóm 32 |
