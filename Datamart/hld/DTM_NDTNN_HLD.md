...
Phục vụ Tab GIÁM SÁT DÒNG VỐN Nhóm 5 — Điểm đóng cửa VN-Index (K_NDTNN_34). `Market Index Dimension` (grain 1 combo Market_Id+Market_Code, SCD4A) thay thế lưu `Market_Id`/`Market_Code` dạng text trực tiếp trên Fact trước đây — xem O_NDTNN_29. **Sửa 24/07/2026:** Fact (`fct_market_index_snpst`) và Dimension (`market_index_dim`) sở hữu bởi QLKD, NDTNN reuse — xem Cụm 6b `DTM_QLKD_HLD.md`. Fact join `Calendar Date Dimension` (qua `snpst_dt_dim_id`) và `Market Index Dimension` (qua `market_index_dim_id`).
...
| Public Company Dimension | 1 row = 1 công ty đại chúng (SCD2) — bao gồm Classification Business Line Name đệm sẵn; `Equity_Ticker_Symbol` là snapshot hiện tại (current-state), không phủ lịch sử đổi mã/nhiều loại CK — xem O_NDTNN_28 |
...
> **Ghi chú:** K_NDTNN_33 reuse trực tiếp `Fact Securities Foreign Trading Snapshot` (xem Star Schema Nhóm 2) — không cần erDiagram riêng. K_NDTNN_34 reuse `Fact Market Index Snapshot` (`fct_market_index_snpst`) sở hữu bởi QLKD (Cụm 6b, `DTM_QLKD_HLD.md`) — grain gốc QLKD 1 tháng, NDTNN filter query-time xuống grain 1 ngày trên cùng Fact, không tạo Fact riêng. `Market Index Dimension` (`market_index_dim`, grain 1 combo Market_Id+Market_Code, SCD4A) cũng sở hữu QLKD, NDTNN reuse — xem O_NDTNN_29 (Closed).
...
| Fact Market Index Snapshot (`fct_market_index_snpst`, reuse — sở hữu QLKD) | Grain gốc (QLKD): 1 chỉ số (market_code) × 1 tháng (cuối tháng). NDTNN filter query-time xuống 1 chỉ số × 1 ngày (lấy bản ghi Index Time lớn nhất trong ngày) — không phải grain vật lý riêng của NDTNN |
...
|---|---|---|---|---|---|---|
| K_NDTNN_36 | Loại hình nhà đầu tư | — | Chiều | `Foreign_Investor.Investor_Type_Code` (từ `FIMS.INVESTOR.InvestorTypeId`) | Dùng filter K_NDTNN_38/36/37. Atomic `Foreign Investor` đã READY (draft), dùng chung Nhóm 2/4/9 | READY |
...
|---|---|---|---|
| K_NDTNN_58 | Thông tin nhà đầu tư | Attribute | `Investor_Name` — FIMS.INVESTOR.Name |
| K_NDTNN_59 | Quốc tịch | Attribute | `Nationality Code` — từ FIMS.INVESTOR.NaId lookup |
| K_NDTNN_60 | Mã số giao dịch (MSGD) | Attribute | `Investor Code` = Transaction Code — FIMS.INVESTOR.TransactionCode |
| K_NDTNN_61 | Ngân hàng lưu ký | Attribute | `Custodian Bank Name` — denorm từ FIMS.BANKMONI.Name qua INVESTOR.BankAddId |
| K_NDTNN_62 | Loại hình NĐT | Attribute | `Investor Type Code` — FIMS.INVESTOR.InvestorTypeId |
| K_NDTNN_63 | Đại diện giao dịch | Attribute | `Director Name` — FIMS.INVESTOR.Director |
...
flowchart LR
    ...
    subgraph RPT["Báo cáo"]
        R1["K_NDTNN_58-62: NDTNN 360 - Nhom 11 Ho so dinh danh"]
    end
    G1 --> R1
```
...
flowchart LR
    ...
    subgraph RPT["Báo cáo"]
        R1["K_NDTNN_66-195,197: NDTNN 360 - Nhom 13 Lich su tuan thu"]
    end
    G1 --> R1
```
...
> `Report_Date` + `Security_Type_Group` — composite PK (BK trace ETL debug). `Security_Type_Group` là Classification Value nội bộ Datamart (không tồn tại trên Atomic) — 4 giá trị cố định: STOCK/BOND/FUND_CERT/TOTAL.

```mermaid
erDiagram
    Foreign_Investor_Trading_Statistics_Report {
        date Report_Date PK
        varchar Security_Type_Group PK
        float Buy_Value
        float Sell_Value
        string Source_System_Code
    }
...
> `Report_Date` + `Account_Number` + `Symbol` + `Trade_Direction_Code` — composite PK (BK trace ETL debug). `Trade_Direction_Code` (Buy/Sell) là 1 phần grain — tách từ `Securities Trade` (1 row per lệnh khớp có cả Buy và Sell).
...
erDiagram
    Foreign_Investor_Trading_Detail_Report {
        ...
        varchar Symbol PK
        varchar Trade_Direction_Code PK
