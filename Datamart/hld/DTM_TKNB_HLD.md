...
flowchart LR
    subgraph SRC["Staging"]
        S1["VSDC.uat_hnx_stg.outright_order_book"]
        S2["VSDC.uat_hnx_stg.repo_order_book"]
        S3["VSDC.uat_hnx_stg.repo2_order_book"]
    end
    subgraph SIL["Atomic"]
        A1["Bond Order Book"]
    end
    subgraph GOLD["Datamart"]
        G1["hnx02_gov_bond_otc_trading_rpt"]
    end
    S1 --> A1
    S2 --> A1
    S3 --> A1
...
flowchart LR
    subgraph SRC["Staging"]
        ...
        S2["ORDERTRADE.TRADE_BOOK_HNX"]
        S3["MDDS.JAD_MARKETINFOR"]
        S4["MDDS.JAD_CSIDXINFOR"]
    end
    subgraph SIL["Atomic"]
        A1["Securities Trade"]
        A2["Market Index Snapshot"]
        A3["Index Constituent Snapshot"]
    end
    subgraph GOLD["Datamart"]
...
**Atomic:** [CẬP NHẬT 2026-09-10] Phần lớn measure của báo cáo (Cột 1-7) vẫn nguồn từ biểu mẫu `BM 24_Dữ liệu về giao dịch TPCP theo loại hình giao dịch` chưa tích hợp CSDL — nhưng nguồn Atomic mới **`Bond Order Book`** (VSDC, `DataModel/working/Atomic/lld/VSDC/lld_VSDC_bond_order_book.yaml`, gộp 3 bảng nguồn `uat_hnx_stg.outright_order_book`/`repo_order_book`/`repo2_order_book`) nay cho phép tính **tổng KL/GT/lợi suất theo 3/7 loại hình giao dịch** (Giao dịch thông thường/mua bán lại lần 1/mua bán lại lần 2 — phân biệt qua `order_book_type_code`). Giới hạn xác nhận:
- **Chỉ tính được tổng theo loại hình**, KHÔNG breakdown được theo loại TP (TPCP/TPCP bảo lãnh/TPCQĐP) — field `instrument_type_code` trên `Bond Order Book` chưa xác nhận đúng scheme 3 giá trị này (xem comment trong LLD YAML).
- **Cột 1 (KL) và Cột 3 (Vùng lợi suất)**: tính được cho cả 3 loại hình (Outright/Repo/Repo2).
- **Cột 2 (GT)**: CHỈ tính được cho Outright (`trading_value`) — Repo/Repo2 không có field GT giao dịch tương đương (chỉ có `par_value`, không đại diện đúng giá trị giao dịch thực).
- **Dòng "Tổng toàn thị trường" (header Cột 1/2/3, K_TKNB_112/135/158)**: giữ PENDING — SUM chỉ trên 3/7 loại hình sẽ cho số liệu sai lệch/thiếu so với ý nghĩa "toàn thị trường", không đủ căn cứ báo READY.
- **Cột 4-7 (KLNN/GTNN theo NĐTNN)**: giữ nguyên PENDING — `Bond Order Book` không có field phân loại quốc tịch nhà đầu tư.
- **Loại hình GD 4-7** (vay-cho vay/hoàn trả sau vay/bán kết hợp mua lại/mua lại): giữ nguyên PENDING — `Bond Order Book` không phủ 4 loại hình này (thiếu nguồn HNX).
- Chiều `Sàn` (JAD_STOCKINFOR) và `Kỳ báo cáo` vẫn không có measure độc lập đi kèm — giữ PENDING như trước.

**Mockup:** Báo cáo HNX02 — cấu trúc bảng biểu 7 Cột, mỗi Cột lặp lại đúng layout 7 loại hình giao dịch × 3 loại TP giống nhau.

**Nguồn:** VSDC (sổ lệnh giao dịch trái phiếu OTC qua `Bond Order Book`, nguồn `uat_hnx_stg.outright_order_book`/`repo_order_book`/`repo2_order_book`).

**Mart:** `hnx02_gov_bond_otc_trading_rpt` — bảng phẳng EAV mới riêng cho báo cáo HNX02, cấu trúc giống `hnx01_stock_trading_rpt`/`hnx07_corp_bond_trading_rpt` (theo đúng quyết định thiết kế TKNB: mỗi báo cáo = 1 bảng phẳng riêng, không share vật lý dù cùng nguồn Atomic).

**Kết luận: READY một phần (7/165 KPI)** — 7 KPI (K_TKNB_113, 117, 121, 136, 159, 163, 167) có nguồn Atomic `Bond Order Book` sẵn sàng (tổng KL/GT/lợi suất theo loại hình, không breakdown loại TP). 158/165 KPI còn lại giữ PENDING (biểu mẫu HNX chưa tích hợp CSDL, hoặc thiếu dimension NĐTNN, hoặc thiếu 4/7 loại hình GD).
...
| K_TKNB_113 | 1. Giao dịch thông thường | Trái phiếu | Cơ sở | `SUM(order_bond_quantity)` FROM `bond_order_book` WHERE `order_book_type_code='OUTRIGHT'` | [STT=5, item_code=`gov_bond_otc_trading_vol_outright`] Nguồn Atomic `Bond Order Book` (VSDC). Không breakdown theo loại TP (TPCP/TPCP bảo lãnh/TPCQĐP) — field `instrument_type_code` chưa xác nhận đúng scheme | READY |
...
| K_TKNB_117 | 2. Giao dịch mua bán lại lần 1 | Trái phiếu | Cơ sở | `SUM(order_bond_quantity)` FROM `bond_order_book` WHERE `order_book_type_code='REPO'` | [STT=9, item_code=`gov_bond_otc_trading_vol_repo1`] Nguồn Atomic `Bond Order Book` (VSDC). Không breakdown theo loại TP | READY |
...
| K_TKNB_121 | 3. Giao dịch mua bán lại lần 2 | Trái phiếu | Cơ sở | `SUM(order_bond_quantity)` FROM `bond_order_book` WHERE `order_book_type_code='REPO2'` | [STT=13, item_code=`gov_bond_otc_trading_vol_repo2`] Nguồn Atomic `Bond Order Book` (VSDC). Không breakdown theo loại TP | READY |
...
| K_TKNB_136 | 1. Giao dịch thông thường | VND | Cơ sở | `SUM(trading_value)` FROM `bond_order_book` WHERE `order_book_type_code='OUTRIGHT'` | [STT=28, item_code=`gov_bond_otc_trading_val_outright`] Nguồn Atomic `Bond Order Book` (VSDC), field `trading_value` — CHỈ có ở Outright, Repo/Repo2 không có field GT tương đương (chỉ `par_value`) | READY |
...
| K_TKNB_159 | 1. Giao dịch thông thường | %/năm | Cơ sở | `AVG(yield_rate)` FROM `bond_order_book` WHERE `order_book_type_code='OUTRIGHT'` | [STT=51, item_code=`gov_bond_otc_yield_outright`] Nguồn Atomic `Bond Order Book` (VSDC), field `yield_rate` | READY |
...
| K_TKNB_163 | 2. Giao dịch mua bán lại lần 1 | %/năm | Cơ sở | `AVG(yield_rate)` FROM `bond_order_book` WHERE `order_book_type_code='REPO'` | [STT=55, item_code=`gov_bond_otc_yield_repo1`] Nguồn Atomic `Bond Order Book` (VSDC) | READY |
...
| K_TKNB_167 | 3. Giao dịch mua bán lại lần 2 | %/năm | Cơ sở | `AVG(yield_rate)` FROM `bond_order_book` WHERE `order_book_type_code='REPO2'` | [STT=59, item_code=`gov_bond_otc_yield_repo2`] Nguồn Atomic `Bond Order Book` (VSDC) | READY |
...
| Bảng nguồn BA | Atomic entity dự kiến | Atomic table dự kiến | KPI liên quan |
|---|---|---|---|
| Báo cáo HNX.BM24 (Dữ liệu về giao dịch TPCP theo loại hình giao dịch) | TBD — chưa có thiết kế Atomic | TBD | K_TKNB_110, K_TKNB_112–K_TKNB_273 (toàn bộ Cột 1-7 và sub-item, trừ K_TKNB_109 Sàn, K_TKNB_111 Kỳ báo cáo, và 7 KPI nay READY qua `Bond Order Book`: K_TKNB_113, 117, 121, 136, 159, 163, 167) |
...
- `Market Index Snapshot` (nguồn `MDDS.JAD_MARKETINFOR`) — READY, dùng cho mục 1 "Chỉ số" HNX-Index (`market_id_code='02'`) và HNXUpcom-Index (`market_id_code='04'`), giống cách Nhóm 1/5/18 đang dùng entity này. [CẬP NHẬT 2026-09-10] BA thực tế đã cung cấp nguồn (`MDDS.jad_marketinfor`, trường `marketindex`) cho HNX-Index/HNXUpcom-Index — sửa lại đánh giá "hoàn toàn thiếu trong BA" trước đây (xem K_TKNB_396, K_TKNB_398).
...
- **Mục 1 (Chỉ số) — HNXFFIndex/HNXFFUpcomIndex (free-float) vẫn THIẾU nguồn trong BA** (`Nguồn=N/A`); **mục 4-5 (KL/GT CK niêm yết-ĐKGD hiện hành)** — đã đối chiếu với biểu mẫu Excel chuẩn `HNX04 Bao cao tong hop ve quy mo TTCK.xlsx` (user cung cấp 2026-08-10) và bổ sung PENDING để giữ đúng vị trí layout.
- [CẬP NHẬT 2026-09-10] **Mục 6-11 (KL/GT CK niêm yết-ĐKGD mới/bổ sung/hủy niêm yết, 54 KPI K_TKNB_418–471) nay READY** — BA mới bổ sung nguồn thật: `Listing Status Change` (VSDC, entity mới `DataModel/working/Atomic/lld/VSDC/lld_VSDC_listing_status_change.yaml`) JOIN `Security Trading Snapshot` cho CP niêm yết/CP ĐKGD/CCQ ETF/TPDNNY; `Additional Gov Bond Listing` (VSDC, `lld_VSDC_additional_gov_bond_listing.yaml`) cho TPCP/TPCQĐP/TPCP bảo lãnh/Tín phiếu KB; `Bond Trading Registration Status` (VSDC, `lld_VSDC_bond_trading_registration_status.yaml`) cho TPDNRL. Cả 3 entity đã đăng ký `DataModel/working/Atomic/lld/manifest.yaml`. Lưu ý: BA chỉ cung cấp "Trong kỳ - Phát sinh" (không có "Cộng dồn") cho mục 6-11 — khác giả định `period_type=ca_hai` ban đầu của 77 KPI bổ sung từ Excel template; các KPI mục 6-11 nay chỉ phản ánh giá trị trong tháng, KHÔNG có biến thể lũy kế. TPDNRL không có dữ liệu cho trạng thái "bổ sung" (mục 8/9) — BA xác nhận NULL, giữ TPDN mục 8/9 = TPDNNY only.
- Mục 12-16, 19-26 và các sub-item TPCQĐP/TPCPBL/Tín phiếu KB của mục 2,3 (biểu mẫu BM24/VSDC.BM1/BM32/BM23/BM34/BM25) vẫn PENDING — chưa có Atomic tương ứng.
...
**Kết luận: 79 KPI READY** (23 KPI gốc: mục 2.1/2.2/2.3/2.6/2.8, mục 3.1/3.2/3.3/3.6/3.8, mục 17.1/17.2, 18.1/18.2, dòng Chiều Sàn/Ngày GD/Kỳ báo cáo/Loại CK; + 54 KPI mới mục 6-11 theo mapping BA 2026-09-10; + 2 KPI mục 1 HNX-Index/HNXUpcom-Index theo mapping BA 2026-09-10 — xem K_TKNB_396/398), **103 KPI PENDING** (mục 1 header + HNXFFIndex/HNXFFUpcomIndex, mục 4-5, 12-16, 19-26 toàn bộ biểu mẫu; các sub-item TPCQĐP/TPCPBL/Tín phiếu KB của mục 2,3 theo biểu mẫu BM24; mục 19 PENDING theo dây chuyền do phụ thuộc mẫu số mục 16 PENDING dù tử số mục 18 READY).
...
| K_TKNB_313 | 12. Vốn hóa thị trường (VND) | VND | Phái sinh | `= K_TKNB_314 + K_TKNB_315` | **[SỬA 2026-09-16, đóng PENDING — commit b8868c9e]** [STT=8, item_code=`mkt_scale_market_cap_total`, period_type=ca_hai] Nguồn `listed_share_info` (VSDC, cùng nguồn vừa mở cho GSTT, xem O_GSTT_22). Header derived-sum của K_TKNB_314 (HNX)+K_TKNB_315 (UPCoM). Mart: `hnx04_market_scale_rpt`."Item Value" | READY |
| K_TKNB_314 | 12.1 Cổ phiếu niêm yết (Vốn hóa) | VND | Cơ sở | `SUM(security_trading_snapshot.close_price × listed_share_info.outstanding_share_quantity) WHERE floor_code = '02' AND trading_dt = :to_date` | **[SỬA 2026-09-16, đóng PENDING]** [STT=8.1, item_code=`mkt_scale_market_cap_stock_listed`, period_type=ca_hai] Nguồn `listed_share_info`. Mart: `hnx04_market_scale_rpt` | READY |
| K_TKNB_315 | 12.2 Cổ phiếu đăng ký giao dịch (Vốn hóa) | VND | Cơ sở | `SUM(security_trading_snapshot.close_price × listed_share_info.outstanding_share_quantity) WHERE floor_code = '04' AND trading_dt = :to_date` | **[SỬA 2026-09-16, đóng PENDING]** [STT=8.2, item_code=`mkt_scale_market_cap_stock_registered`, period_type=ca_hai] Nguồn `listed_share_info` (UPCoM = "đăng ký giao dịch"). Mart: `hnx04_market_scale_rpt` | READY |
...
| K_TKNB_395 | 1. Chỉ số | Điểm | Phái sinh | Header = tổng/trung bình 4 chỉ số con (1.1 HNX-Index, HNXFFIndex, 1.2 HNXUpcom-Index, HNXFFUpcomIndex) | [STT=23, item_code=`mkt_scale_index_total`, period_type=trong_ky] [CẬP NHẬT 2026-09-10] BA đã cung cấp nguồn thật cho HNX-Index/HNXUpcom-Index (xem K_TKNB_396/398) nhưng HNXFFIndex/HNXFFUpcomIndex (free-float) vẫn chưa có nguồn (`Nguồn=N/A`) — header giữ PENDING theo dây chuyền vì phụ thuộc đủ 4 sub-item; chỉ có 2 cột (Trong kỳ-phát sinh, Trong kỳ-Tăng/giảm), KHÔNG có Cộng dồn vì chỉ số không lũy kế được | PENDING |
| K_TKNB_396 | 1.1 HNX-Index | Điểm | Cơ sở | `market_index_val` (giá trị) và `index_percent_change` (Trong kỳ - Tăng/giảm %) FROM `market_index_snapshot` WHERE `market_id_code = '02'` AND `trading_dt = :to_date`, lấy bản ghi `index_time` mới nhất | [STT=23.1, item_code=`mkt_scale_index_hnx`, period_type=trong_ky] [CẬP NHẬT 2026-09-10] BA đã cung cấp nguồn thật: `Nguồn=MDDS`, `Bảng nguồn=MDDS.jad_marketinfor`, `Trường nguồn=marketindex` (dòng BA "Chỉ số - Trong kỳ - phát sinh - HNX-Index" và "...Tăng/Giảm - HNX-Index", `Trạng thái mapping=Done`). Atomic `Market Index Snapshot` đã READY, cùng entity/filter `market_id_code='02'` đang dùng ở K_TKNB_4 (Nhóm 1) — không cần Atomic mới, sửa lại đánh giá PENDING/TBD trước đây | READY |
| K_TKNB_397 | HNXFFIndex | Điểm | Cơ sở | Chưa xác định — chờ nguồn Atomic thật | [STT=23.1b, item_code=`mkt_scale_index_hnx_ff`, period_type=trong_ky] BA xác nhận `Nguồn=N/A` cho free-float index (dòng con của 1.1, không có số hiển thị riêng trong template) — thật sự chưa có nguồn, không phải do thiếu khảo sát; Atomic entity dự kiến TBD; Mart dự kiến: `hnx04_market_scale_rpt` (bảng phẳng riêng cho báo cáo HNX04, theo quyết định thiết kế TKNB) | PENDING |
| K_TKNB_398 | 1.2 HNXUpcom-Index | Điểm | Cơ sở | `market_index_val` (giá trị) và `index_percent_change` (Trong kỳ - Tăng/giảm %) FROM `market_index_snapshot` WHERE `market_id_code = '04'` AND `trading_dt = :to_date`, lấy bản ghi `index_time` mới nhất | [STT=23.2, item_code=`mkt_scale_index_hnx_upcom`, period_type=trong_ky] [CẬP NHẬT 2026-09-10] BA đã cung cấp nguồn thật: `Nguồn=MDDS`, `Bảng nguồn=MDDS.jad_marketinfor`, `Trường nguồn=marketindex` (dòng BA "Chỉ số - Trong kỳ - phát sinh - HNXUpcom-Index" và "...Tăng/Giảm - HNXUpcom-Index", `Trạng thái mapping=Done`). Atomic `Market Index Snapshot` đã READY, cùng entity/filter `market_id_code='04'` đang dùng ở K_TKNB_6 (Nhóm 1) — không cần Atomic mới, sửa lại đánh giá PENDING/TBD trước đây | READY |
| K_TKNB_399 | HNXFFUpcomIndex | Điểm | Cơ sở | Chưa xác định — chờ nguồn Atomic thật | [STT=23.2b, item_code=`mkt_scale_index_hnx_ff_upcom`, period_type=trong_ky] BA xác nhận `Nguồn=N/A` cho free-float index (dòng con của 1.2) — thật sự chưa có nguồn, không phải do thiếu khảo sát; Atomic entity dự kiến TBD; Mart dự kiến: `hnx04_market_scale_rpt` (bảng phẳng riêng cho báo cáo HNX04, theo quyết định thiết kế TKNB) | PENDING |
...
| K_TKNB_418 | 6. KL CK niêm yết và ĐKGD mới (chứng khoán) | Chứng khoán | Phái sinh | = K_TKNB_419 + K_TKNB_420 + K_TKNB_421 + K_TKNB_422 + K_TKNB_423 + K_TKNB_424 + K_TKNB_425 + K_TKNB_426 | [STT=6, item_code=`mkt_scale_new_listed_registered_vol_total`] Header — SUM 8 sub-item (6.1..6.8). Nay đủ điều kiện READY vì toàn bộ sub-item đã có nguồn Atomic thật. | READY |
| K_TKNB_419 | 6.1 Cổ phiếu niêm yết mới (số lượng) | Chứng khoán | Cơ sở | SUM(sc.new_listed_share_quantity) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='ST' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=6.1, item_code=`mkt_scale_new_listed_registered_vol_stock_listed`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Cổ phiếu niêm yết mới (số lượng). | READY |
| K_TKNB_420 | 6.2 Cổ phiếu đăng ký giao dịch mới (số lượng) | Chứng khoán | Cơ sở | SUM(sc.new_listed_share_quantity) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='04' AND s.stock_tp_code='ST' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=6.2, item_code=`mkt_scale_new_listed_registered_vol_stock_registered`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Cổ phiếu đăng ký giao dịch mới (số lượng). | READY |
| K_TKNB_421 | 6.3 Trái phiếu chính phủ mới (số lượng) | Chứng khoán | Cơ sở | SUM(b.additional_bond_quantity) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết mới' AND b.bond_tp_code='Trái phiếu chính phủ' | [STT=6.3, item_code=`mkt_scale_new_listed_registered_vol_gov_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết mới'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_422 | 6.4 Trái phiếu chính quyền địa phương mới (số lượng) | Chứng khoán | Cơ sở | SUM(b.additional_bond_quantity) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết mới' AND b.bond_tp_code='Trái phiếu chính quyền địa phương' | [STT=6.4, item_code=`mkt_scale_new_listed_registered_vol_local_gov_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết mới'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_423 | 6.5 Trái phiếu chính phủ bảo lãnh mới (số lượng) | Chứng khoán | Cơ sở | SUM(b.additional_bond_quantity) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết mới' AND b.bond_tp_code='Trái phiếu chính phủ bảo lãnh' | [STT=6.5, item_code=`mkt_scale_new_listed_registered_vol_gov_guar_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết mới'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_424 | 6.6 Trái phiếu doanh nghiệp mới (số lượng) | Chứng khoán | Cơ sở | TPDNNY: SUM(sc.new_listed_share_quantity) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='BO'; TPDNRL: SUM(registered_trading_bond_quantity) FROM bond_trading_registration_status WHERE trading_registration_status='Đăng ký giao dịch'; Tổng TPDN = TPDNNY + TPDNRL | [STT=6.6, item_code=`mkt_scale_new_listed_registered_vol_corp_bond`] TPDNNY: `Listing Status Change` JOIN `Security Trading Snapshot`. TPDNRL: `Bond Trading Registration Status` filter `trading_registration_status='Đăng ký giao dịch'`. Tổng = TPDNNY + TPDNRL. | READY |
| K_TKNB_425 | 6.7 Tín phiếu kho bạc mới (số lượng) | Chứng khoán | Cơ sở | SUM(b.additional_bond_quantity) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết mới' AND b.bond_tp_code='Tín phiếu kho bạc' | [STT=6.7, item_code=`mkt_scale_new_listed_registered_vol_tbill`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết mới'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_426 | 6.8 Chứng chỉ quỹ ETF mới (số lượng) | Chứng khoán | Cơ sở | SUM(sc.new_listed_share_quantity) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='EF' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=6.8, item_code=`mkt_scale_new_listed_registered_vol_etf`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Chứng chỉ quỹ ETF mới (số lượng). | READY |
| K_TKNB_427 | 7. Giá trị CK niêm yết và ĐKGD mới (VND) | VND | Phái sinh | = K_TKNB_428 + K_TKNB_429 + K_TKNB_430 + K_TKNB_431 + K_TKNB_432 + K_TKNB_433 + K_TKNB_434 + K_TKNB_435 | [STT=7, item_code=`mkt_scale_new_listed_registered_value_total`] Header — SUM 8 sub-item (7.1..7.8). Nay đủ điều kiện READY vì toàn bộ sub-item đã có nguồn Atomic thật. | READY |
| K_TKNB_428 | 7.1 Cổ phiếu niêm yết mới (giá trị) | VND | Cơ sở | SUM(sc.listed_share_value) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='ST' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=7.1, item_code=`mkt_scale_new_listed_registered_value_stock_listed`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Cổ phiếu niêm yết mới (giá trị). | READY |
| K_TKNB_429 | 7.2 Cổ phiếu đăng ký giao dịch mới (giá trị) | VND | Cơ sở | SUM(sc.listed_share_value) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='04' AND s.stock_tp_code='ST' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=7.2, item_code=`mkt_scale_new_listed_registered_value_stock_registered`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Cổ phiếu đăng ký giao dịch mới (giá trị). | READY |
| K_TKNB_430 | 7.3 Trái phiếu chính phủ mới (giá trị) | VND | Cơ sở | SUM(b.additional_bond_quantity * b.par_value) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết mới' AND b.bond_tp_code='Trái phiếu chính phủ' | [STT=7.3, item_code=`mkt_scale_new_listed_registered_value_gov_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết mới'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_431 | 7.4 Trái phiếu chính quyền địa phương mới (giá trị) | VND | Cơ sở | SUM(b.additional_bond_quantity * b.par_value) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết mới' AND b.bond_tp_code='Trái phiếu chính quyền địa phương' | [STT=7.4, item_code=`mkt_scale_new_listed_registered_value_local_gov_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết mới'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_432 | 7.5 Trái phiếu chính phủ bảo lãnh mới (giá trị) | VND | Cơ sở | SUM(b.additional_bond_quantity * b.par_value) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết mới' AND b.bond_tp_code='Trái phiếu chính phủ bảo lãnh' | [STT=7.5, item_code=`mkt_scale_new_listed_registered_value_gov_guar_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết mới'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_433 | 7.6 Trái phiếu doanh nghiệp mới (giá trị) | VND | Cơ sở | TPDNNY: SUM(sc.listed_share_value) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='BO'; TPDNRL: SUM(registered_trading_bond_quantity * par_value) FROM bond_trading_registration_status WHERE trading_registration_status='Đăng ký giao dịch'; Tổng TPDN = TPDNNY + TPDNRL | [STT=7.6, item_code=`mkt_scale_new_listed_registered_value_corp_bond`] TPDNNY: `Listing Status Change` JOIN `Security Trading Snapshot`. TPDNRL: `Bond Trading Registration Status` filter `trading_registration_status='Đăng ký giao dịch'`. Tổng = TPDNNY + TPDNRL. | READY |
| K_TKNB_434 | 7.7 Tín phiếu kho bạc mới (giá trị) | VND | Cơ sở | SUM(b.additional_bond_quantity * b.par_value) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết mới' AND b.bond_tp_code='Tín phiếu kho bạc' | [STT=7.7, item_code=`mkt_scale_new_listed_registered_value_tbill`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết mới'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_435 | 7.8 Chứng chỉ quỹ ETF mới (giá trị) | VND | Cơ sở | SUM(sc.listed_share_value) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='EF' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=7.8, item_code=`mkt_scale_new_listed_registered_value_etf`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Chứng chỉ quỹ ETF mới (giá trị). | READY |
| K_TKNB_436 | 8. KL CK niêm yết và ĐKGD bổ sung (chứng khoán) | Chứng khoán | Phái sinh | = K_TKNB_437 + K_TKNB_438 + K_TKNB_439 + K_TKNB_440 + K_TKNB_441 + K_TKNB_442 + K_TKNB_443 + K_TKNB_444 | [STT=8, item_code=`mkt_scale_additional_listed_registered_vol_total`] Header — SUM 8 sub-item (8.1..8.8). Nay đủ điều kiện READY vì toàn bộ sub-item đã có nguồn Atomic thật. | READY |
| K_TKNB_437 | 8.1 Cổ phiếu niêm yết bổ sung (số lượng) | Chứng khoán | Cơ sở | SUM(sc.additional_listed_share_quantity) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='ST' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=8.1, item_code=`mkt_scale_additional_listed_registered_vol_stock_listed`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Cổ phiếu niêm yết bổ sung (số lượng). | READY |
| K_TKNB_438 | 8.2 Cổ phiếu đăng ký giao dịch bổ sung (số lượng) | Chứng khoán | Cơ sở | SUM(sc.additional_listed_share_quantity) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='04' AND s.stock_tp_code='ST' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=8.2, item_code=`mkt_scale_additional_listed_registered_vol_stock_registered`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Cổ phiếu đăng ký giao dịch bổ sung (số lượng). | READY |
| K_TKNB_439 | 8.3 Trái phiếu chính phủ bổ sung (số lượng) | Chứng khoán | Cơ sở | SUM(b.additional_bond_quantity) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết bổ sung' AND b.bond_tp_code='Trái phiếu chính phủ' | [STT=8.3, item_code=`mkt_scale_additional_listed_registered_vol_gov_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết bổ sung'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_440 | 8.4 Trái phiếu chính quyền địa phương bổ sung (số lượng) | Chứng khoán | Cơ sở | SUM(b.additional_bond_quantity) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết bổ sung' AND b.bond_tp_code='Trái phiếu chính quyền địa phương' | [STT=8.4, item_code=`mkt_scale_additional_listed_registered_vol_local_gov_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết bổ sung'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_441 | 8.5 Trái phiếu chính phủ bảo lãnh bổ sung (số lượng) | Chứng khoán | Cơ sở | SUM(b.additional_bond_quantity) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết bổ sung' AND b.bond_tp_code='Trái phiếu chính phủ bảo lãnh' | [STT=8.5, item_code=`mkt_scale_additional_listed_registered_vol_gov_guar_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết bổ sung'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_442 | 8.6 Trái phiếu doanh nghiệp bổ sung (số lượng) | Chứng khoán | Cơ sở | TPDNNY: SUM(sc.additional_listed_share_quantity) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='BO'; TPDNRL: Không có dữ liệu (BA ghi NULL cho trường hợp bổ sung) — Tổng TPDN = TPDNNY (TPDNRL=0) | [STT=8.6, item_code=`mkt_scale_additional_listed_registered_vol_corp_bond`] TPDNNY: `Listing Status Change` JOIN `Security Trading Snapshot` filter CP niêm yết TPDN (floor_code='02', stock_tp_code='BO'). TPDNRL: PENDING riêng — BA xác nhận không có dữ liệu bổ sung cho TPDNRL (`registration_status_change` không có state 'bổ sung'). | READY |
| K_TKNB_443 | 8.7 Tín phiếu kho bạc bổ sung (số lượng) | Chứng khoán | Cơ sở | SUM(b.additional_bond_quantity) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết bổ sung' AND b.bond_tp_code='Tín phiếu kho bạc' | [STT=8.7, item_code=`mkt_scale_additional_listed_registered_vol_tbill`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết bổ sung'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_444 | 8.8 Chứng chỉ quỹ ETF bổ sung (số lượng) | Chứng khoán | Cơ sở | SUM(sc.additional_listed_share_quantity) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='EF' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=8.8, item_code=`mkt_scale_additional_listed_registered_vol_etf`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Chứng chỉ quỹ ETF bổ sung (số lượng). | READY |
| K_TKNB_445 | 9. Giá trị CK niêm yết và ĐKGD bổ sung (VND) | VND | Phái sinh | = K_TKNB_446 + K_TKNB_447 + K_TKNB_448 + K_TKNB_449 + K_TKNB_450 + K_TKNB_451 + K_TKNB_452 + K_TKNB_453 | [STT=9, item_code=`mkt_scale_additional_listed_registered_value_total`] Header — SUM 8 sub-item (9.1..9.8). Nay đủ điều kiện READY vì toàn bộ sub-item đã có nguồn Atomic thật. | READY |
| K_TKNB_446 | 9.1 Cổ phiếu niêm yết bổ sung (giá trị) | VND | Cơ sở | SUM(sc.additional_listed_share_value) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='ST' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=9.1, item_code=`mkt_scale_additional_listed_registered_value_stock_listed`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Cổ phiếu niêm yết bổ sung (giá trị). | READY |
| K_TKNB_447 | 9.2 Cổ phiếu đăng ký giao dịch bổ sung (giá trị) | VND | Cơ sở | SUM(sc.additional_listed_share_value) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='04' AND s.stock_tp_code='ST' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=9.2, item_code=`mkt_scale_additional_listed_registered_value_stock_registered`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Cổ phiếu đăng ký giao dịch bổ sung (giá trị). | READY |
| K_TKNB_448 | 9.3 Trái phiếu chính phủ bổ sung (giá trị) | VND | Cơ sở | SUM(b.additional_bond_quantity * b.par_value) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết bổ sung' AND b.bond_tp_code='Trái phiếu chính phủ' | [STT=9.3, item_code=`mkt_scale_additional_listed_registered_value_gov_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết bổ sung'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_449 | 9.4 Trái phiếu chính quyền địa phương bổ sung (giá trị) | VND | Cơ sở | SUM(b.additional_bond_quantity * b.par_value) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết bổ sung' AND b.bond_tp_code='Trái phiếu chính quyền địa phương' | [STT=9.4, item_code=`mkt_scale_additional_listed_registered_value_local_gov_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết bổ sung'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_450 | 9.5 Trái phiếu chính phủ bảo lãnh bổ sung (giá trị) | VND | Cơ sở | SUM(b.additional_bond_quantity * b.par_value) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết bổ sung' AND b.bond_tp_code='Trái phiếu chính phủ bảo lãnh' | [STT=9.5, item_code=`mkt_scale_additional_listed_registered_value_gov_guar_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết bổ sung'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_451 | 9.6 Trái phiếu doanh nghiệp bổ sung (giá trị) | VND | Cơ sở | TPDNNY: SUM(sc.additional_listed_share_value) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='BO'; TPDNRL: Không có dữ liệu (BA ghi NULL cho trường hợp bổ sung) — Tổng TPDN = TPDNNY (TPDNRL=0) | [STT=9.6, item_code=`mkt_scale_additional_listed_registered_value_corp_bond`] TPDNNY: `Listing Status Change` JOIN `Security Trading Snapshot` filter CP niêm yết TPDN (floor_code='02', stock_tp_code='BO'). TPDNRL: PENDING riêng — BA xác nhận không có dữ liệu bổ sung cho TPDNRL (`registration_status_change` không có state 'bổ sung'). | READY |
| K_TKNB_452 | 9.7 Tín phiếu kho bạc bổ sung (giá trị) | VND | Cơ sở | SUM(b.additional_bond_quantity * b.par_value) FROM additional_gov_bond_listing b WHERE b.listing_status='Niêm yết bổ sung' AND b.bond_tp_code='Tín phiếu kho bạc' | [STT=9.7, item_code=`mkt_scale_additional_listed_registered_value_tbill`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Niêm yết bổ sung'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_453 | 9.8 Chứng chỉ quỹ ETF bổ sung (giá trị) | VND | Cơ sở | SUM(sc.additional_listed_share_value) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='EF' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=9.8, item_code=`mkt_scale_additional_listed_registered_value_etf`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Chứng chỉ quỹ ETF bổ sung (giá trị). | READY |
| K_TKNB_454 | 10. KL CK niêm yết và ĐKGD hủy niêm yết (chứng khoán) | Chứng khoán | Phái sinh | = K_TKNB_455 + K_TKNB_456 + K_TKNB_457 + K_TKNB_458 + K_TKNB_459 + K_TKNB_460 + K_TKNB_461 + K_TKNB_462 | [STT=10, item_code=`mkt_scale_delisted_listed_registered_vol_total`] Header — SUM 8 sub-item (10.1..10.8). Nay đủ điều kiện READY vì toàn bộ sub-item đã có nguồn Atomic thật. | READY |
| K_TKNB_455 | 10.1 Cổ phiếu niêm yết (hủy NY, số lượng) | Chứng khoán | Cơ sở | SUM(sc.delisted_share_quantity) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='ST' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=10.1, item_code=`mkt_scale_delisted_listed_registered_vol_stock_listed`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Cổ phiếu niêm yết (hủy NY, số lượng). | READY |
| K_TKNB_456 | 10.2 Cổ phiếu đăng ký giao dịch (hủy NY, số lượng) | Chứng khoán | Cơ sở | SUM(sc.delisted_share_quantity) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='04' AND s.stock_tp_code='ST' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=10.2, item_code=`mkt_scale_delisted_listed_registered_vol_stock_registered`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Cổ phiếu đăng ký giao dịch (hủy NY, số lượng). | READY |
| K_TKNB_457 | 10.3 Trái phiếu chính phủ (hủy NY, số lượng) | Chứng khoán | Cơ sở | SUM(b.additional_bond_quantity) FROM additional_gov_bond_listing b WHERE b.listing_status='Hủy niêm yết' AND b.bond_tp_code='Trái phiếu chính phủ' | [STT=10.3, item_code=`mkt_scale_delisted_listed_registered_vol_gov_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Hủy niêm yết'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_458 | 10.4 Trái phiếu chính quyền địa phương (hủy NY, số lượng) | Chứng khoán | Cơ sở | SUM(b.additional_bond_quantity) FROM additional_gov_bond_listing b WHERE b.listing_status='Hủy niêm yết' AND b.bond_tp_code='Trái phiếu chính quyền địa phương' | [STT=10.4, item_code=`mkt_scale_delisted_listed_registered_vol_local_gov_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Hủy niêm yết'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_459 | 10.5 Trái phiếu chính phủ bảo lãnh (hủy NY, số lượng) | Chứng khoán | Cơ sở | SUM(b.additional_bond_quantity) FROM additional_gov_bond_listing b WHERE b.listing_status='Hủy niêm yết' AND b.bond_tp_code='Trái phiếu chính phủ bảo lãnh' | [STT=10.5, item_code=`mkt_scale_delisted_listed_registered_vol_gov_guar_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Hủy niêm yết'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_460 | 10.6 Trái phiếu doanh nghiệp (hủy NY, số lượng) | Chứng khoán | Cơ sở | TPDNNY: SUM(sc.delisted_share_quantity) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='BO'; TPDNRL: SUM(registered_trading_bond_quantity) FROM bond_trading_registration_status WHERE trading_registration_status='Hủy đăng ký giao dịch'; Tổng TPDN = TPDNNY + TPDNRL | [STT=10.6, item_code=`mkt_scale_delisted_listed_registered_vol_corp_bond`] TPDNNY: `Listing Status Change` JOIN `Security Trading Snapshot`. TPDNRL: `Bond Trading Registration Status` filter `trading_registration_status='Hủy đăng ký giao dịch'`. Tổng = TPDNNY + TPDNRL. | READY |
| K_TKNB_461 | 10.7 Tín phiếu kho bạc (hủy NY, số lượng) | Chứng khoán | Cơ sở | SUM(b.additional_bond_quantity) FROM additional_gov_bond_listing b WHERE b.listing_status='Hủy niêm yết' AND b.bond_tp_code='Tín phiếu kho bạc' | [STT=10.7, item_code=`mkt_scale_delisted_listed_registered_vol_tbill`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Hủy niêm yết'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_462 | 10.8 Chứng chỉ quỹ ETF (hủy NY, số lượng) | Chứng khoán | Cơ sở | SUM(sc.delisted_share_quantity) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='EF' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=10.8, item_code=`mkt_scale_delisted_listed_registered_vol_etf`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Chứng chỉ quỹ ETF (hủy NY, số lượng). | READY |
| K_TKNB_463 | 11. Giá trị CK niêm yết và ĐKGD hủy niêm yết (VND) | VND | Phái sinh | = K_TKNB_464 + K_TKNB_465 + K_TKNB_466 + K_TKNB_467 + K_TKNB_468 + K_TKNB_469 + K_TKNB_470 + K_TKNB_471 | [STT=11, item_code=`mkt_scale_delisted_listed_registered_value_total`] Header — SUM 8 sub-item (11.1..11.8). Nay đủ điều kiện READY vì toàn bộ sub-item đã có nguồn Atomic thật. | READY |
| K_TKNB_464 | 11.1 Cổ phiếu niêm yết (hủy NY, giá trị) | VND | Cơ sở | SUM(sc.delisted_share_value) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='ST' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=11.1, item_code=`mkt_scale_delisted_listed_registered_value_stock_listed`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Cổ phiếu niêm yết (hủy NY, giá trị). | READY |
| K_TKNB_465 | 11.2 Cổ phiếu đăng ký giao dịch (hủy NY, giá trị) | VND | Cơ sở | SUM(sc.delisted_share_value) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='04' AND s.stock_tp_code='ST' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=11.2, item_code=`mkt_scale_delisted_listed_registered_value_stock_registered`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Cổ phiếu đăng ký giao dịch (hủy NY, giá trị). | READY |
| K_TKNB_466 | 11.3 Trái phiếu chính phủ (hủy NY, giá trị) | VND | Cơ sở | SUM(b.additional_bond_quantity * b.par_value) FROM additional_gov_bond_listing b WHERE b.listing_status='Hủy niêm yết' AND b.bond_tp_code='Trái phiếu chính phủ' | [STT=11.3, item_code=`mkt_scale_delisted_listed_registered_value_gov_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Hủy niêm yết'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_467 | 11.4 Trái phiếu chính quyền địa phương (hủy NY, giá trị) | VND | Cơ sở | SUM(b.additional_bond_quantity * b.par_value) FROM additional_gov_bond_listing b WHERE b.listing_status='Hủy niêm yết' AND b.bond_tp_code='Trái phiếu chính quyền địa phương' | [STT=11.4, item_code=`mkt_scale_delisted_listed_registered_value_local_gov_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Hủy niêm yết'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_468 | 11.5 Trái phiếu chính phủ bảo lãnh (hủy NY, giá trị) | VND | Cơ sở | SUM(b.additional_bond_quantity * b.par_value) FROM additional_gov_bond_listing b WHERE b.listing_status='Hủy niêm yết' AND b.bond_tp_code='Trái phiếu chính phủ bảo lãnh' | [STT=11.5, item_code=`mkt_scale_delisted_listed_registered_value_gov_guar_bond`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Hủy niêm yết'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_469 | 11.6 Trái phiếu doanh nghiệp (hủy NY, giá trị) | VND | Cơ sở | TPDNNY: SUM(sc.delisted_share_value) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='BO'; TPDNRL: SUM(registered_trading_bond_quantity * par_value) FROM bond_trading_registration_status WHERE trading_registration_status='Hủy đăng ký giao dịch'; Tổng TPDN = TPDNNY + TPDNRL | [STT=11.6, item_code=`mkt_scale_delisted_listed_registered_value_corp_bond`] TPDNNY: `Listing Status Change` JOIN `Security Trading Snapshot`. TPDNRL: `Bond Trading Registration Status` filter `trading_registration_status='Hủy đăng ký giao dịch'`. Tổng = TPDNNY + TPDNRL. | READY |
| K_TKNB_470 | 11.7 Tín phiếu kho bạc (hủy NY, giá trị) | VND | Cơ sở | SUM(b.additional_bond_quantity * b.par_value) FROM additional_gov_bond_listing b WHERE b.listing_status='Hủy niêm yết' AND b.bond_tp_code='Tín phiếu kho bạc' | [STT=11.7, item_code=`mkt_scale_delisted_listed_registered_value_tbill`] Nguồn Atomic `Additional Gov Bond Listing` (VSDC), filter `listing_status='Hủy niêm yết'` GROUP BY `bond_tp_code`. | READY |
| K_TKNB_471 | 11.8 Chứng chỉ quỹ ETF (hủy NY, giá trị) | VND | Cơ sở | SUM(sc.delisted_share_value) FROM listing_status_change sc JOIN security_trading_snapshot s ON sc.ticker_symbol=s.symbol AND DATE_FORMAT(sc.effective_dt,'yyyy-MM')=DATE_FORMAT(s.trading_dt,'yyyy-MM') WHERE s.floor_code='02' AND s.stock_tp_code='EF' (scheme MDDS_STOCK_TYPE chưa profile đủ values — dùng code ST/BO/EF theo mô tả sơ bộ, xác nhận lại khi profile MDDS.StockInfor) | [STT=11.8, item_code=`mkt_scale_delisted_listed_registered_value_etf`] Nguồn Atomic `Listing Status Change` (VSDC) JOIN `Security Trading Snapshot` (MDDS) theo ticker_symbol+tháng. Chứng chỉ quỹ ETF (hủy NY, giá trị). | READY |
**Bảng mapping nguồn (Atomic Placeholder — cho dòng PENDING):**
...
| Free-float Index (HNXFFIndex/HNXFFUpcomIndex — mục 1 Chỉ số, Sheet1 Excel HNX04) | TBD — chưa có thiết kế Atomic | TBD | K_TKNB_395 (header, PENDING theo dây chuyền), K_TKNB_397, K_TKNB_399 — BA xác nhận `Nguồn=N/A`, chưa có nguồn thật. HNX-Index/HNXUpcom-Index (K_TKNB_396/398) đã READY dùng `Market Index Snapshot` |
| Excel template HNX04 Sheet3 (mục 4-5 KL/GT CK niêm yết-ĐKGD hiện hành) | TBD — chưa có thiết kế Atomic | TBD | K_TKNB_400–417 — BA chưa cung cấp, bổ sung từ biểu mẫu Excel chuẩn |
...
| K_TKNB_588 | 6. Tổng giá trị vốn hóa thị trường cổ phiếu | Tỷ VND | Phái sinh | `= K_TKNB_589` | **[SỬA 2026-09-16, đóng PENDING — commit b8868c9e]** [STT=18, item_code=`total_market_cap_hose`] Nguồn `listed_share_info` (VSDC, cùng nguồn vừa mở cho GSTT, xem O_GSTT_22). Header passthrough của K_TKNB_589 (HOSE). Mart: `hsx01_stock_trading_rpt`."Item Value" | READY |
| K_TKNB_589 | 6.1 Cổ phiếu niêm yết | Tỷ VND | Cơ sở | `SUM(security_trading_snapshot.close_price × listed_share_info.outstanding_share_quantity) WHERE floor_code = '10' AND trading_dt = :to_date` | **[SỬA 2026-09-16, đóng PENDING]** [STT=19, item_code=`listed_stock_market_cap_hose`] Nguồn `listed_share_info` (VSDC), lookback bản ghi gần nhất `<= trading_dt`. Mart: `hsx01_stock_trading_rpt` | READY |
...
- `Securities Company` (nguồn `SCMS.SC_FIRM_INFO`) — READY, số lượng CTCK cấp phép. Resolved 2026-09-04 — dùng JOIN `Classification Firm Status` (`cl_firm_status`) filter `cl_firm_status_nm = 'Đang hoạt động'` thay cho field `BUSINESS_LICENSE_DATE` (không tồn tại), xem K_TKNB_1010.
...
| K_TKNB_923 | Tổng giá trị vốn hóa thị trường | Tỷ đồng | Phái sinh | `= K_TKNB_924 + K_TKNB_925 + K_TKNB_926` | **[SỬA 2026-09-16, đóng PENDING — commit b8868c9e]** [STT=6, item_code=`total_market_capitalization`] Nguồn `listed_share_info` (VSDC, cùng nguồn vừa mở cho GSTT, xem O_GSTT_22). Header derived-sum của K_TKNB_924 (HOSE)+925 (HNX)+926 (UPCoM). Mart: `tkniengiam_market_annual_rpt`."Item Value" | READY |
| K_TKNB_924 | - SGDCK HCM (vốn hóa) | Tỷ đồng | Cơ sở | `SUM(security_trading_snapshot.close_price × listed_share_info.outstanding_share_quantity) WHERE floor_code = '10' AND trading_dt = :to_date` | **[SỬA 2026-09-16, đóng PENDING, giải quyết nghi vấn lệch nhãn]** [STT=7, item_code=`market_cap_hcm`] Nghi vấn trước đây (BA ghi dieu_kien "group by HNX" cho nhãn HCM) đã được xác nhận đúng khi build công thức thật: HCM = HOSE = `floor_code='10'`. Nguồn `listed_share_info`. Mart: `tkniengiam_market_annual_rpt` | READY |
| K_TKNB_925 | - SGDCK HN (vốn hóa) | Tỷ đồng | Cơ sở | `SUM(security_trading_snapshot.close_price × listed_share_info.outstanding_share_quantity) WHERE floor_code = '02' AND trading_dt = :to_date` | **[SỬA 2026-09-16, đóng PENDING, giải quyết nghi vấn lệch nhãn]** [STT=8, item_code=`market_cap_hn`] Nghi vấn trước đây (BA ghi dieu_kien "Group by UpCom" cho nhãn HN) đã được xác nhận đúng khi build công thức thật: HN = HNX = `floor_code='02'`. Nguồn `listed_share_info`. Mart: `tkniengiam_market_annual_rpt` | READY |
| K_TKNB_926 | - Thị trường đăng ký giao dịch UPCoM (vốn hóa) | Tỷ đồng | Cơ sở | `SUM(security_trading_snapshot.close_price × listed_share_info.outstanding_share_quantity) WHERE floor_code = '04' AND trading_dt = :to_date` | **[SỬA 2026-09-16, đóng PENDING]** [STT=9, item_code=`market_cap_upcom`] Nguồn `listed_share_info`. Mart: `tkniengiam_market_annual_rpt` | READY |
...
| K_TKNB_1010 | Số lượng công ty chứng khoán được cấp phép | CTCK | Cơ sở | `COUNT DISTINCT securities_company.sc_id JOIN cl_firm_status ON securities_company.cl_firm_status_id = cl_firm_status.cl_firm_status_id WHERE cl_firm_status.cl_firm_status_nm = 'Đang hoạt động'` | [STT=93, item_code=`licensed_securities_company_count`] **Resolved 2026-09-04:** Đã chốt filter thay cho field `BUSINESS_LICENSE_DATE` (BA dùng nhưng không tồn tại trên `securities_company` — chỉ có `foundation_license_file_path`, đường dẫn file, không phải ngày). Dùng `Classification Firm Status` (`cl_firm_status`, entity Reference Data Set thật từ `SCMS.CAT_SC_FIRM_STATUS`, business_meaning liệt kê 4 trạng thái: Đang hoạt động/Tạm ngừng/Đình chỉ/Đóng cửa) — JOIN theo `cl_firm_status_id`, filter theo `cl_firm_status_nm`. **LƯU Ý KỸ THUẬT CHƯA CHỐT (không chặn thiết kế):** giá trị chuỗi chính xác `'Đang hoạt động'` lấy từ business_meaning mô tả trong YAML Atomic, chưa xác nhận với dữ liệu thật của `cl_firm_status` (đây là reference data set nạp từ nguồn, không phải enum cố định trong schema) — cần Data Modeler/DBA đối chiếu giá trị thật trước khi viết ETL, tương tự lưu ý đã áp dụng ở O_GSTT_8. | READY |
...
**Atomic:**
- `Market Index Snapshot` (nguồn `MDDS.JAD_MARKETINFOR`) — READY. [SỬA 2026-09-23] Lấy `market_index_val` bản ghi cuối ngày cho 6 `market_code` → cột `market_index_val` + lookup FK `index_constituent_dim_id` trên `Fact Market Trading Snapshot` (xem Cụm 18b).
...
**Source:** `Fact Market Trading Snapshot` (sở hữu TKNB, xem Cụm 18b) → `Calendar Date Dimension`, `Index Constituent Dimension` (reuse — sở hữu GSTT)
...
| K_TKNB_1013 | Loại chỉ số | - | Chiều | `fct_market_trading_snpst.index_constituent_dim_id` → `index_constituent_dim.index_code` WHERE `index_code IN ('HOSE','HNX','UPCOM','30','HNX30','100')` (hiển thị `index_nm`) | [STT=2, item_code=`dim_index_type_mss030a`] [SỬA 2026-09-23] BA dòng 1131 — 6 MARKETCODE: VNIndex `'HOSE'`, HNX Index `'HNX'`, UPCOM Index `'UPCOM'`, VN30 `'30'`, HNX30 `'HNX30'`, VN100 `'100'`. FK `index_constituent_dim_id` trên `Fact Market Trading Snapshot` (sở hữu TKNB), lookup `index_code = market_index_snapshot.market_code` | READY |
| K_TKNB_1014 | Giá trị chỉ số CP toàn thị trường | Điểm | Cơ sở | `fct_market_trading_snpst.market_index_val` — `market_index_snapshot.market_index_val` bản ghi cuối ngày (ROW_NUMBER theo `index_time` DESC) WHERE `index_code IN ('HOSE','HNX','UPCOM')` | [STT=3, item_code=`market_index_value`] [SỬA 2026-09-23] BA dòng 1132 — cột mới `market_index_val` trên `Fact Market Trading Snapshot`; bỏ reuse `Fact Market Index Snapshot`/`Market Index Dimension` (QLKD) | READY |
| K_TKNB_1015 | Tổng giá trị giao dịch CP toàn thị trường | Tỷ đồng | Cơ sở | `SUM(fct_market_trading_snpst.total_trading_val)` WHERE `securities_trade.market_id_code IN ('STO','STX','UPX')` GROUP BY `trade_dt` | [SỬA 2026-09-23] Grain Fact = 1 Trade Date × 1 Index Code: giá trị toàn thị trường lặp lại trên mỗi Index Code — BI lấy 1 giá trị/ngày, KHÔNG SUM qua chỉ số. [STT=4, item_code=`total_trading_value_mss030a`] Không phụ thuộc Loại chỉ số — giá trị lặp lại trên 4 dòng cùng Ngày GD ở tầng trình diễn. Atomic READY. **[SỬA 2026-09-22]** Lưu vật lý trên `Fact Market Trading Snapshot` (mới) — xem Cụm 18b. | READY |
| K_TKNB_1016 | Tổng khối lượng giao dịch CP toàn thị trường | CP | Cơ sở | `SUM(fct_market_trading_snpst.total_trading_vol)` WHERE `securities_trade.market_id_code IN ('STO','STX','UPX')` GROUP BY `trade_dt` | [SỬA 2026-09-23] Grain Fact = 1 Trade Date × 1 Index Code: giá trị toàn thị trường lặp lại trên mỗi Index Code — BI lấy 1 giá trị/ngày, KHÔNG SUM qua chỉ số. [STT=5, item_code=`total_trading_volume_mss030a`] Không phụ thuộc Loại chỉ số. Atomic READY. | READY |
| K_TKNB_1017 | KLGD khớp lệnh CP toàn thị trường | CP | Cơ sở | `SUM(fct_market_trading_snpst.matched_trading_vol)` WHERE `securities_trade.market_id_code IN ('STO','STX','UPX')` AND `securities_trade.board_tp_code IN ('G1','G2','G3','G4','G7','G8')` GROUP BY `trade_dt` | [SỬA 2026-09-23] Grain Fact = 1 Trade Date × 1 Index Code: giá trị toàn thị trường lặp lại trên mỗi Index Code — BI lấy 1 giá trị/ngày, KHÔNG SUM qua chỉ số. [STT=6, item_code=`matched_trading_volume_mss030a`] Không phụ thuộc Loại chỉ số. Atomic READY. | READY |
| K_TKNB_1018 | GTGD khớp lệnh CP toàn thị trường | Tỷ đồng | Cơ sở | `SUM(fct_market_trading_snpst.matched_trading_val)` WHERE `securities_trade.market_id_code IN ('STO','STX','UPX')` AND `securities_trade.board_tp_code IN ('G1','G2','G3','G4','G7','G8')` GROUP BY `trade_dt` | [SỬA 2026-09-23] Grain Fact = 1 Trade Date × 1 Index Code: giá trị toàn thị trường lặp lại trên mỗi Index Code — BI lấy 1 giá trị/ngày, KHÔNG SUM qua chỉ số. [STT=7, item_code=`matched_trading_value_mss030a`] Không phụ thuộc Loại chỉ số. Atomic READY. | READY |
| K_TKNB_1019 | KLGD thỏa thuận CP toàn thị trường | CP | Cơ sở | `SUM(fct_market_trading_snpst.negotiated_trading_vol)` WHERE `securities_trade.market_id_code IN ('STO','STX','UPX')` AND `securities_trade.board_tp_code IN ('T1','T2','T3','T4','T6','R1')` GROUP BY `trade_dt` | [SỬA 2026-09-23] Grain Fact = 1 Trade Date × 1 Index Code: giá trị toàn thị trường lặp lại trên mỗi Index Code — BI lấy 1 giá trị/ngày, KHÔNG SUM qua chỉ số. [STT=8, item_code=`negotiated_trading_volume_mss030a`] Không phụ thuộc Loại chỉ số. Atomic READY. | READY |
| K_TKNB_1020 | GTGD thỏa thuận CP toàn thị trường | Tỷ đồng | Cơ sở | `SUM(fct_market_trading_snpst.negotiated_trading_val)` WHERE `securities_trade.market_id_code IN ('STO','STX','UPX')` AND `securities_trade.board_tp_code IN ('T1','T2','T3','T4','T6','R1')` GROUP BY `trade_dt` | [SỬA 2026-09-23] Grain Fact = 1 Trade Date × 1 Index Code: giá trị toàn thị trường lặp lại trên mỗi Index Code — BI lấy 1 giá trị/ngày, KHÔNG SUM qua chỉ số. [STT=9, item_code=`negotiated_trading_value_mss030a`] Không phụ thuộc Loại chỉ số. Atomic READY. | READY |
| K_TKNB_1021 | KLGD lô lẻ CP toàn thị trường | CP | Cơ sở | `SUM(fct_market_trading_snpst.odd_lot_trading_vol)` WHERE `securities_trade.market_id_code IN ('STO','STX','UPX')` AND `securities_trade.board_tp_code IN ('G4','T4','T6')` GROUP BY `trade_dt` | [SỬA 2026-09-23] Grain Fact = 1 Trade Date × 1 Index Code: giá trị toàn thị trường lặp lại trên mỗi Index Code — BI lấy 1 giá trị/ngày, KHÔNG SUM qua chỉ số. [STT=10, item_code=`odd_lot_trading_volume_mss030a`] Không phụ thuộc Loại chỉ số. Atomic READY. | READY |
| K_TKNB_1022 | GTGD lô lẻ CP toàn thị trường | Tỷ đồng | Cơ sở | `SUM(fct_market_trading_snpst.odd_lot_trading_val)` WHERE `securities_trade.market_id_code IN ('STO','STX','UPX')` AND `securities_trade.board_tp_code IN ('G4','T4','T6')` GROUP BY `trade_dt` | [SỬA 2026-09-23] Grain Fact = 1 Trade Date × 1 Index Code: giá trị toàn thị trường lặp lại trên mỗi Index Code — BI lấy 1 giá trị/ngày, KHÔNG SUM qua chỉ số. [STT=11, item_code=`odd_lot_trading_value_mss030a`] Không phụ thuộc Loại chỉ số. Atomic READY. | READY |
...
erDiagram
    ...
    Index_Constituent_Dimension ||--o{ Fact_Market_Trading_Snapshot : "Index_Constituent_Dimension_Id"
    Calendar_Date_Dimension ||--o{ Fact_Market_Trading_Snapshot : "Snapshot_Date_Dimension_Id"
```

> **Ghi chú:** [SỬA 2026-09-23] `Fact Market Trading Snapshot` (`fct_market_trading_snpst`, sở hữu TKNB) mang cả Chiều Loại chỉ số (FK `index_constituent_dim_id`) và `market_index_val` — grain 1 Trade Date × 1 Index Code, cùng pattern `Fact Foreign Proprietary Trading Index Snapshot` (Nhóm 23). Không còn reuse `Fact Market Index Snapshot`/`Market Index Dimension` của QLKD. 8 đo lường GTGD/KLGD toàn thị trường lặp lại theo Index Code — không SUM qua chỉ số. Flat: `tknb_fct_market_trading_snpst_flat` (bảng #13).
...
| Tên bảng | Grain |
|---|---|
| Fact Market Trading Snapshot (`fct_market_trading_snpst`, mới — sở hữu TKNB) | 1 row = 1 Trade Date × 1 Index Code (6 mã) — 8 đo lường toàn thị trường (cộng gộp HOSE/HNX/UPCoM) lặp lại theo Index Code |
...
|---|---|---|---|
| `hnx01_stock_trading_rpt` | 1 dòng / 1 chỉ tiêu (`item_code`) / 1 kỳ báo cáo (`report_period_dt`) — cấu trúc EAV theo đúng grain 1 dòng của mẫu biểu (STT \| Chỉ tiêu \| Đơn vị tính \| Giá trị), không phải wide-table 1 dòng/1 ngày với nhiều cột measure | K_TKNB_1–108 (Nhóm 1) | READY (98 KPI, +3 — K_TKNB_20/21/22 sửa 2026-09-16) / PENDING (13 KPI label-only + 2 KPI chờ Atomic VSDC.BM41/MB1) |
| `hnx02_gov_bond_otc_trading_rpt` | 1 dòng / 1 chỉ tiêu (`item_code`) / 1 kỳ báo cáo (`report_period_dt`) — cùng cấu trúc EAV với `hnx01_stock_trading_rpt` | K_TKNB_109–273 (Nhóm 2) | READY (7/165 KPI — K_TKNB_113,117,121,136,159,163,167, nguồn `Bond Order Book`) / PENDING (158/165 KPI — biểu mẫu HNX.BM24 chưa CSDL cho 4/7 loại hình GD + dimension NĐTNN chưa có + dòng tổng toàn thị trường) |
...
| `hnx04_market_scale_rpt` | 1 dòng / 1 chỉ tiêu (`item_code`) / 1 kỳ báo cáo (`report_period_dt`) / 1 loại kỳ (`period_type` ∈ {trong_ky, cong_don}) — mở rộng thêm chiều `period_type` so với 2 bảng trên vì báo cáo HNX04 có cả số liệu "Trong kỳ - Phát sinh" và "Cộng dồn đến cuối kỳ - Phát sinh" cho cùng 1 chỉ tiêu (riêng mục 6-11 chỉ có `period_type=trong_ky`, không có `cong_don`) | K_TKNB_290–471 (Nhóm 4) | READY (80 KPI, +3 — K_TKNB_313/314/315 sửa 2026-09-16 — 23 KPI gốc + 54 KPI mục 6-11 theo mapping BA 2026-09-10, nguồn `Listing Status Change`/`Additional Gov Bond Listing`/`Bond Trading Registration Status` VSDC) / PENDING (102 KPI — biểu mẫu BM24/BM32/BM23/BM34/BM25, mục 1/4-5) |
...
| `hsx01_stock_trading_rpt` | 1 dòng / 1 chỉ tiêu (`item_code`) / 1 kỳ báo cáo (`report_period_dt`) — cùng cấu trúc EAV, cho thị trường HOSE (khác `hnx01_stock_trading_rpt` là thị trường HNX) | K_TKNB_571–695 (Nhóm 10) | READY (111 KPI, +2 — K_TKNB_588/589 sửa 2026-09-16) / PENDING (14 KPI — phân ngành GICS còn PENDING (K_TKNB_590-600), VSDC.MB1 cổ phiếu quỹ) |
...
| Báo cáo về giao dịch trên thị trường Trái phiếu Chính phủ (HNX02) | hnx02_gov_bond_otc_trading_rpt | new | Cùng quyết định thiết kế TKNB — bảng phẳng riêng. Nguồn Atomic mới `Bond Order Book` (VSDC) — không reuse `hnx01_stock_trading_rpt`/`hnx03_derivative_trading_rpt` dù cùng cấu trúc EAV, vì nguồn Atomic khác hẳn (VSDC OTC bond order book, không phải ORDERTRADE/MDDS equity). |
...
1. **Nguồn VSDC biểu mẫu (BM1/BM41/MB1) chưa có CSDL** — **[SỬA 2026-09-16, commit b8868c9e]** K_TKNB_20/21/22 đã chuyển READY — nguồn "Khối lượng chứng khoán đang lưu hành" hóa ra đã có sẵn ở Atomic dưới tên `listed_share_info` (VSDC `outstanding_shares`, `src_stm_code='VSDC_OUTSTANDING_SHARES'`), chỉ là thiết kế trước đây chưa biết/chưa dùng — cùng gốc rễ với O_GSTT_22 (module GSTT). Còn lại K_TKNB_47/48 vẫn PENDING: "Vốn hóa thị trường HNX" (BM41) và "Khối lượng CP quỹ mua lại" (VSDC.MB1) — 2 báo cáo khác hẳn, không liên quan `outstanding_shares`. **Lưu ý quy trình:** `listed_share_info` chưa có entry chính thức trong `dm_manifest.yaml`/`working manifest` tại thời điểm dùng — chỉ có tài liệu mapping Bronze→Atomic (`DataModel/working/Atomic/lld/VSDC/mapping_vsdc_ods_atm.md`) và đã verify hoạt động trên UAT. Cần Atomic team đăng ký bổ sung entity này vào manifest sớm.
...
23. **[Resolved 2026-09-16] Nhóm 17 — nghi vấn lệch nhãn HCM/HN và điều kiện `group by` ở mục vốn hóa (K_TKNB_924, K_TKNB_925, idx 7/8)**: `thong_tin`="SGDCK HCM" nhưng `dieu_kien`="group by HNX"; `thong_tin`="SGDCK HN" nhưng `dieu_kien`="Group by UpCom" — nghi ngờ ban đầu là BA nhập lệch dòng/copy-paste nhầm điều kiện. **Đã xác nhận khi build công thức thật (commit b8868c9e):** không phải lỗi — "HCM" trong ngữ cảnh này = SGDCK TP.HCM = HOSE (`floor_code='10'`), còn `dieu_kien` ghi "group by HNX" là copy-paste sai CHỈ ở cột đó (dòng mô tả điều kiện), không ảnh hưởng công thức thực tế đã build đúng theo tên sàn. Tương tự K_TKNB_925 (HN=HNX=`floor_code='02'`). Cả 3 dòng (924/925/926) nay READY.
