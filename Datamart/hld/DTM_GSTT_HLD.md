...
| K_GSTT_144 | KLNN ròng thỏa thuận | Cổ phiếu | Phái sinh | `SUM(Buy Foreign Investor Type Code IN ('10','20') → Execution Volume WHERE Board Type Code IN ('T1','T2','T3','T4','T6','R1')) − SUM(Sell Foreign Investor Type Code IN ('10','20') → Execution Volume WHERE Board Type Code IN ('T1','T2','T3','T4','T6','R1')) GROUP BY Symbol, Trade Date` | **Mới (2026-09-09) — BA STT 1 dòng con 21/21, trước đây thiếu KPI, phát hiện qua đối chiếu số dòng con BA↔KPI (mục #10 self-check).** Khác K_GSTT_19 "KLNN ròng" (khớp lệnh, filter `Market Id Code`) — đây là biến thể thỏa thuận, filter theo `Board Type Code`/`Board ID`. BA xác nhận `IN ('T1','T2','T3','T4','T6','R1')` — bao gồm `R1` (Negotiation Repo, scheme `ORDERTRADE_BOARD_TYPE`). **Lưu ý (O_GSTT_20, Open):** K_GSTT_17/18 "Tổng KL/GT thỏa thuận" (cùng Nhóm) hiện filter thiếu `R1` dù cùng đặc tả BA gốc — chưa sửa, chờ xác nhận riêng. Pre-aggregate — cùng cảnh báo double-count như K_GSTT_13 | READY |
...
| O_GSTT_20 | Nhóm 1 | Phát hiện 2026-09-09 khi thiết kế K_GSTT_144 ("KLNN ròng thỏa thuận", BA STT 1 dòng con 21): BA chỉ định filter Board Type/Board ID `IN ('T1','T2','T3','T4','T6','R1')` cho CẢ 3 chỉ tiêu "thỏa thuận" trong cùng Nhóm — K_GSTT_17 (Tổng KL thỏa thuận), K_GSTT_18 (Tổng GT thỏa thuận), và K_GSTT_144 (KLNN ròng thỏa thuận, mới). Tuy nhiên thiết kế hiện tại của K_GSTT_17/18 (`total_negotiated_vol`/`total_negotiated_val`, có từ trước) chỉ filter `IN ('T1','T2','T3','T4','T6')` — **thiếu `R1`** (Negotiation Repo, scheme `ORDERTRADE_BOARD_TYPE`) so với đặc tả BA gốc. K_GSTT_144 được thiết kế mới lần này đã bao gồm đúng `R1` theo BA. Cần xác nhận với Data Modeler/nghiệp vụ: có nên bổ sung `R1` vào filter của K_GSTT_17/18 (và flat table `total_negotiated_vol`/`total_negotiated_val` đã tồn tại, ảnh hưởng dữ liệu lịch sử) để nhất quán với K_GSTT_144 và đúng đặc tả BA hay không, trước khi sửa | Open |
