...
> **[SỬA 2026-09-25] Hiển thị dòng 0:** Fact giữ cố định 4 dòng/phân loại/(mã CK hoặc chỉ số)/ngày — không đổi grain (đổi grain sẽ làm mất GT thỏa thuận ở chế độ Tổng GTGD). Detail Mapping Nhóm 28–31 thêm dòng FILTER `HAVING SUM(ABS(buy)) + SUM(ABS(sell)) <> 0` theo đúng cặp cột của chế độ đang xem (tổng / `matched_*` / `negotiated_*`) — ngày chỉ có giao dịch thỏa thuận không hiện ở chế độ Khớp lệnh (VD ACL 27/08/2026, board T4).
