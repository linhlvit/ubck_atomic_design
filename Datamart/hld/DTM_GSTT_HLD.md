...
| K_GSTT_35 | Giá trị chỉ số | Điểm | Cơ sở | `Fact Market Index Snapshot.Market Index Value WHERE Market Code = mapping(Index Code) AND cùng ngày, JOIN qua Bridge theo mã CK` | Đổi JOIN sang qua Bridge theo mã CK (cùng cơ chế K_GSTT_75, Nhóm 24) — giá trị lặp lại trên mọi mã cùng rổ | READY |
| K_GSTT_39 | % thay đổi | % | Phái sinh | `Fact Market Index Snapshot.Index Percent Change WHERE Market Code = mapping(Index Code) AND cùng ngày, JOIN qua Bridge theo mã CK` | Đổi từ `K_GSTT_38` (thay đổi tuyệt đối) sang `K_GSTT_39` (% thay đổi) — theo sheet Tổng hợp công thức, dòng 3 của Nhóm 35 là `(Giá đóng cửa/Giá tham chiếu − 1)×100` | READY |
...
**Star Schema:** Không có bảng mới — 100% reuse `Fact Market Index Snapshot`, `Index Constituent Dimension`, `Fact Index Constituent Snapshot` (Bridge), `Security Trading Snapshot Dimension` đã vẽ ở Nhóm 1/5.
...
**Bảng grain:** Không có bảng mới — cùng grain `Fact Index Constituent Snapshot` (Bridge, Symbol × Index × Date) đã có ở Nhóm 1. Grain hiển thị Nhóm 35 = 1 dòng/mã CK (khác Nhóm 5 = 1 dòng/chỉ số) dù cùng nguồn dữ liệu.
