...
> **[MỚI 2026-09-21, theo yêu cầu Data Modeler]** Nhóm 28/29 ("Xu hướng dòng tiền — Giao dịch theo phân loại Nhà đầu tư") trước đây lưu phân loại NĐT (Cá nhân/Tổ chức trong nước) thành 8 cột giá trị cố định (`Individual_Buy/Sell_Value/Volume`, `Domestic_Institution_Buy/Sell_Value/Volume`) trên `Fact Stock Portfolio Snapshot`, buộc tầng BI phải `CASE {nhánh} WHEN ... THEN individual_buy_val WHEN ... THEN domestic_institution_buy_val END` để switch cột theo lựa chọn Chiều "Phân loại nhà đầu tư" (K_GSTT_85) — không có cột vật lý nào đại diện cho chính "Phân loại nhà đầu tư" như 1 giá trị dữ liệu thật. Fact mới tách riêng, grain hẹp hơn (`mã CK × ngày × Phân loại NĐT`), có cột vật lý `Investor Category Code` — K_GSTT_85 trở thành FK/attribute thật, K_GSTT_86-94 (Nhóm 28/29) chỉ còn `SUM(...) WHERE Investor Category Code = :x`, không còn CASE WHEN. **Không đổi grain/cột của `Fact Stock Portfolio Snapshot`** — Nhóm 21 (Foreign)/Nhóm 27 (Proprietary)/Nhóm 33 (Proprietary Volume) tiếp tục dùng nguyên các cột đã có trên Fact đó, không bị ảnh hưởng.
> **Nguồn phân loại (đã xác nhận với Data Modeler, tham khảo trực tiếp BA_analyst_GSTT.csv dòng "Phân loại nhà đầu tư", STT 28/29):** phân nhánh theo thứ tự ưu tiên Tự doanh → Nước ngoài → Cá nhân/Tổ chức trong nước (giữ nguyên logic đã duyệt ở Nhóm 27/28 cũ):
> 1. **Tự doanh:** `Buy/Sell Client House Classification Code = '30'` (cả HOSE/HNX).
> 2. **Nước ngoài:** `Buy/Sell Foreign Investor Type Code IN ('10','20')` (cả HOSE/HNX).
> 3. **Cá nhân** (loại trừ 1, 2): HOSE `Buy/Sell Investor Type Code IN ('1000','8000','1100','1200')` HOẶC HNX `Buy/Sell Investor Type Code = '8000'`.
> 4. **Tổ chức trong nước** (loại trừ 1, 2): HOSE `Buy/Sell Investor Type Code IN ('2000','2100','2200','3000','3100','3200','3300','4000','5000','5200','6000','9000')` HOẶC HNX `Buy/Sell Investor Type Code IN ('1000','2000','3000','3100','4000','5000','6000','7000','7100')`.
...
| K_GSTT_35 | Giá trị chỉ số | Điểm | Cơ sở | `LOOKUP Fact Market Index Snapshot ON Fact Market Index Snapshot.Market Index Dimension Id = Market Index Dimension.Market Index Dimension Id AND Market Index Dimension.Index Name = Index Constituent Dimension.Index Code AND Fact Market Index Snapshot.Snapshot Date Dimension Id = Calendar Date Dimension.Calendar Date Dimension Id → Fact Market Index Snapshot.Market Index Value` | **[SỬA 2026-09-22, datamart-review — hoàn thiện thiết kế]** Trước đây chỉ ghi `Fact Market Index Snapshot.Market Index Value` không kèm điều kiện JOIN — copy nhầm nguyên công thức của K_GSTT_35 ở Nhóm 5 (nơi grain trùng khớp tự nhiên với Fact Market Index Snapshot nên không cần JOIN), trong khi Nhóm 6 có grain khác (theo Index Code qua `Fact Stock Portfolio Snapshot`, không phải Market Code) — cùng loại lỗi Grain Mismatch như case K_GSTT_61. Đã bổ sung đúng khóa bridge `Market Index Dimension.Index Name = Index Constituent Dimension.Index Code` (nguồn `MDDS.JAD_MARKETINFOR.INDEXNAME`, xác nhận 2026-09-22) — giải quyết một phần Open Issue O_GSTT_3 (xem Section 5). Giá trị LẶP LẠI trên mọi dòng Mã CK cùng Chỉ số + Ngày (broadcast, giống K_GSTT_149-151/61). | READY |
...
> **Ghi chú "Đỉnh cũ" — **[SỬA 2026-09-19, lần 4]** đảo ngược O_GSTT_6:** reuse K_GSTT_106 (Giá cao nhất 52 tuần, nay Max High Price — xem chi tiết đảo ngược ở Nhóm 32), không còn K_GSTT_28.
...
| K_GSTT_140 | Đỉnh cũ (3 tháng) | VNĐ | Phái sinh | `MAX(Fact Stock Portfolio Snapshot.High Price) OVER (PARTITION BY Symbol ORDER BY Trading Date ROWS BETWEEN 64 PRECEDING AND CURRENT ROW)` | **[SỬA 2026-09-19, lần 4]** Reuse từ K_GSTT_140 (Nhóm 32) — xem ghi chú "Đỉnh cũ" ở trên | READY |
| K_GSTT_141 | Đỉnh cũ (6 tháng) | VNĐ | Phái sinh | `MAX(Fact Stock Portfolio Snapshot.High Price) OVER (PARTITION BY Symbol ORDER BY Trading Date ROWS BETWEEN 129 PRECEDING AND CURRENT ROW)` | **[SỬA 2026-09-19, lần 4]** Reuse từ K_GSTT_141 (Nhóm 32) — xem ghi chú "Đỉnh cũ" ở trên | READY |
| K_GSTT_106 | Đỉnh cũ (1 năm) | VNĐ | Phái sinh | `MAX(Fact Stock Portfolio Snapshot.High Price) OVER (PARTITION BY Symbol ORDER BY Trading Date ROWS BETWEEN 259 PRECEDING AND CURRENT ROW)` | **[SỬA 2026-09-19, lần 4]** Reuse từ K_GSTT_106 (Nhóm 32) — xem ghi chú "Đỉnh cũ" ở trên | READY |
| K_GSTT_56 | LNST | VNĐ | Cơ sở | `Fact Stock Portfolio Snapshot.Net Profit After Tax` | Reuse từ Nhóm 6 — Resolved 2026-08-26 (rule GSĐC, xem O_GSTT_1) | READY |
| K_GSTT_57 | VCSH | VNĐ | Cơ sở | `Fact Stock Portfolio Snapshot.Owner Equity` | Reuse từ Nhóm 6 — Resolved 2026-08-26 (rule GSĐC, xem O_GSTT_1) | READY |
| K_GSTT_55 | Số cổ phiếu đang lưu hành | Cổ phiếu | Cơ sở | `Fact Stock Portfolio Snapshot.Outstanding Share Quantity` | Reuse từ Nhóm 6 — Resolved 2026-08-26, sửa nguồn 2026-09-16 (`listed_share_info`, xem O_GSTT_2 + O_GSTT_22) | READY |
| K_GSTT_58 | P/E thị trường | Lần | Chỉ tiêu phái sinh | `K_GSTT_10 / (Fact Stock Portfolio Snapshot.Net Profit After Tax TTM / K_GSTT_55)` | Reuse từ Nhóm 6 — Resolved 2026-08-26. LNST dùng TTM 4 quý (rule GSĐC); K_GSTT_55 đã có nguồn (`listed_share_info`, sửa nguồn 2026-09-16 — xem O_GSTT_22) | READY |
...
> **Ghi chú "Đáy cũ" — **[SỬA 2026-09-19, lần 4]** đảo ngược O_GSTT_6:** reuse K_GSTT_107 (Giá thấp nhất 52 tuần, nay Min Low Price — xem chi tiết đảo ngược ở Nhóm 32), không còn K_GSTT_29.
...
| K_GSTT_142 | Đáy cũ (3 tháng) | VNĐ | Phái sinh | `MIN(Fact Stock Portfolio Snapshot.Low Price) OVER (PARTITION BY Symbol ORDER BY Trading Date ROWS BETWEEN 64 PRECEDING AND CURRENT ROW)` | **[SỬA 2026-09-19, lần 4]** Reuse từ K_GSTT_142 (Nhóm 32) — xem ghi chú "Đáy cũ" ở trên | READY |
| K_GSTT_143 | Đáy cũ (6 tháng) | VNĐ | Phái sinh | `MIN(Fact Stock Portfolio Snapshot.Low Price) OVER (PARTITION BY Symbol ORDER BY Trading Date ROWS BETWEEN 129 PRECEDING AND CURRENT ROW)` | **[SỬA 2026-09-19, lần 4]** Reuse từ K_GSTT_143 (Nhóm 32) — xem ghi chú "Đáy cũ" ở trên | READY |
| K_GSTT_107 | Đáy cũ (1 năm) | VNĐ | Phái sinh | `MIN(Fact Stock Portfolio Snapshot.Low Price) OVER (PARTITION BY Symbol ORDER BY Trading Date ROWS BETWEEN 259 PRECEDING AND CURRENT ROW)` | **[SỬA 2026-09-19, lần 4]** Reuse từ K_GSTT_107 (Nhóm 32) — xem ghi chú "Đáy cũ" ở trên | READY |
| K_GSTT_56 | LNST | VNĐ | Cơ sở | `Fact Stock Portfolio Snapshot.Net Profit After Tax` | Reuse từ Nhóm 6 — Resolved 2026-08-26 (rule GSĐC, xem O_GSTT_1) | READY |
| K_GSTT_57 | VCSH | VNĐ | Cơ sở | `Fact Stock Portfolio Snapshot.Owner Equity` | Reuse từ Nhóm 6 — Resolved 2026-08-26 (rule GSĐC, xem O_GSTT_1) | READY |
| K_GSTT_55 | Số cổ phiếu đang lưu hành | Cổ phiếu | Cơ sở | `Fact Stock Portfolio Snapshot.Outstanding Share Quantity` | Reuse từ Nhóm 6 — Resolved 2026-08-26, sửa nguồn 2026-09-16 (`listed_share_info`, xem O_GSTT_2 + O_GSTT_22) | READY |
| K_GSTT_58 | P/E thị trường | Lần | Chỉ tiêu phái sinh | `K_GSTT_10 / (Fact Stock Portfolio Snapshot.Net Profit After Tax TTM / K_GSTT_55)` | Reuse từ Nhóm 6 — Resolved 2026-08-26. LNST dùng TTM 4 quý (rule GSĐC); K_GSTT_55 đã có nguồn (`listed_share_info`, sửa nguồn 2026-09-16 — xem O_GSTT_22) | READY |
...
> **Phân loại:** Dashboard
> **Atomic:** 100% reuse Nhóm 1 (`Security Trading Snapshot`, `Securities Trade`, `Public Company`, `Classification Business Line`) + Nhóm 6 (`Public Company Share Statistics` Resolved 2026-08-26, sửa nguồn 2026-09-16 (`listed_share_info`, xem O_GSTT_2 + O_GSTT_22), EAV IDS Resolved 2026-08-26 (rule GSĐC, xem O_GSTT_1)) + Nhóm 7 (Bộ chỉ số thị trường/ngành) — 4 measure NĐTNN **ròng** (K_GSTT_135–138) khai sinh trực tiếp tại Nhóm này, không có nguồn mới khác. **[SỬA 2026-09-18]** Trước ghi nhầm là K_GSTT_70–73 — bộ đó là KLNN/GTNN mua-bán **gộp**, khai sinh tại Nhóm 23 (reuse ở Nhóm 26/33), không dùng ở Nhóm này.
>
...
>
> **Ghi chú tái sử dụng:** BA liệt kê 6 dòng con — 4 dòng đã có KPI ID sẵn từ Nhóm 1 (Mã CK, % thay đổi, Giá) và Nhóm 23 (KLNN mua = K_GSTT_70, KLNN bán = K_GSTT_71 — bộ GỘP; **[SỬA 2026-09-18, review Nhóm 21]** trước ghi nhầm Nhóm 21) — reuse thẳng. "KLNN ròng" trùng hoàn toàn K_GSTT_19 (Nhóm 1, đã là hiệu số mua-bán KL theo Foreign Investor Type Code) — không khai KPI mới. Biến thể bản đồ nhiệt (treemap, màu theo % thay đổi hoặc KLNN ròng) của cùng bộ chỉ tiêu Nhóm 25, không có cấu trúc Datamart riêng.
...
> **Phân loại:** Dashboard
> **Atomic:** 100% reuse Nhóm 1 (`Security Trading Snapshot`, `Securities Trade`) + Nhóm 3 (`Open/High/Low Price`) + Nhóm 21 (Foreign Investor) + Nhóm 27 (Client House/Tự doanh) — **[SỬA 2026-09-21]** Phân loại NĐT (Cá nhân/Tổ chức trong nước/Tự doanh/Nước ngoài) nay lưu trên `Fact Investor Category Trading Snapshot` (mới, xem Section 1 Cụm 1c), KHÔNG còn mở rộng cột trên `Fact Stock Portfolio Snapshot`.
>
> **Ghi chú tái sử dụng:** BA liệt kê 14 dòng con (dòng 401–414) — 8 dòng đã có KPI ID sẵn từ Nhóm 1 (Mã CK, Chỉ số, Thay đổi, % thay đổi) và Nhóm 3 (Giá mở cửa, Giá cao/thấp/đóng cửa) — reuse thẳng, **[SỬA 2026-09-22 lần 3]** trước đây bỏ sót "Giá mở cửa" (dòng 403 → K_GSTT_27) và "thay đổi" (dòng 407 → K_GSTT_11), xem sửa lỗi bên dưới. "GT tự doanh ròng" trùng hoàn toàn K_GSTT_83 (Nhóm 27), "GT NN ròng" trùng hoàn toàn K_GSTT_77 (Nhóm 25, = K_GSTT_72 − K_GSTT_73) — không khai KPI mới cho 2 dòng này, **giữ nguyên nguồn cũ trên `Fact Stock Portfolio Snapshot`** (không di chuyển sang Fact mới — 2 KPI này không dùng chung slicer K_GSTT_85, là measure đơn lẻ cố định theo đúng định nghĩa gốc tại Nhóm 27/25). 2 dòng còn lại ("GT cá nhân ròng", "GT tổ chức trong nước ròng") là chỉ tiêu mới — xem ghi chú dưới đây.
> **Lịch sử nguồn phân loại NĐT (giữ nguyên, không xóa — precedent O_GSTT_1/6/10/22):**
> - **2026-07-29:** Bản thiết kế trước dùng `Buy/Sell Investor Type Code` (scheme `ORDERTRADE_INVESTOR_TYPE`) để phân biệt Cá nhân/Tổ chức — bị đánh giá "sai nguồn" theo Note BA khi đó ("Hiện tại chưa có cơ sở để phân biệt chính xác... sẽ phân biệt qua **số tài khoản**") — đổi sang dùng `Buy/Sell Account Number` (`buy_account_nbr`/`sell_account_nbr` trên `Securities Trade`).
> - **[SỬA 2026-09-19, BA cập nhật STT 28]** BA cập nhật lại STT 28: điều kiện lọc quay về dùng `Buy/Sell Invest Type` (HOSE) / `investor classification code` (HNX) — tức field `Buy/Sell Investor Type Code` (scheme `ORDERTRADE_INVESTOR_TYPE`) đã bị bác ở lần sửa 07/2026, nay dùng lại. Lưu vật lý dưới dạng 10 cột cố định (`Individual_*`/`Domestic_Institution_*`) trên `Fact Stock Portfolio Snapshot`, tầng BI dùng `CASE {nhánh} WHEN...` để switch cột theo K_GSTT_85 — không có cột vật lý riêng cho "Phân loại nhà đầu tư" như 1 giá trị dữ liệu.
> - **[SỬA 2026-09-21, theo yêu cầu Data Modeler — xác nhận trực tiếp bảng mã Investor Type Code từ BA_analyst_GSTT.csv]** Giữ nguyên nguồn `Investor Type Code` (đã đúng, xác nhận lại 2026-09-19) nhưng đổi cách **lưu trữ vật lý**: tách sang Fact mới grain hẹp `mã CK/ngày/Phân loại NĐT` (`Fact Investor Category Trading Snapshot`, Cụm 1c) với cột `Investor Category Code` vật lý — loại bỏ hoàn toàn CASE WHEN switch cột ở tầng BI. **Đã cân nhắc và loại bỏ phương án** dùng bảng `BM_DANH_MUC_NDT(TAI_KHOAN, LOAI_NDT)` xuất hiện tại 1 vị trí lẻ trong BA (Nhóm 29, dòng "Chỉ số", cột Câu lệnh màn) — Data Modeler xác nhận đây không phải nguồn đúng, giữ nguyên `Investor Type Code` trên `Securities Trade` đã có sẵn Atomic.
> - **[SỬA 2026-09-22, BA cập nhật lại STT 28 — đơn giản hoá điều kiện phân loại, thống nhất HOSE/HNX]** BA thay ô Mô tả/Công thức của dòng "Phân loại nhà đầu tư" (STT 28, `BA_analyst_GSTT.csv` dòng 400) từ bảng liệt kê mã dài riêng theo từng sàn sang **đúng 1 điều kiện nhị phân, áp dụng đồng nhất cả HOSE và HNX**: `Investor Type Code = '8000'` → Cá nhân trong nước, `Investor Type Code <> '8000'` → Tổ chức trong nước (cả 2 đều xét sau khi đã loại trừ Tự doanh/NĐTNN). Đồng thời đơn giản hoá điều kiện Nước ngoài từ `IN ('10','20')` sang `<> '00'` — khớp đúng nguyên văn công thức BA hiện tại (`buy/sell_foreigner_investor_type_(code) <> '00'`). Không đổi nguồn Atomic (vẫn `Securities Trade.Buy/Sell Investor Type Code` / `Buy/Sell Foreign Investor Type Code` / `Buy/Sell Client House Classification Code` — cùng field đã xác nhận từ 2026-09-19/21), không đổi Fact/grain — chỉ đổi GIÁ TRỊ so sánh khi ETL populate `Investor_Category_Code`. Cùng đợt, BA hoàn tất 5 dòng trước đây còn thiếu Bảng nguồn/Trường nguồn ("Giá trị mua/bán/ròng/mua ròng/bán ròng theo phân loại nhà đầu tư") — khớp đúng công thức K_GSTT_90-94 đã thiết kế sẵn (xem SỬA 2026-09-21 lần 2 dưới), không phát sinh thay đổi thiết kế cho 5 dòng này.
> Quy tắc phân loại (**[SỬA 2026-09-22]** — thay thế bảng mã dài 2026-09-19/21, xem lịch sử trên):
> - **Tự doanh:** `Buy/Sell Client House Classification Code = '30'` (cả HOSE/HNX) — xét trước tiên.
> - **Nước ngoài:** `Buy/Sell Foreign Investor Type Code <> '00'` (cả HOSE/HNX) — xét thứ nhì.
> - **Cá nhân trong nước** (loại trừ 2 nhánh trên, tức `Client House Classification Code = '10'` AND `Foreign Investor Type Code = '00'`): `Investor Type Code = '8000'` — áp dụng đồng nhất cả HOSE/HNX, không còn tách danh sách mã riêng theo sàn.
> - **Tổ chức trong nước** (cùng điều kiện loại trừ trên): `Investor Type Code <> '8000'` — áp dụng đồng nhất cả HOSE/HNX.
> ETL populate riêng phía Mua (`Buy ...`) và phía Bán (`Sell ...`) rồi UNION/FULL OUTER JOIN theo `(Symbol, Trade Date, Investor Category Code)` — xem chi tiết Section 1 Cụm 1c.
> **[SỬA 2026-09-21 lần 2, theo yêu cầu Data Modeler — 5 dòng BA đang soạn dở]** `BA_analyst_GSTT.csv` (STT 28) có thêm 5 dòng con: "Giá trị mua theo phân loại nhà đầu tư", "Giá trị bán theo phân loại nhà đầu tư", "Giá trị ròng theo phân loại nhà đầu tư", "Giá trị mua ròng theo phân loại nhà đầu tư", "Giá trị bán ròng theo phân loại nhà đầu tư" — cột "Nhóm yêu cầu"/"Bảng nguồn"/"SQL tham khảo" của 4/5 dòng đang để trống (BA chưa điền xong), chỉ 1 dòng có đủ thông tin. Tên gọi khớp 1-1 với 5 KPI đã khai sinh ở Nhóm 29 (K_GSTT_90 GT mua, K_GSTT_91 GT bán, K_GSTT_92 GT ròng, K_GSTT_93 GT mua ròng, K_GSTT_94 GT bán ròng — cùng công thức `SUM(Buy/Sell Value) WHERE Investor Category Code = :nhánh`, tham số hóa theo K_GSTT_85) — xác nhận trực tiếp với Data Modeler: **reuse thẳng 5 KPI này vào Nhóm 28**, không khai ID mới (dedup theo đúng quy ước đã dùng cho K_GSTT_83/77). Khác K_GSTT_86/87/83/77 (4 giá trị RÒNG cố định, 1 cho mỗi phân khúc, phục vụ biểu đồ cột hiển thị đồng thời cả 4 phân khúc) — K_GSTT_90-94 là bộ 5 giá trị THAM SỐ HÓA theo 1 phân khúc được chọn qua slicer K_GSTT_85 (phục vụ nhu cầu xem chi tiết mua/bán/khớp lệnh của TỪNG phân khúc riêng lẻ, bổ sung cho biểu đồ cột 4-cột đã có) — 2 cách hiển thị khác nhau trên cùng 1 nguồn dữ liệu, không loại trừ nhau.
> **[SỬA 2026-09-22 lần 2 — SỬA LỖI đối chiếu lại nguyên văn BA, tự-review theo phản hồi user]** Đối chiếu lại nguyên văn 6 dòng con "Giá trị .../ Tự doanh Cá nhân Tổ chức trong nước Nước ngoài" của STT 28 (`ba_slice.py --nhom 28 --with-sql`, dòng 409–414) phát hiện SAI ở lần sửa 2026-09-21 lần 2 phía trên: dòng 412/413/414 **KHÔNG PHẢI** "Giá trị mua ròng"/"Giá trị bán ròng" như đã ghi nhầm — tên đúng nguyên văn là **"Giá trị mua theo phân loại nhà đầu tư loại khớp lệnh"** (dòng 412), **"Giá trị bán theo phân loại nhà đầu tư loại khớp lệnh"** (dòng 413), **"Giá trị ròng theo phân loại nhà đầu tư theo khớp lệnh"** (dòng 414) — kèm Điều kiện tổng hợp tường minh: `MARKET_ID IN ('STK','STX','UPX')` + `BOARD_TYPE/BOARD_ID IN ('G1','G2','G3','G4','G7','G8')`. Đây là 3 chỉ tiêu **Mua/Bán/Ròng riêng cho phần KHỚP LỆNH** (loại trừ thỏa thuận), hoàn toàn khác khái niệm "mua ròng > 0 / bán ròng < 0" (tách dấu của GT ròng tổng) mà K_GSTT_93/94 đang biểu diễn. **K_GSTT_93/94 không có dòng BA nào của STT 28 làm căn cứ** — dòng "reuse 1-1" ghi ở lần sửa trước là SAI, đã bị nhầm với 2 KPI cùng tên khai sinh trước đó ở Nhóm 29 (bản thân 2 KPI đó ở Nhóm 29 cũng không có dòng BA STT 29 nào đứng tên tương ứng — xem ghi chú sửa lỗi tương tự ở Nhóm 29). **Hành động:** (1) Đánh `DEPRECATED` cho K_GSTT_93/94 tại Nhóm này (giữ dòng lại để truy vết lịch sử, không xóa ID). (2) Khai 3 KPI mới đúng theo BA: `K_GSTT_153` (GT mua khớp lệnh), `K_GSTT_154` (GT bán khớp lệnh), `K_GSTT_155` (GT ròng khớp lệnh = 153 − 154) — khai sinh tại Nhóm 29 (BA STT 29 có đủ bộ 3 dòng tương ứng dòng 424–426, dùng chung `Matched Buy/Sell Value` đã có sẵn trên `Fact Investor Category Trading Snapshot`, cùng cột phục vụ K_GSTT_88), Nhóm 28 reuse cả 3. (3) Cập nhật Mockup: bỏ cột "GT mua ròng | GT bán ròng" (không có căn cứ), thêm "GT mua khớp lệnh | GT bán khớp lệnh | GT ròng khớp lệnh".

> **[SỬA 2026-09-22 lần 3 — SỬA LỖI bỏ sót "Giá mở cửa" và "thay đổi", tự-review theo phản hồi user]** Rà lại toàn bộ 14 dòng con (401–414) phát hiện bỏ sót 2 dòng đã có sẵn KPI ID mà trước đây ghi nhầm "không có" hoặc lặng lẽ bỏ qua: (1) **Dòng 403 "Giá mở cửa"** (Đánh giá: Trùng, Bảng nguồn `JAD_MARKETINFOR/JAD_STOCKINFOR`, Trường nguồn `open`) → reuse **K_GSTT_27** (khai sinh Nhóm 3, cùng `Security Trading Snapshot Dimension.Open Price`) — ghi chú cũ "Nhóm 3 (Giá cao/thấp/đóng cửa — không có Giá mở cửa)" là SAI, dòng 403 tồn tại rõ ràng trong BA. (2) **Dòng 407 "thay đổi"** (Đánh giá: Trùng, Mô tả "So sánh giữa giá tham chiếu với giá đóng cửa", cùng công thức LAG(closeIndex) với K_GSTT_11) → reuse **K_GSTT_11** (khai sinh Nhóm 1, "Thay đổi (+/-)" = Giá đóng cửa − Giá TC) — trước đây chỉ lấy % thay đổi (K_GSTT_12, dòng 408) mà bỏ sót giá trị tuyệt đối (K_GSTT_11, dòng 407) đứng ngay trước đó. Bổ sung cả 2 vào Bảng KPI và Mockup.

**Mockup:**

| Phân loại NĐT | Mã ck | Chỉ số | Giá mở | Giá cao | Giá thấp | Giá đóng | Thay đổi | % thay đổi | GT cá nhân ròng | GT tổ chức trong nước ròng | GT tự doanh ròng | GT NN ròng | GT mua | GT bán | GT ròng | GT mua khớp lệnh | GT bán khớp lệnh | GT ròng khớp lệnh |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Cá nhân | VCB | VN30 | 82.00 | 83.00 | 81.50 | 82.50 | +0.50 | +0.61% | +2.5 Tỷ | -1.8 Tỷ | +1.1 Tỷ | +0.4 Tỷ | 12.3 Tỷ | 9.8 Tỷ | +2.5 Tỷ | 10.1 Tỷ | 8.2 Tỷ | +1.9 Tỷ |

> **[SỬA 2026-09-22]** Bỏ 2 cột "GT mua ròng"/"GT bán ròng" khỏi mockup (không có dòng BA STT 28 nào làm căn cứ, xem ghi chú sửa lỗi trên) — thay bằng "GT mua khớp lệnh"/"GT bán khớp lệnh"/"GT ròng khớp lệnh" đúng theo dòng 412–414 của BA. **[SỬA 2026-09-22 lần 3]** Bổ sung cột "Giá mở" (K_GSTT_27) và "Thay đổi" (K_GSTT_11) bị bỏ sót.

**Source:** `Fact Investor Category Trading Snapshot` (mới) + `Fact Stock Portfolio Snapshot` (Tự doanh/NN, reuse Nhóm 25/27) → `Security Trading Snapshot Dimension`, `Index Constituent Dimension`.

**Bảng KPI:**

| KPI ID | Tên KPI | Đơn vị | Tính chất | Công thức | Ghi chú | Trạng thái |
|---|---|---|---|---|---|---|
| K_GSTT_85 | Phân loại nhà đầu tư | — | Chiều | `Fact Investor Category Trading Snapshot.Investor Category Code` — 4 giá trị: Cá nhân, Tổ chức trong nước, Tự doanh, Nước ngoài | Là cột vật lý (FK/attribute thật trên Fact mới), không còn Slicer ảo CASE WHEN (SỬA 2026-09-21). **[SỬA 2026-09-22]** BA đơn giản hoá điều kiện phân loại khi populate cột này: NĐTNN = `Foreign Investor Type Code <> '00'` (trước đây `IN ('10','20')`); Cá nhân/Tổ chức trong nước = `Investor Type Code = '8000'` / `<> '8000'` đồng nhất HOSE-HNX (trước đây bảng mã dài riêng theo sàn) — xem ghi chú nguồn trên | READY |
| K_GSTT_1 | Mã ck | — | Chiều | `Security Trading Snapshot Dimension.Symbol` | Reuse từ Nhóm 1 | READY |
| K_GSTT_4 | Chỉ số | — | Chiều | `Index Constituent Dimension.Index Name` | Reuse từ Nhóm 1. **[SỬA 2026-09-22, theo yêu cầu user]** Đổi từ `Index Code` sang `Index Name` — cột hiển thị nhãn chỉ số trên Mockup (VD "VN30"), cùng cơ chế đã dùng cho K_GSTT_4 ở Nhóm 35 | READY |
| K_GSTT_27 | Giá mở cửa | VNĐ | Cơ sở | `Security Trading Snapshot Dimension.Open Price` | **[MỚI 2026-09-22 lần 3]** Reuse từ Nhóm 3 — bổ sung, dòng BA STT 28 (dòng 403) bị bỏ sót trước đây | READY |
| K_GSTT_28 | Giá cao nhất | VNĐ | Cơ sở | `Security Trading Snapshot Dimension.High Price` | Reuse từ Nhóm 3 | READY |
| K_GSTT_29 | Giá thấp nhất | VNĐ | Cơ sở | `Security Trading Snapshot Dimension.Low Price` | Reuse từ Nhóm 3 | READY |
| K_GSTT_10 | Giá đóng cửa | VNĐ | Cơ sở | `Security Trading Snapshot Dimension.Close Price` | Reuse từ Nhóm 1 | READY |
| K_GSTT_11 | Thay đổi (+/-) | VNĐ | Cơ sở | `Security Trading Snapshot Dimension.Price Change` | **[MỚI 2026-09-22 lần 3]** Reuse từ Nhóm 1 — bổ sung, dòng BA STT 28 (dòng 407) bị bỏ sót trước đây, chỉ lấy nhầm % thay đổi (K_GSTT_12) mà bỏ qua giá trị tuyệt đối | READY |
| K_GSTT_12 | % thay đổi | % | Phái sinh | `Price Change / Reference Price × 100` | Reuse từ Nhóm 1 | READY |
| K_GSTT_86 | GT cá nhân ròng | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Buy Value) − SUM(Fact Investor Category Trading Snapshot.Sell Value) WHERE Investor Category Code = 'Cá nhân' GROUP BY Symbol, Trade Date` | **[SỬA 2026-09-21]** Nguồn `Fact Investor Category Trading Snapshot` (mới) — filter đơn giản theo Investor Category Code, không còn CASE WHEN switch cột | READY |
| K_GSTT_87 | GT tổ chức trong nước ròng | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Buy Value) − SUM(Fact Investor Category Trading Snapshot.Sell Value) WHERE Investor Category Code = 'Tổ chức trong nước' GROUP BY Symbol, Trade Date` | **[SỬA 2026-09-21]** Cùng cơ chế K_GSTT_86 | READY |
| K_GSTT_83 | GT tự doanh ròng | VNĐ | Phái sinh | `K_GSTT_82 − K_GSTT_84` | Trùng hoàn toàn K_GSTT_83 (Nhóm 27) — không khai KPI mới, giữ nguyên nguồn `Fact Stock Portfolio Snapshot` | READY |
| K_GSTT_77 | GT NN ròng | VNĐ | Phái sinh | `K_GSTT_72 − K_GSTT_73` | Trùng hoàn toàn K_GSTT_77 (Nhóm 25) — không khai KPI mới, giữ nguyên nguồn `Fact Stock Portfolio Snapshot` | READY |
| K_GSTT_90 | GT mua | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Buy Value) WHERE Investor Category Code = :nhánh (K_GSTT_85) GROUP BY Symbol, Trade Date` | **[MỚI 2026-09-21]** Reuse từ Nhóm 29 — BA STT 28 thêm dòng "Giá trị mua theo phân loại nhà đầu tư" khớp 1-1 công thức, không khai ID mới | READY |
| K_GSTT_91 | GT bán | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Sell Value) WHERE Investor Category Code = :nhánh (K_GSTT_85) GROUP BY Symbol, Trade Date` | **[MỚI 2026-09-21]** Reuse từ Nhóm 29 — BA STT 28 thêm dòng "Giá trị bán theo phân loại nhà đầu tư" khớp 1-1 công thức, không khai ID mới | READY |
| K_GSTT_92 | GT ròng | VNĐ | Phái sinh | `K_GSTT_90 − K_GSTT_91` | **[MỚI 2026-09-21]** Reuse từ Nhóm 29 — BA STT 28 thêm dòng "Giá trị ròng theo phân loại nhà đầu tư" khớp 1-1 công thức, không khai ID mới. Khác K_GSTT_86/87/83/77 — đây là bản THAM SỐ HÓA theo 1 phân khúc chọn qua K_GSTT_85, không phải 4 giá trị cố định song song | READY |
| K_GSTT_93 | GT mua ròng | VNĐ | Phái sinh | `K_GSTT_92 WHERE K_GSTT_92 > 0` | **[DEPRECATED 2026-09-22]** Ghi nhầm là reuse của dòng BA "Giá trị mua ròng theo phân loại nhà đầu tư" — dòng BA STT 28 tương ứng thực ra là "Giá trị mua theo phân loại nhà đầu tư loại khớp lệnh" (khác khái niệm hoàn toàn, xem K_GSTT_153). Không có dòng BA nào của STT 28/29 làm căn cứ cho phép tách dấu dương/âm này — giữ dòng để truy vết lịch sử, không dùng cho thiết kế mới | DEPRECATED |
| K_GSTT_94 | GT bán ròng | VNĐ | Phái sinh | `K_GSTT_92 WHERE K_GSTT_92 < 0` | **[DEPRECATED 2026-09-22]** Cùng lý do K_GSTT_93 — dòng BA STT 28 tương ứng thực ra là "Giá trị bán theo phân loại nhà đầu tư loại khớp lệnh" (xem K_GSTT_154) | DEPRECATED |
| K_GSTT_153 | GT mua khớp lệnh | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Matched Buy Value) WHERE Investor Category Code = :nhánh (K_GSTT_85) GROUP BY Symbol, Trade Date` | **[MỚI 2026-09-22]** Khai sinh tại Nhóm 29 (BA STT 29 dòng "GT Mua của khớp lệnh theo phân loại nhà đầu tư..."), reuse tại Nhóm 28 đúng dòng "Giá trị mua theo phân loại nhà đầu tư loại khớp lệnh" (STT 28). Dùng `Matched Buy Value` đã có sẵn trên Fact (cùng cột phục vụ K_GSTT_88) — không cần cột mới | READY |
| K_GSTT_154 | GT bán khớp lệnh | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Matched Sell Value) WHERE Investor Category Code = :nhánh (K_GSTT_85) GROUP BY Symbol, Trade Date` | **[MỚI 2026-09-22]** Cùng cơ chế K_GSTT_153, chiều bán — reuse tại Nhóm 28 đúng dòng "Giá trị bán theo phân loại nhà đầu tư loại khớp lệnh" (STT 28) | READY |
| K_GSTT_155 | GT ròng khớp lệnh | VNĐ | Phái sinh | `K_GSTT_153 − K_GSTT_154` | **[MỚI 2026-09-22]** Khai sinh tại Nhóm 29, reuse tại Nhóm 28 đúng dòng "Giá trị ròng theo phân loại nhà đầu tư theo khớp lệnh" (STT 28) | READY |
...
> `Investor_Category_Code` là Classification Value thuần (giá trị: `CA_NHAN`/`TO_CHUC_TRONG_NUOC`/`TU_DOANH`/`NUOC_NGOAI`) — không FK sang Dimension riêng, theo đúng Rule #4 CLAUDE.md (Classification Value chỉ 1 trường Code). K_GSTT_83/77 (Tự doanh/NN ròng, reuse Nhóm 25/27) tiếp tục dùng `Fact Stock Portfolio Snapshot` không đổi — không xuất hiện trong erDiagram này.

**Lineage Mart → Báo cáo:**

```mermaid
flowchart LR
    F1["Fact Investor Category Trading Snapshot"] --> RPT42["Xu hướng dòng tiền — GD theo phân loại NĐT (biểu đồ GT ròng)"]
    F3["Fact Stock Portfolio Snapshot"] --> RPT42
    D1["Security Trading Snapshot Dimension"] --> RPT42
    D4["Index Constituent Dimension"] --> RPT42
```

**Bảng grain:** `Fact Investor Category Trading Snapshot` — 1 row/mã CK/ngày/Phân loại NĐT (4 dòng/mã CK/ngày). K_GSTT_83/77 vẫn grain `Fact Stock Portfolio Snapshot` — 1 row/mã CK/ngày.

> **Coverage rule:** Áp dụng cho `Fact Investor Category Trading Snapshot` — đủ cả 4 phân khúc NĐT (Cá nhân/Tổ chức trong nước/Tự doanh/Nước ngoài) trên cùng 1 Fact, tránh bổ sung lẻ tẻ ở Nhóm 29. **[SỬA 2026-09-21]** Thay thế Coverage rule cũ (áp dụng cho `Fact Stock Portfolio Snapshot`) — xem lịch sử nguồn ở trên.
...
> **Phân loại:** Dashboard
> **Atomic:** 100% reuse Nhóm 1 (`Security Trading Snapshot`, `Securities Trade`) + Nhóm 28 (Phân loại NĐT, `Fact Investor Category Trading Snapshot`) — bổ sung "GT khớp lệnh"/"GT thỏa thuận" tách theo Board Type Code (cùng cơ chế với K_GSTT_17/18, Nhóm 1), nay lưu sẵn trên Fact mới thay vì tính động qua CASE WHEN.
>
> **Ghi chú tái sử dụng:** BA liệt kê 15 dòng con (dòng 415–429) — 5 dòng đã có KPI ID sẵn từ Nhóm 1 (Mã chứng khoán, Chỉ số, Giá, Thay đổi, % thay đổi) và Nhóm 28 (Phân loại nhà đầu tư = K_GSTT_85) — reuse thẳng. **[SỬA 2026-09-22 lần 3]** Trước đây bỏ sót dòng 419 "thay đổi" (giá trị tuyệt đối) → reuse **K_GSTT_11**, chỉ lấy nhầm % thay đổi (K_GSTT_12, dòng 420). 9 dòng giá trị (dòng 421–429) tổ chức theo 3 nhóm × Mua/Bán/Ròng (Tổng/Khớp lệnh/Thỏa thuận) — xem ghi chú sửa lỗi dưới đây.
> **[SỬA 2026-09-21, theo yêu cầu Data Modeler]** "GT khớp lệnh"/"GT thỏa thuận" (K_GSTT_88/89) và "GT mua"/"GT bán"/"GT ròng" theo phân loại NĐT (K_GSTT_90-94) nay đọc trực tiếp từ `Fact Investor Category Trading Snapshot` (Cụm 1c, xem Nhóm 28) — Fact grain `mã CK/ngày/Phân loại NĐT` có sẵn 6 cột giá trị (`Buy Value`, `Sell Value`, `Matched Buy/Sell Value`, `Negotiated Buy/Sell Value`), tất cả populate 1 lần tại ETL cho cả 4 phân khúc (Cá nhân/Tổ chức trong nước/Tự doanh/Nước ngoài). Tầng BI chỉ còn `WHERE Investor Category Code = :nhánh` (tham số từ K_GSTT_85) — **KHÔNG còn `CASE {nhánh} WHEN 'Ca nhan' THEN individual_buy_val WHEN...`** switch giữa các cột khác nguồn/khác bảng như thiết kế trước (Resolved 2026-09-04/2026-09-19). Ghi chú lịch sử "GT khớp lệnh"/"GT thỏa thuận" Resolved 2026-09-04 + bổ sung R1 2026-09-17 giữ nguyên không xóa (precedent O_GSTT_20) — chỉ đổi nguồn vật lý từ `securities_trade` (tính động) sang cột đã pre-aggregate sẵn trên Fact mới.
> **[SỬA 2026-09-21 lần 2 — "Tổng GTGD" tách khỏi K_GSTT_14, khai KPI mới K_GSTT_152]** Rà lại theo yêu cầu Data Modeler + đối chiếu SQL tham khảo gốc của BA (STT 29, dòng "Chỉ số" — CTE `giao_dich_phan_loai`/`gt_mua_theo_ngay`/`gt_ban_theo_ngay`/`gt_rong_theo_ngay`, tham số `:loai_gtgd` toggle `TONG_GTGD`/`KHOP_LENH`): "Tổng GTGD" trong ngữ cảnh bảng NÀY (cùng dòng với các cột đã lọc theo Phân loại NĐT) **phải lọc theo Phân loại NĐT** — trước đây nhầm reuse thẳng K_GSTT_14 (Nhóm 1, tổng thị trường KHÔNG lọc NĐT, không đổi giá trị dù đổi dòng Phân loại NĐT đang xem — sai ngữ nghĩa bảng). SQL tham khảo gốc của BA xác nhận `gt_mua`/`gt_ban`/`gt_rong` đều tính trên tập đã phân loại theo NĐT (`giao_dich_phan_loai`, JOIN `DANH_MUC_NDT`/phân loại theo `Client House Classification Code`) — TONG_GTGD (không lọc Board Type) là 1 trong các state của tham số `:loai_gtgd`, cùng cấp với KHOP_LENH, áp dụng trên tập ĐÃ phân loại NĐT đó, không phải tổng thị trường thô. **Khai KPI mới K_GSTT_152** ("Tổng GTGD theo phân loại NĐT") = `Buy Value + Sell Value` (không lọc Board Type, cùng cơ chế "Tổng" = TONG_GTGD của `:loai_gtgd`) trên `Fact Investor Category Trading Snapshot`, filter theo Investor Category Code — không đổi cột vật lý (đã có sẵn `buy_val`/`sell_val`), chỉ khai KPI_ID mới do công thức khác K_GSTT_14 (K_GSTT_14 giữ nguyên định nghĩa cũ, vẫn dùng cho Nhóm 1/3/9/10/23/24/30, KHÔNG đổi). Nguồn phân loại NĐT áp dụng đồng nhất `Investor Type Code` (đã xác nhận, không dùng `BM_DANH_MUC_NDT` trong SQL tham khảo — bảng đó chưa có trên Atomic, xem ghi chú Nhóm 28).
> **[SỬA 2026-09-22 — SỬA LỖI đối chiếu lại nguyên văn BA STT 29, tự-review theo phản hồi user]** BA STT 29 thực ra có **9 dòng giá trị** (không phải 5 như "GT mua/GT bán/GT ròng/GT mua ròng/GT bán ròng" đã thiết kế) — tổ chức thành 3 nhóm × (Mua/Bán/Ròng): **Tổng** (dòng "GT Mua/Bán/ròng theo phân loại nhà đầu tư tự doanh/nước ngoài" → K_GSTT_90/91/92, không đổi), **Khớp lệnh** (dòng "GT Mua/Bán/ròng CỦA KHỚP LỆNH theo phân loại nhà đầu tư tự doanh/nước ngoài", điều kiện Board Type IN ('G1'-'G4')) và **Thỏa thuận** (dòng "GT Mua/Bán/ròng CỦA THỎA THUẬN theo phân loại nhà đầu tư tự doanh/nước ngoài", điều kiện Board Type IN ('T1','T2','T3','T4','T6','R1')) — **KHÔNG có dòng BA nào tên "GT mua ròng"/"GT bán ròng"** (tách dấu dương/âm của GT ròng tổng) như K_GSTT_93/94 đang mô tả — 2 KPI này bị khai sinh nhầm ở lần renumber v4.3 (trước khi BA điền đủ chi tiết 9 dòng) và không có căn cứ. **Hành động:** (1) Đánh `DEPRECATED` cho K_GSTT_93/94. (2) Khai 6 KPI mới đúng theo 6 dòng còn thiếu: `K_GSTT_153` (GT mua khớp lệnh), `K_GSTT_154` (GT bán khớp lệnh), `K_GSTT_155` (GT ròng khớp lệnh = 153−154) — dùng `Matched Buy/Sell Value` đã có sẵn trên Fact (cùng cột phục vụ K_GSTT_88); `K_GSTT_156` (GT mua thỏa thuận), `K_GSTT_157` (GT bán thỏa thuận), `K_GSTT_158` (GT ròng thỏa thuận = 156−157) — dùng `Negotiated Buy/Sell Value` đã có sẵn (cùng cột phục vụ K_GSTT_89). K_GSTT_88 ("GT khớp lệnh" = Matched Buy+Sell cộng lại) và K_GSTT_89 ("GT thỏa thuận" = Negotiated Buy+Sell cộng lại) **giữ nguyên không đổi** — vẫn là tổng GTGD 2 chiều hợp lệ, chỉ bổ sung thêm 6 KPI trên để có đủ chi tiết Mua/Bán riêng theo đúng 9 dòng BA. K_GSTT_153/154/155 khai sinh tại Nhóm này, reuse ngược sang Nhóm 28 (BA STT 28 có 3 dòng tương ứng, không có nhánh thỏa thuận).

**Mockup:**

| Mã CK | Chỉ số | Phân loại NĐT | GT khớp lệnh | GT mua khớp lệnh | GT bán khớp lệnh | GT ròng khớp lệnh | GT thỏa thuận | GT mua thỏa thuận | GT bán thỏa thuận | GT ròng thỏa thuận | Tổng GTGD | GT mua | GT bán | GT ròng | Giá | Thay đổi | % thay đổi |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| VCB | VN30 | Cá nhân | 18.3 Tỷ | 10.1 Tỷ | 8.2 Tỷ | +1.9 Tỷ | 3.8 Tỷ | 2.2 Tỷ | 1.6 Tỷ | +0.6 Tỷ | 22.1 Tỷ | 12.3 Tỷ | 9.8 Tỷ | +2.5 Tỷ | 82.50 | +0.50 | +0.61% |

> **[SỬA 2026-09-22]** Bỏ 2 cột "GT mua ròng"/"GT bán ròng" khỏi mockup (không có dòng BA STT 29 nào làm căn cứ, xem ghi chú sửa lỗi trên) — thêm đủ 6 cột Mua/Bán/Ròng riêng cho Khớp lệnh và Thỏa thuận đúng theo 9 dòng giá trị của BA. **[SỬA 2026-09-22 lần 3]** Bổ sung cột "Thay đổi" (K_GSTT_11, dòng 419) bị bỏ sót.

**Source:** `Fact Investor Category Trading Snapshot` (mới, K_GSTT_88-94 [93/94 DEPRECATED], K_GSTT_152-158) → `Security Trading Snapshot Dimension`, `Index Constituent Dimension`.
...
| K_GSTT_4 | Chỉ số | — | Chiều | `Index Constituent Dimension.Index Code` | Reuse từ Nhóm 1 | READY |
| K_GSTT_85 | Phân loại nhà đầu tư | — | Chiều | `Fact Investor Category Trading Snapshot.Investor Category Code` — 4 giá trị: Cá nhân, Tổ chức trong nước, Tự doanh, Nước ngoài | Reuse từ Nhóm 28 — cột vật lý thật trên Fact mới (SỬA 2026-09-21). **[SỬA 2026-09-22]** BA cập nhật lại điều kiện phân loại khi populate Fact (NĐTNN: `<> '00'` thay vì `IN ('10','20')`; Cá nhân/Tổ chức trong nước: `Investor Type Code = '8000'`/`<> '8000'` đồng nhất HOSE-HNX) — xem chi tiết Nhóm 28, không ảnh hưởng K_GSTT_88-94/152 (chỉ filter theo Investor Category Code đã tính sẵn) | READY |
| K_GSTT_88 | GT khớp lệnh | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Matched Buy Value) + SUM(Fact Investor Category Trading Snapshot.Matched Sell Value) WHERE Investor Category Code = :nhánh (K_GSTT_85) GROUP BY Symbol, Trade Date` | **[SỬA 2026-09-21]** Đổi nguồn sang `Fact Investor Category Trading Snapshot` (Matched Buy/Sell Value pre-aggregate sẵn theo Board Type Code NOT IN thỏa thuận, đã gồm `R1`) — không còn SUM động trên `securities_trade`. Lịch sử Resolved 2026-09-04 + bổ sung R1 2026-09-17 giữ nguyên | READY |
| K_GSTT_89 | GT thỏa thuận | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Negotiated Buy Value) + SUM(Fact Investor Category Trading Snapshot.Negotiated Sell Value) WHERE Investor Category Code = :nhánh (K_GSTT_85) GROUP BY Symbol, Trade Date` | **[SỬA 2026-09-21]** Cùng cơ chế K_GSTT_88 | READY |
| K_GSTT_152 | Tổng GTGD theo phân loại NĐT | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Buy Value) + SUM(Fact Investor Category Trading Snapshot.Sell Value) WHERE Investor Category Code = :nhánh (K_GSTT_85) GROUP BY Symbol, Trade Date` | **[MỚI 2026-09-21]** KPI mới, thay cho reuse K_GSTT_14 (Nhóm 1) — K_GSTT_14 là tổng thị trường KHÔNG lọc NĐT, sai ngữ nghĩa khi đặt cùng bảng đã lọc theo Phân loại NĐT. Tương ứng state `TONG_GTGD` của tham số `:loai_gtgd` trong SQL tham khảo gốc BA (không lọc Board Type, trên tập đã phân loại NĐT) — cùng cấp với KHOP_LENH (K_GSTT_88). K_GSTT_14 giữ nguyên định nghĩa cũ, không đổi, vẫn dùng cho Nhóm 1/3/9/10/23/24/30 | READY |
| K_GSTT_90 | GT mua | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Buy Value) WHERE Investor Category Code = :nhánh (K_GSTT_85) GROUP BY Symbol, Trade Date` | **[SỬA 2026-09-21]** Đổi nguồn sang `Fact Investor Category Trading Snapshot` — filter đơn giản theo Investor Category Code, thay cho `CASE {nhanh} WHEN 'Ca nhan' THEN individual_buy_val WHEN...` trước đây | READY |
| K_GSTT_91 | GT bán | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Sell Value) WHERE Investor Category Code = :nhánh (K_GSTT_85) GROUP BY Symbol, Trade Date` | **[SỬA 2026-09-21]** Cùng cơ chế K_GSTT_90, chiều bán | READY |
| K_GSTT_92 | GT ròng | VNĐ | Phái sinh | `K_GSTT_90 − K_GSTT_91` | Derive tại tầng BI, không đổi công thức | READY |
| K_GSTT_93 | GT mua ròng | VNĐ | Phái sinh | `K_GSTT_92 WHERE K_GSTT_92 > 0` | **[DEPRECATED 2026-09-22]** Không có dòng BA STT 29 nào làm căn cứ cho phép tách dấu dương/âm này — khai sinh nhầm trước khi BA điền đủ 9 dòng chi tiết (xem ghi chú sửa lỗi trên). Giữ dòng để truy vết lịch sử, không dùng cho thiết kế mới | DEPRECATED |
| K_GSTT_94 | GT bán ròng | VNĐ | Phái sinh | `K_GSTT_92 WHERE K_GSTT_92 < 0` | **[DEPRECATED 2026-09-22]** Cùng lý do K_GSTT_93 | DEPRECATED |
| K_GSTT_153 | GT mua khớp lệnh | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Matched Buy Value) WHERE Investor Category Code = :nhánh (K_GSTT_85) GROUP BY Symbol, Trade Date` | **[MỚI 2026-09-22]** Khai sinh tại Nhóm này — BA STT 29 dòng "GT Mua của khớp lệnh theo phân loại nhà đầu tư tự doanh/nước ngoài". Dùng `Matched Buy Value` đã có sẵn trên Fact (cùng cột phục vụ K_GSTT_88) — không cần cột mới. Reuse ngược sang Nhóm 28 | READY |
| K_GSTT_154 | GT bán khớp lệnh | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Matched Sell Value) WHERE Investor Category Code = :nhánh (K_GSTT_85) GROUP BY Symbol, Trade Date` | **[MỚI 2026-09-22]** Cùng cơ chế K_GSTT_153, chiều bán — BA STT 29 dòng "GT Bán của khớp lệnh...". Reuse ngược sang Nhóm 28 | READY |
| K_GSTT_155 | GT ròng khớp lệnh | VNĐ | Phái sinh | `K_GSTT_153 − K_GSTT_154` | **[MỚI 2026-09-22]** BA STT 29 dòng "GT ròng của khớp lệnh...". Reuse ngược sang Nhóm 28 | READY |
| K_GSTT_156 | GT mua thỏa thuận | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Negotiated Buy Value) WHERE Investor Category Code = :nhánh (K_GSTT_85) GROUP BY Symbol, Trade Date` | **[MỚI 2026-09-22]** BA STT 29 dòng "GT Mua của thỏa thuận theo phân loại nhà đầu tư tự doanh/nước ngoài". Dùng `Negotiated Buy Value` đã có sẵn trên Fact (cùng cột phục vụ K_GSTT_89) — không cần cột mới. Chỉ dùng ở Nhóm này (Nhóm 28 không có nhánh thỏa thuận) | READY |
| K_GSTT_157 | GT bán thỏa thuận | VNĐ | Phái sinh | `SUM(Fact Investor Category Trading Snapshot.Negotiated Sell Value) WHERE Investor Category Code = :nhánh (K_GSTT_85) GROUP BY Symbol, Trade Date` | **[MỚI 2026-09-22]** Cùng cơ chế K_GSTT_156, chiều bán — BA STT 29 dòng "GT Bán của thỏa thuận..." | READY |
| K_GSTT_158 | GT ròng thỏa thuận | VNĐ | Phái sinh | `K_GSTT_156 − K_GSTT_157` | **[MỚI 2026-09-22]** BA STT 29 dòng "GT ròng của thỏa thuận..." | READY |
| K_GSTT_10 | Giá | VNĐ | Cơ sở | `Security Trading Snapshot Dimension.Close Price` | Reuse từ Nhóm 1 (K_GSTT_10 = Giá đóng cửa) | READY |
| K_GSTT_11 | Thay đổi (+/-) | VNĐ | Cơ sở | `Security Trading Snapshot Dimension.Price Change` | **[MỚI 2026-09-22 lần 3]** Reuse từ Nhóm 1 — bổ sung, dòng BA STT 29 (dòng 419) bị bỏ sót trước đây, chỉ lấy nhầm % thay đổi (K_GSTT_12) mà bỏ qua giá trị tuyệt đối | READY |
| K_GSTT_12 | % thay đổi | % | Phái sinh | `Price Change / Reference Price × 100` | Reuse từ Nhóm 1 | READY |

**Star Schema:** 100% reuse `Fact Investor Category Trading Snapshot` (Nhóm 28, Cụm 1c) cho K_GSTT_88-94/152-158, `Security Trading Snapshot Dimension`/`Index Constituent Dimension` cho K_GSTT_1/4/10/12 — không tạo bảng mới ở Nhóm này. K_GSTT_93/94 DEPRECATED, không dùng Fact/cột nào.

**Lineage Mart → Báo cáo:**

```mermaid
flowchart LR
    F1["Fact Investor Category Trading Snapshot"] --> RPT43["Xu hướng dòng tiền — GD theo phân loại NĐT (bản đồ nhiệt)"]
    D1["Security Trading Snapshot Dimension"] --> RPT43
    D4["Index Constituent Dimension"] --> RPT43
```

**Bảng grain:** `Fact Investor Category Trading Snapshot` — 1 row/mã CK/ngày/Phân loại NĐT (K_GSTT_88-92, K_GSTT_152-158; K_GSTT_93/94 DEPRECATED).

> **Coverage rule:** Không áp dụng — Nhóm này không tạo/mở rộng Fact hay Dimension nào, 100% reuse `Fact Investor Category Trading Snapshot` (Nhóm 28).
...
| KPI ID | Tên KPI | Đơn vị | Tính chất | Công thức | Ghi chú | Trạng thái |
|---|---|---|---|---|---|---|
| K_GSTT_100 | Mã cổ phiếu | — | Chiều | `Operational Public Company Shareholding.Equity Ticker Symbol` | **[SỬA 2026-09-19]** Reuse từ Nhóm 31 — xem ghi chú sửa bug + denormalize tại đó | READY |
...
| K_GSTT_104 | Chức vụ người nội bộ | — | Chiều | `arrayStringConcat(Operational Public Company Shareholding.Position Code, ', ')` | **[SỬA 2026-09-19]** Reuse từ Nhóm 31 — xem ghi chú sửa bug tại đó | READY |
...
> **[SỬA 2026-09-22, datamart-review]** K_GSTT_35 — thay hàm giả `mapping(Index Code)` (chưa phải SQL thật) bằng khóa JOIN đã xác nhận `Market Index Dimension.Index Name = Index Constituent Dimension.Index Code` (nguồn `MDDS.JAD_MARKETINFOR.INDEXNAME`) — xem chi tiết tại dòng K_GSTT_35 trong bảng KPI và O_GSTT_3 (Section 5). K_GSTT_39 ("% thay đổi") vẫn dùng `mapping(Index Code)` — CHƯA sửa trong lượt này (ngoài phạm vi yêu cầu), dù cùng pattern và có thể áp dụng cùng khóa join này khi rà soát riêng.
...
| K_GSTT_4 | Chỉ số | — | Chiều | `Index Constituent Dimension.Index Name` | Đổi từ biến thể `Market Index Dimension` (Nhóm 5, cấp chỉ số) sang biến thể `Index Constituent Dimension` (Nhóm 1, cấp mã CK) — đúng grain Nhóm 35 | READY |
| K_GSTT_35 | Giá trị chỉ số | Điểm | Cơ sở | `LOOKUP Fact Index Constituent Snapshot ON Fact Index Constituent Snapshot.Index Constituent Dimension Id = Index Constituent Dimension.Index Constituent Dimension Id AND <mã CK đang xét> → Index Constituent Dimension.Index Code → LOOKUP Market Index Dimension ON Market Index Dimension.Index Name = Index Constituent Dimension.Index Code → LOOKUP Fact Market Index Snapshot ON Fact Market Index Snapshot.Market Index Dimension Id = Market Index Dimension.Market Index Dimension Id AND cùng Snapshot Date → Fact Market Index Snapshot.Market Index Value` | **[SỬA 2026-09-22, datamart-review]** Thay hàm giả `mapping(Index Code)` (chưa phải SQL thật) bằng khóa JOIN đã xác nhận `Market Index Dimension.Index Name = Index Constituent Dimension.Index Code` (nguồn `MDDS.JAD_MARKETINFOR.INDEXNAME`, xác nhận 2026-09-22 — cùng khóa vừa áp dụng cho K_GSTT_35 Nhóm 6, xem O_GSTT_3 Section 5). Vẫn giữ nguyên cơ chế JOIN qua Bridge (`Fact Index Constituent Snapshot`) theo mã CK đang xét (cùng cơ chế K_GSTT_75, Nhóm 24) để lấy đúng Index Code của mã đó trước khi bridge sang Fact Market Index Snapshot — giá trị lặp lại trên mọi mã cùng rổ | READY |
| K_GSTT_39 | % thay đổi | % | Phái sinh | `Fact Market Index Snapshot.Index Percent Change WHERE Market Code = mapping(Index Code) AND cùng ngày, JOIN qua Bridge theo mã CK` | Đổi từ `K_GSTT_38` (thay đổi tuyệt đối) sang `K_GSTT_39` (% thay đổi) — theo sheet Tổng hợp công thức, dòng 3 của Nhóm 35 là `(Giá đóng cửa/Giá tham chiếu − 1)×100`. **Ghi chú 2026-09-22:** vẫn dùng hàm giả `mapping(Index Code)` — chưa sửa trong lượt này, ngoài phạm vi yêu cầu (xem ghi chú đầu Nhóm) | READY |
| K_GSTT_47 | KLGD | Cổ phiếu | Phái sinh | `Fact Index Constituent Snapshot.Index Total Matched Volume` (cột tính sẵn trên Bridge, KHÔNG cần MAX/GROUP BY — Bridge đã đúng grain Symbol×Index×Date) | Reuse từ Nhóm 5 (K_GSTT_47 = KLGD của chỉ số), bỏ MAX/GROUP BY vì Nhóm 35 hiển thị theo mã CK | READY |
...
**Bảng grain:** Không có bảng mới — cùng grain `Fact Index Constituent Snapshot` (Bridge, Symbol × Index × Date) đã có ở Nhóm 1. Grain hiển thị Nhóm 35 = 1 dòng/mã CK (khác Nhóm 5 = 1 dòng/chỉ số) dù cùng nguồn dữ liệu. **[SỬA 2026-09-22]** K_GSTT_35 lookup thêm từ `Fact Market Index Snapshot` (`fct_market_index_snpst`, reuse — sở hữu QLKD) qua khóa Index Name = Index Code, không làm thay đổi grain hiển thị của Nhóm 35.
...
| O_GSTT_3 | Nhóm 5, Nhóm 6, Nhóm 35 | K_GSTT_47–49 (KLGD/GTGD/KLNN ròng/GTNN ròng theo chỉ số) cần bảng mapping `Market Code ↔ Index Code` (VD: market_code='30' ↔ index_code='VN30') vì `Market Index Dimension` (định danh Market Code/Index Type Code) và `Index Constituent Dimension` (định danh Index Code) dùng 2 hệ định danh khác nhau, không có join key 1-1 sẵn có trong Atomic. Cần xác nhận với nghiệp vụ bảng mapping đầy đủ trước khi lên LLD — **vẫn Open cho K_GSTT_47-49** (Nhóm 5), chưa sửa trong lượt này. **[SỬA 2026-09-22, datamart-review — Resolved một phần]** Phần "lấy giá trị điểm chỉ số theo Index Code" (K_GSTT_35, Nhóm 6 + Nhóm 35) nay đã Resolved bằng khóa join khác — KHÔNG dùng `Market Code`, mà dùng `Market Index Dimension.Index Name = Index Constituent Dimension.Index Code` (nguồn xác nhận: `market_index_snapshot.index_nm` ← `MDDS.JAD_MARKETINFOR.INDEXNAME`, do người dùng chỉ định trực tiếp 2026-09-22). Đây là khóa RIÊNG BIỆT, không kế thừa/tương đương với khóa `Market Code = Index Code` đã dùng (rủi ro, xem ghi chú `[MỚI 2026-09-15]` ở Cụm 2) cho cột "Index Name" — khóa đó vẫn chỉ là quyết định phạm vi hẹp (LEFT JOIN lấy tên hiển thị), không giải quyết O_GSTT_3. Chiều `Market Code → Index Code` cho K_GSTT_47-52 (Nhóm 5) và K_GSTT_39 (Nhóm 35, "% thay đổi") **vẫn Open, ngoài phạm vi sửa lần này** — có thể cùng khóa `index_nm = index_code` áp dụng được, nhưng cần rà soát riêng (công thức các KPI đó lấy từ Bridge `Fact Index Constituent Snapshot` chứ không lookup trực tiếp `Fact Market Index Snapshot` như K_GSTT_35, cách áp dụng có thể khác). | Resolved một phần |
...
| O_GSTT_28 | **Nhóm 35 — KẾT LUẬN CUỐI CÙNG (sau O_GSTT_26 và O_GSTT_27, cả hai đều SUPERSEDED): Nhóm 35 tính theo MÃ CK — grain KHÔNG đổi so với O_GSTT_26, chỉ có công thức "% thay đổi" là sai thật:** Sau khi O_GSTT_27 đảo ngược O_GSTT_26 về lại "theo chỉ số" (dựa trên cách đọc sai chữ "Mã chỉ số" trong sheet Tổng hợp công thức là tên grain), người dùng bác bỏ trực tiếp và dứt khoát 2 lần liên tiếp: **"nhóm 35 theo mã ck, nhóm 5 theo rổ chỉ số"**, đề nghị đọc lại `BA_analyst_GSTT.csv`. Xác nhận lại: BA Nhóm 5 ghi Độ chi tiết = "Danh mục chỉ số - theo giờ"; BA Nhóm 35 (STT 34) ghi Độ chi tiết = **"Mã ck"** cho toàn bộ 9 dòng — đúng quy ước "Mã ck" = mã chứng khoán dùng xuyên suốt BA_analyst_GSTT.csv (không phải "mã chỉ số" như suy diễn sai ở O_GSTT_27; dòng "Mã chỉ số" trong sheet Tổng hợp công thức chỉ liệt kê GIÁ TRỊ của cột Chỉ số — VNIndex/HNXIndex/UPCoM — không phải tên grain của báo cáo). **Resolved 2026-09-19 (lần cuối)** — khôi phục lại đúng thiết kế theo mã CK của O_GSTT_26 (K_GSTT_4 dùng `Index Constituent Dimension`, K_GSTT_35 JOIN qua Bridge theo mã CK, K_GSTT_47/48/49/50/51/52 bỏ MAX/GROUP BY, bổ sung K_GSTT_1), ĐỒNG THỜI giữ lại phần sửa thật của O_GSTT_27 (đổi K_GSTT_38 → K_GSTT_39, JOIN theo cùng cơ chế mã CK) và phần sửa mart_table/mart_column độc lập. | Khôi phục thiết kế theo mã CK (O_GSTT_26) + giữ lại fix K_GSTT_38→39 (O_GSTT_27) — xem ghi chú Nhóm 35 | K_GSTT_1, 4, 35, 39, 47, 48, 49, 50, 51, 52 | **Resolved 2026-09-19 (lần cuối)** |
| O_GSTT_27 | ⚠️ **SUPERSEDED 2026-09-19 (xem O_GSTT_28) — Nhóm 35 SỬA ĐÚNG (kế thừa từ O_GSTT_26 đã đảo ngược, xem bên dưới): sai KPI "thay đổi", không phải sai grain:** Người dùng phản hồi trực tiếp 2026-09-19 bác bỏ kết luận grain của O_GSTT_26 ("Nhóm 35 tính theo mã CK") — khẳng định lại Nhóm 35 tính theo CHỈ SỐ giống hệt Nhóm 5, đồng thời cung cấp trực tiếp sheet "Tổng hợp công thức" (9 dòng, khớp đúng 9 dòng BA STT 34) để đối chiếu lại. Đối chiếu cho thấy vấn đề THẬT không phải grain mà là: dòng thứ 3 sheet ghi "% thay đổi giá = (Giá đóng cửa/Giá tham chiếu − 1)×100" — PHẦN TRĂM — trong khi thiết kế dùng `K_GSTT_38` (Index Change, tuyệt đối, theo BA_analyst_GSTT.csv Nhóm 35 dòng 508 dùng LAG). Nhóm 5 vốn đã có sẵn cặp riêng biệt Thay đổi (K_GSTT_38, BA dòng 77)/% Thay đổi (K_GSTT_39, BA dòng 78) — Nhóm 35 lẽ ra phải reuse K_GSTT_39, không phải K_GSTT_38. ⚠️ **KẾT LUẬN GRAIN Ở ĐÂY SAI** — đã bị người dùng bác bỏ trực tiếp 2 lần ngay sau đó (xem O_GSTT_28): Nhóm 35 THỰC SỰ tính theo mã CK như O_GSTT_26 kết luận ban đầu, "Mã chỉ số" trong sheet Tổng hợp công thức chỉ là giá trị của cột Chỉ số, không phải tên grain. Riêng phần sửa K_GSTT_38→K_GSTT_39 vẫn ĐÚNG và được giữ lại trong O_GSTT_28. Giữ nguyên nội dung lịch sử này không xóa, theo đúng tiền lệ O_GSTT_6/O_GSTT_10/O_GSTT_22/O_GSTT_26. ~~**Resolved 2026-09-19** — đổi K_GSTT_38 → K_GSTT_39 cho Nhóm 35; đã ĐẢO NGƯỢC toàn bộ thay đổi grain sai của O_GSTT_26 (K_GSTT_4 về lại `Market Index Dimension`, K_GSTT_35 về lại lấy trực tiếp theo chỉ số, K_GSTT_47/48/49/50/51/52 khôi phục `MAX(...) GROUP BY Index Code`, bỏ K_GSTT_1 mới thêm nhầm).~~ | ~~Đổi K_GSTT_38 → K_GSTT_39; đảo ngược grain về theo chỉ số (giống Nhóm 5); giữ lại phần sửa mart_table/mart_column~~ | K_GSTT_4, 35, 38, 39, 47, 48, 49, 50, 51, 52 | **SUPERSEDED — xem O_GSTT_28** |
| O_GSTT_26 | ⚠️ **SUPERSEDED 2026-09-19 (xem O_GSTT_27) — Nhóm 35 sai grain: copy nguyên công thức cấp CHỈ SỐ từ Nhóm 5, trong khi BA yêu cầu cấp MÃ CK:** Phát hiện 2026-09-19 khi review lần lượt các Nhóm reuse (34, 35). Thiết kế trước đây coi Nhóm 35 là "biến thể trình bày" thuần túy của Nhóm 5 (Data Explorer lưới dữ liệu thô của cùng bộ chỉ tiêu, GHI RÕ trong ghi chú cũ "ở cấp độ chỉ số thị trường, không phải cấp mã CK") — nhưng đối chiếu lại BA cho thấy: Nhóm 5 (Diễn biến chỉ số thị trường) ghi Độ chi tiết = "Danh mục chỉ số - theo giờ" (đúng cấp chỉ số), còn Nhóm 35 (Data Explorer, STT 34 riêng) ghi Độ chi tiết = **"Mã ck"** cho toàn bộ 9 dòng — 2 Nhóm dùng CHUNG nguồn dữ liệu/công thức tính giá trị nhưng khác grain hiển thị. Thiết kế cũ vô tình đảo ngược: coi grain của Nhóm 35 giống Nhóm 5, dẫn tới dùng `MAX(...) GROUP BY Index Code` (đúng cho Nhóm 5) và biến thể `Market Index Dimension` cho K_GSTT_4 (đúng cho Nhóm 5) — cả 2 đều sai cho Nhóm 35. ⚠️ **KẾT LUẬN NÀY SAI** — đã bị người dùng bác bỏ trực tiếp ngay sau đó (xem O_GSTT_27): "Mã ck" trong BA ở đây không có nghĩa là "mã chứng khoán riêng lẻ"; Nhóm 35 thực chất tính theo CHỈ SỐ giống Nhóm 5. Toàn bộ thay đổi grain mô tả bên dưới đã bị ĐẢO NGƯỢC. Giữ nguyên nội dung lịch sử này không xóa, theo đúng tiền lệ O_GSTT_6/O_GSTT_10/O_GSTT_22. ~~**Resolved 2026-09-19** — sửa lại 9 KPI theo đúng grain "1 dòng/mã CK" (bỏ GROUP BY collapse, đổi K_GSTT_4 sang biến thể Index Constituent Dimension, đổi JOIN của K_GSTT_35/38 qua Bridge theo mã CK), bổ sung K_GSTT_1 (Mã CK) làm dimension nền tảng. Nhân tiện sửa luôn lỗi độc lập: `mart_table`/`mart_column` của K_GSTT_47/48/49/51/52 trước đó trỏ nhầm `Fact Stock Portfolio Snapshot` dù `logic` đã đúng dùng cột Bridge `fct_index_constituent_snpst` — không liên quan đến bug grain nhưng phát hiện cùng lúc khi rà lại 2 cột này.~~ | ~~Đã sửa 9 KPI + bổ sung K_GSTT_1 — xem ghi chú Nhóm 35~~ | K_GSTT_1, 4, 35, 38, 47, 48, 49, 50, 51, 52 | **SUPERSEDED — xem O_GSTT_27** |
