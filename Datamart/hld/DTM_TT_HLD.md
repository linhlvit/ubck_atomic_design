...
> Atomic: `Penalty Decision X Violation Record` ← THANHTRA.PENALTY_DECISION_VIOLATION_RECORD — **READY** (`DataModel/Atomic/Event/dm_atm_penalty_decision_x_violation_record-THANHTRA.PENALTY_DECISION_VIOLATION_RECORD.yaml`, junction bắc cầu Penalty Decision + Violation Record, không có business attribute riêng)
> Atomic: `Violation Record` ← THANHTRA.VIOLATION_RECORD, `Violation Record Behavior` ← THANHTRA.VIOLATION_RECORD_BEHAVIOR — **READY** (reuse Nhóm 3)
> Ghi chú:
...
> - **Sửa 2026-08-07 (phát hiện qua `/datamart-review`, đối chiếu sâu SQL BA đầy đủ):** BA đã cập nhật SQL tham khảo — bỏ hẳn text-matching 11 pattern LIKE (`CASE WHEN LOWER(Name) LIKE '%...' THEN ... ELSE NULL END`), chuyển sang lấy trực tiếp `Violation_Behavior_Name` thật (`CASE WHEN Name IS NOT NULL THEN Name ELSE 'Khác' END`, GROUP BY động không giới hạn, ELSE 'Khác' — không loại bỏ dữ liệu không khớp). Xem O_TT_12.
> - **Sửa 2026-08-15 (đóng O_TT_16 — Atomic entity `Penalty Decision X Violation Record` đã được thiết kế):** SQL BA STT 13 thực chất có 2 nhánh xác định `Violation_Behavior_Name` bằng `CASE WHEN`: nhánh chính `PENALTY_DECISION → PENALTY_DECISION_SUBJECT → PENALTY_DECISION_SUBJECT_BEHAVIOR → VIOLATION_BEHAVIOR` (driving từ `PENALTY_DECISION`, LEFT JOIN), và nhánh phụ song song (cũng LEFT JOIN thẳng từ `PENALTY_DECISION`, độc lập nhánh chính) `PENALTY_DECISION → PENALTY_DECISION_VIOLATION_RECORD → VIOLATION_RECORD → VIOLATION_RECORD_BEHAVIOR → VIOLATION_BEHAVIOR`, dùng khi nhánh chính không xác định được tên hành vi (kể cả khi QĐ không có `PENALTY_DECISION_SUBJECT_BEHAVIOR` nào). Vì đây là 1 giá trị/QĐ (không phải theo đối tượng), **chuyển field `Violation_Behavior_Name` sang tính tại `Penalty Decision Dimension`** (grain đúng 1 QĐ, không phải `Penalty Decision Subject Behavior Dimension`) bằng CASE WHEN 2 nhánh, ELSE 'Khác'. **Fact Penalty Decision Subject Behavior đổi 3 FK phụ thuộc Subject/Subject Behavior từ INNER sang LEFT JOIN** (`Penalty_Decision_Subject_Dimension_Id`, `Penalty_Decision_Subject_Behavior_Dimension_Id` — nay nullable) để QĐ không có Subject Behavior vẫn có mặt trên Fact — trước đây bị loại hoàn toàn khỏi Fact do INNER JOIN, không chỉ sai phân loại 'Khác' như mô tả ban đầu của O_TT_16. K_TT_46 đổi COUNT từ `Penalty_Decision_Subject_Behavior_Dimension_Id` sang `Penalty_Decision_Dimension_Id` để đếm đúng theo QĐ (khớp SQL BA `COUNT DISTINCT pd.ID`), không phụ thuộc QĐ có Subject Behavior hay không. **Không ảnh hưởng Nhóm 20** — Nhóm 20 (SQL BA STT 20, dòng driving `a = PENALTY_DECISION`) không có nhánh fallback qua Violation Record, tiếp tục dùng `Violation_Behavior_Name` trên `Penalty Decision Subject Behavior Dimension` (giữ nguyên, không đổi) cho công thức "Loại hình xử lý" và `Applied_Fine_Amount` (giữ nguyên grain, NULL tự nhiên khi QĐ không có Subject Behavior — đúng bản chất dữ liệu, SQL BA Nhóm 20 cũng chỉ SUM `APPLIED_FINE_AMOUNT` không có fallback).
...
| KPI ID | Tên KPI | Đơn vị | Tính chất | Công thức | Ghi chú |
|---|---|---|---|---|---|
| K_TT_46 | Số QĐ XP theo loại hành vi | Cuộc | Base | COUNT(DISTINCT Penalty_Decision_Dimension_Id) WHERE Year(Issued_Date)=selected_year GROUP BY Penalty_Decision_Dimension.Violation_Behavior_Name | Sửa 2026-08-15 (đóng O_TT_16) — nguồn Violation_Behavior_Name chuyển sang Penalty Decision Dimension (đã CASE WHEN 2 nhánh + fallback 'Khác'); COUNT đổi sang Penalty_Decision_Dimension_Id để đếm đúng theo QĐ, không phụ thuộc QĐ có Penalty Decision Subject Behavior hay không. Tỷ lệ % tính ở tầng Báo cáo: COUNT(nhóm)/SUM(COUNT toàn bộ nhóm cùng năm) × 100%, không phải KPI Derived |
| K_TT_47 | Phân loại hành vi | — | Chiều | `Penalty Decision Dimension.Violation_Behavior_Name` — đã CASE WHEN 2 nhánh (chính + fallback qua Penalty Decision Violation Record) + 'Khác', không cần COALESCE thêm ở tầng KPI | Chiều lọc/nhóm dùng chung cho K_TT_46. Sửa 2026-08-15 — xem ghi chú K_TT_46 |
...
>
> Field mapping Atomic source: `Penalty_Decision_Subject_Behavior_Code` ← `Penalty Decision Subject Behavior.Penalty Decision Subject Behavior Code` (`PENALTY_DECISION_SUBJECT_BEHAVIOR.ID`, BK per-row unique); `Penalty_Decision_Dimension_Id` ← lookup `Penalty_Decision_Dimension` WHERE `Penalty_Decision_Code` = `Penalty Decision.Penalty Decision Code` (**Sửa 2026-08-15** — lookup trực tiếp từ `Penalty Decision` (driving, LEFT JOIN xuống `Penalty Decision Subject` → `Penalty Decision Subject Behavior`), không còn qua `Penalty Decision Subject` nữa; FK này luôn có giá trị); `Penalty_Decision_Subject_Dimension_Id` ← lookup `Penalty_Decision_Subject_Dimension` WHERE `Penalty_Decision_Subject_Code` = `Penalty Decision Subject.Penalty Decision Subject Code` (`PENALTY_DECISION_SUBJECT_BEHAVIOR.PENALTY_DECISION_SUBJECT_ID` join `PENALTY_DECISION_SUBJECT.ID`, **nullable** — NULL khi QĐ không có Penalty Decision Subject nào). Issued_Date lấy từ `Penalty Decision` trực tiếp. KPI COUNT() cho K_TT_46/47 dùng `COUNT(DISTINCT Penalty_Decision_Dimension_Id)` (xem O_TT_16, đổi từ `Penalty_Decision_Subject_Behavior_Dimension_Id` để không phụ thuộc QĐ có Subject Behavior hay không); Nhóm 20 vẫn dùng `COUNT(DISTINCT Penalty_Decision_Dimension_Id)` như cũ (không đổi). **(Thêm 2026-08-08)** `Applied_Fine_Amount` ← `Penalty Decision Subject Behavior.Applied Fine Amount` (`PENALTY_DECISION_SUBJECT_BEHAVIOR.APPLIED_FINE_AMOUNT`, direct trên driving table) — đúng grain Fact, phục vụ K_TT_72/74/76/78/80/82/84 (Nhóm 20). Đóng O_TT_13.
>
> **(Sửa 2026-08-15, đóng O_TT_16)** `Violation_Behavior_Name` chuyển từ thuộc tính trên `Penalty Decision Subject Behavior Dimension` sang thuộc tính trên `Penalty Decision Dimension` (grain đúng 1 QĐ, khớp nhu cầu K_TT_46/47). ETL logic: `LEFT JOIN Penalty Decision Subject → Penalty Decision Subject Behavior → Violation Behavior (vb, nhánh chính)`, đồng thời `LEFT JOIN Penalty Decision X Violation Record → Violation Record → Violation Record Behavior → Violation Behavior (vb2, nhánh phụ, Atomic entity mới `penalty_decision_x_violation_record`)`, rồi `CASE WHEN vb.Name IS NOT NULL THEN vb.Name WHEN vb2.Name IS NOT NULL THEN vb2.Name ELSE 'Khác' END` — khớp đúng CASE WHEN 2 nhánh trong SQL BA (STT 13). `Penalty Decision Subject Behavior Dimension.Violation_Behavior_Name` (cột cũ) giữ nguyên không đổi, tiếp tục phục vụ Nhóm 20 (không có nhánh fallback theo đúng SQL BA STT 20).
>
...
| Tên bảng | Grain | Date key | Filter mặc định |
|---|---|---|---|
| Fact Penalty Decision Subject Behavior | 1 QĐ × 1 đối tượng × 1 hành vi khi QĐ có Subject Behavior; 1 QĐ × 1 dòng (2 FK Subject/Subject Behavior NULL) khi không có — driving `PENALTY_DECISION` LEFT JOIN xuống `PENALTY_DECISION_SUBJECT` → `PENALTY_DECISION_SUBJECT_BEHAVIOR` → `VIOLATION_BEHAVIOR` (Sửa 2026-08-15, xem O_TT_16) — 4 FK: Calendar Date Dimension, Penalty Decision Subject Behavior Dimension (nullable), Penalty Decision Dimension, Penalty Decision Subject Dimension (nullable) | `Issued_Date_Dimension_Id` ← join Penalty Decision → Calendar Date Dimension | Year = selected_year (slicer NĂM 202X) |
...
| Fact Penalty Decision Subject Behavior | Event | Driving `PENALTY_DECISION`, LEFT JOIN xuống `PENALTY_DECISION_SUBJECT` → `PENALTY_DECISION_SUBJECT_BEHAVIOR` → `VIOLATION_BEHAVIOR` (Sửa 2026-08-15, xem O_TT_16) — 1 QĐ × 1 đối tượng × 1 hành vi khi có Subject Behavior, 1 dòng/QĐ (FK Subject/Subject Behavior NULL) khi không có — 4 FK: Calendar Date Dimension, Penalty Decision Subject Behavior Dimension (nullable), Penalty Decision Dimension, Penalty Decision Subject Dimension (nullable). Measure: `Applied_Fine_Amount` (Thêm 2026-08-08, đúng grain Fact, NULL khi không có Subject Behavior). Date key: Issued Date (từ Penalty Decision trực tiếp) | K_TT_46–47 (Nhóm 13), K_TT_70–84 (Nhóm 20 — reuse) | READY — xem Cụm 3b |
...
| Penalty Decision Dimension | Reference per module | Thuộc tính định danh quyết định xử phạt — `Penalty_Decision_Code` (BK, PK nguồn `THANHTRA.PENALTY_DECISION.ID`). `Total_Fine_Amount` vẫn là measure trên Fact Penalty Decision, không chuyển sang Dimension này. **(Thêm 2026-08-15, đóng O_TT_16)** `Violation_Behavior_Name` — CASE WHEN 2 nhánh (chính qua Subject Behavior, fallback qua Penalty Decision X Violation Record) + ELSE 'Khác', phục vụ K_TT_46/47 (Nhóm 13) | READY |
