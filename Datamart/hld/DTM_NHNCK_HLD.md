...
| O_NHNCK_33 | (Yêu cầu người thiết kế 2026-08-11) Atomic designer gửi trực tiếp DDL 2 bảng `atomic.uat_atm.investment_account` và `atomic.uat_atm.major_shareholder` (chưa có YAML LLD chính thức trong `DataModel/Atomic/`). So với thiết kế trước (ngoại lệ `open_investors`/`major_shareholder` cột cũ, xem O_NHNCK_6): (1) `open_investors` bị thay thế hoàn toàn bởi `investment_account` — không phải đổi tên, cấu trúc cột khác hẳn; (2) `major_shareholder` giữ tên bảng nhưng đổi toàn bộ tên cột sang chuẩn Atomic tiếng Anh (`ma_ck`→`securities_code`, `so_luong_ck_cuoi_ky`→`closing_security_quantity`); (3) khóa join CCCD đổi từ `so_dang_ky_so_huu` (VSDC-specific) sang `identification_nbr` (chuẩn hóa, cùng pattern `Involved Party Alternative Identification.identification_nbr` đã dùng ở K_NHNCK_85/O_NHNCK_6) trên cả 2 bảng. K_NHNCK_87/88 (Account Number/Holder Name) đổi nguồn sang `investment_account.investment_account_nbr`/`investor_nm`. K_NHNCK_89/103 (Main Held Securities Code/VSDC Held Securities Volume) đổi nguồn sang `major_shareholder.securities_code`/`closing_security_quantity`. **Vấn đề chưa xử lý xong:** filter "chỉ tài khoản cá nhân" trước đây dùng `open_investors.loai_hinh = 'cá nhân'` (giá trị rõ ràng); DDL mới không có cột `loai_hinh` — chỉ có `investment_account_tp_code` (INT, mã phân loại). Người thiết kế xác nhận vẫn cần giữ logic lọc "chỉ cá nhân" theo đúng yêu cầu BA gốc, nhưng giá trị `investment_account_tp_code` nào tương ứng "cá nhân" **chưa được cung cấp** — BA hiện chỉ có giá trị mã filter cá nhân của 1 nguồn khác không liên quan (`Trading_Hose/Trading_HNX.Buy_Client/House Classification Code = '10'`), không áp dụng được cho cột mới này. | Đã cập nhật nguồn/tên cột tại `datamart_attributes.csv`, LLD CSV NHNCK, `DTM_NHNCK_Detail_Mapping.csv`, HLD (ghi chú đầu Nhóm 7, bảng KPI, ghi chú schema, bảng tổng hợp trạng thái). ETL logic filter cá nhân để placeholder `investment_account_tp_code = <TBD_cá_nhân>` — chờ BA/Atomic xác nhận giá trị mã cụ thể trước khi triển khai ETL thật. | K_NHNCK_87, K_NHNCK_88, K_NHNCK_89, K_NHNCK_103 | Open (chờ xác nhận giá trị `investment_account_tp_code` = cá nhân) |
