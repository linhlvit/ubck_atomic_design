...
Phục vụ **Tab TRA CỨU HỒ SƠ 360°** — Nhóm 7 (Vai trò tại DN niêm yết, Tài khoản cross-broker PENDING).

> **(Sửa 2026-08-12)** Đổi nguồn từ `NHNCK.OrganizationReports` sang chuỗi SCMS — xem O_NHNCK_34. Chuỗi join qua CCCD (`Involved Party Alternative Identification`) trực tiếp tới `Securities Company Shareholder`, không qua `Securities Company Insider Related Person` — `sc_shareholder.sc_id` đã là FK trực tiếp tới `Securities Company`, không cần entity trung gian.

```mermaid
flowchart LR
    subgraph SRC["Source NHNCK/SCMS"]
        S1["NHNCK.PROFESSIONALS"]
        S2["SCMS.SC_FIRM_SHAREHOLDER"]
    ...
    subgraph SIL["Atomic"]
        ...
        SV4["Involved Party Alternative Identification"]
    end
...
##### Cụm 11: Data Explorer — Tra cứu danh sách NHN (`Operational Practitioner Data Explorer`)

Phục vụ **Tab DATA EXPLORER** — Nhóm 13 (bảng tra cứu flat toàn bộ NHN theo filter Loại chứng chỉ và Trạng thái, bao gồm cả NHN chưa có CCHN nào). Lấy trực tiếp từ Atomic, không khai thác qua Fact/Dim. Driving: `Securities Practitioner` (toàn bộ NHN) — LEFT JOIN `Securities Practitioner License Certificate Document` (fan-out N CCHN/NHN, NULL nếu chưa có CCHN).
...
> **(Sửa 2026-08)** `Operational Practitioner 360 Profile` driving từ `Securities Practitioner` — toàn bộ NHN, kể cả chưa từng có CCHN (dùng chung cho Nhóm 13 Data Explorer cần lấy đủ, xem O_NHNCK_31). Riêng **Dashboard 360° (Nhóm 5) chỉ hiển thị NHN có CCHN** — mockup danh sách/header đều giả định có Loại CCHN/Số CCHN/Trạng thái, không có UI cho case chưa-có-CCHN. Bắt buộc filter `WHERE Active_Certificate_Number IS NOT NULL` tại bước Chỉ tiêu báo cáo khi khai thác Nhóm 5 — Data Explorer (Nhóm 13) KHÔNG áp filter này (giữ toàn bộ NHN theo đúng mục đích BA).
...
flowchart LR
    ...
    G1 -- "filter Active_Certificate_Number IS NOT NULL" --> R1
```
...
| Tên bảng | Grain |
|---|---|
| `Operational Practitioner 360 Profile` | 1 NHN (latest state) — (Sửa 2026-08) bảng gốc chứa toàn bộ NHN; Nhóm 5 filter `Active_Certificate_Number IS NOT NULL` tại query time để chỉ hiển thị NHN có CCHN |
...
| K_NHNCK_81 | Tên DN niêm yết (NHN tham gia) | Text | Base | (Sửa 2026-08-12) `Securities Company`.Securities Company Short Name | Reuse từ Nhóm 7 — đổi nguồn sang SCMS, xem O_NHNCK_34 |
| K_NHNCK_82 | Vai trò NHN tại DN niêm yết | Text | Base | (Sửa 2026-08-12) `Securities Company Shareholder`.Representative Position | Reuse từ Nhóm 7 — xem O_NHNCK_34 |
...
> Atomic chính (K_NHNCK_81/82/84 — Sửa 2026-08-12): `Securities Company` (`securities_company`) ← SCMS.SC_FIRM_INFO — **READY**; `Securities Company Shareholder` (`sc_shareholder`) ← SCMS.SC_FIRM_SHAREHOLDER — **READY** (`sc_shareholder.sc_id` là FK trực tiếp tới `securities_company`, không cần entity trung gian). Cầu nối CCCD: `Involved Party Alternative Identification` (`ip_alternative_identification`, nguồn NHNCK.PROFESSIONALS và SCMS.SC_FIRM_SHAREHOLDER) — **READY**.
...
> Atomic phụ (K_NHNCK_87/88/89/103 — Sửa 2026-08-11): `investment_account` và `major_shareholder` — cấu trúc bảng (DDL `atomic.uat_atm.investment_account`, `atomic.uat_atm.major_shareholder`) do Atomic designer gửi trực tiếp, chưa có file YAML LLD chính thức trong `DataModel/Atomic/`. Khác ngoại lệ "coi tên BA ghi là Atomic thật" trước đây (`open_investors`) — đây là cấu trúc Atomic thật do người thiết kế Atomic xác nhận, chỉ chờ hoàn thiện YAML LLD. `open_investors` (VSDC/MSS placeholder cũ) đã được thay thế hoàn toàn bởi `investment_account`; `major_shareholder` giữ tên bảng nhưng đổi toàn bộ tên cột sang chuẩn Atomic (xem ghi chú schema bên dưới). Xem O_NHNCK_33.
> Ghi chú: Panel "Vai trò tại DN niêm yết" và panel "Mạng lưới người liên quan" là 2 bảng tác nghiệp độc lập. **(Sửa 2026-08-12, K_NHNCK_81/82/84)** BA đổi nguồn từ `Securities Practitioner Organization Employment Report` (NHNCK.ORGANIZATION_REPORTS) sang chuỗi SCMS: nối CCCD qua `ip_alternative_identification` trực tiếp tới `sc_shareholder`, rồi lấy `securities_company` qua FK có sẵn `sc_shareholder.sc_id`. SQL BA tham khảo đi qua `SC_FIRM_INSIDER_RELATION` trước rồi mới join `SC_FIRM_INFO_ID` sang `SC_FIRM_SHAREHOLDER` (qua `sc_id`/CTCK) — pattern này (a) đúng là fan-out đã bị bác bỏ ngày 2026-07-20 cho K_NHNCK_85 (join qua CTCK không xác định đúng người), và (b) hoàn toàn không cần thiết vì `sc_shareholder.sc_id` đã là FK trực tiếp tới `securities_company` — không cần qua `sc_insider_related_person` để lấy thông tin công ty. Người thiết kế xác nhận sửa lại: nối `securities_practitioner` ↔ `sc_shareholder` qua CCCD (`ip_alternative_identification.identification_nbr`, 2 hop — nguồn NHNCK rồi nguồn SCMS.SC_FIRM_SHAREHOLDER), filter `sc_shareholder.individual_ind = 1` (tương ứng `s.INDIVIDUAL='1'` trong SQL BA), rồi lấy `securities_company` trực tiếp qua `sc_shareholder.sc_id` filter `listed_ind=1` — nhất quán hoàn toàn với chuỗi join K_NHNCK_85 đã dùng, bỏ hẳn `sc_insider_related_person` khỏi thiết kế Nhóm 7. **(Sửa 2026-08-12, lần 2)** BA cập nhật tiếp BA_analyst_NHNCK.csv, bỏ hẳn điều kiện `AND i.LISTING_EXCHANGE = 'UPCOM'` khỏi SQL tham khảo — phạm vi mở rộng từ "chỉ DN niêm yết sàn UPCOM" sang "mọi DN niêm yết" (HOSE/HNX/UPCOM), chỉ còn filter `listed_ind = 1`. Đổi tên KPI bỏ chữ "UPCOM" cho khớp phạm vi mới: K_NHNCK_81 "Tên DN niêm yết/UPCOM" → "Tên DN niêm yết"; K_NHNCK_82 (dòng reuse Nhóm 6) "Vai trò NHN tại DN niêm yết/UPCOM" → "Vai trò NHN tại DN niêm yết". Xem O_NHNCK_34. K_NHNCK_85 "Số lượng cổ phiếu sở hữu" đã xác nhận nguồn thật là SCMS (không phải VSDC) — chuyển READY, xem O_NHNCK_6. Sửa tên attribute: `Workplace Name` → `Practitioner Workplace At Report`; `Position Name` → `Practitioner Position At Report` (theo YAML `sp_organization_employment_report`) — **chỉ còn áp dụng cho Nhóm 8 (Quá trình hành nghề)**, không còn dùng cho Nhóm 7 sau khi đổi nguồn. **(Sửa 2026-08-07)** Panel "Tài khoản & Số dư" (K_NHNCK_87/88/89/103) chuyển từ PENDING sang READY — BA đã bổ sung tên bảng/cột nguồn cụ thể. **(Sửa 2026-08-11)** Cập nhật lại theo DDL Atomic thật: `investment_account` thay `open_investors`; `major_shareholder` đổi tên cột (`ma_ck`→`securities_code`, `so_luong_ck_cuoi_ky`→`closing_security_quantity`); khóa join CCCD đổi từ `so_dang_ky_so_huu` sang `identification_nbr` trên cả 2 bảng. Filter "chỉ tài khoản cá nhân" nay qua `investment_account_tp_code IN ('3','4')` — xem O_NHNCK_33.

**Bảng KPI:**

| KPI ID | Tên KPI | Tính chất | Trạng thái | Nguồn | Ghi chú |
|---|---|---|---|---|---|
| K_NHNCK_81 | Tên DN niêm yết | Base | READY | (Sửa 2026-08-12, lần 2) `Securities Company`.Securities Company Short Name — join trực tiếp qua FK `Securities Company Shareholder.Securities Company Id`, filter `Listed Indicator = 1` (BA bỏ điều kiện Listing Exchange Code = 'UPCOM' — mở rộng sang mọi sàn niêm yết) | Khai sinh tại Nhóm 7 — đổi nguồn từ ORGANIZATION_REPORTS sang SCMS, xem O_NHNCK_34 |
| K_NHNCK_82 | Vai trò tại DN | Base | READY | (Sửa 2026-08-12) `Securities Company Shareholder`.Representative Position | Khai sinh tại Nhóm 7 — xem O_NHNCK_34 |
| K_NHNCK_83 | Trạng thái vai trò | Base | READY | (Sửa 2026-08-12) `Securities Practitioner`.Practice Status Code (scheme PRACTITIONER_PRACTICE_STATUS: 0=Chưa HN/1=Đang HN/2=Thu hồi có cấp lại/3=Thu hồi không cấp lại/4=Cấp có thời hạn) | Đổi nguồn: BA ghi `Professional_Histories.status_work` join `PROFESSIONALS` qua `professional_id` — Atomic đã denormalize STATUS_WORK lên `Securities Practitioner.Practice Status Code` (snapshot hiện tại), `sp_reason_change_history` (Atomic của PROFESSIONAL_HISTORIES) không lưu field này theo thiết kế. Xem O_NHNCK_34 |
| K_NHNCK_84 | Mã CTCK | Base | READY | (Sửa 2026-08-12) `Securities Company`.Securities Company Code | Khai sinh tại Nhóm 7 — đổi nguồn từ Organization Reference sang `Securities Company` (SCMS), xem O_NHNCK_34 |
...
| K_NHNCK_87 | Số tài khoản | Base | READY | `investment_account`.`investment_account_nbr` | (Sửa 2026-08-11) Đổi nguồn từ `open_investors.ma_tkgd` sang `investment_account.investment_account_nbr` theo DDL Atomic mới — xem O_NHNCK_33 |
| K_NHNCK_88 | Tên chủ tài khoản | Base | READY | `investment_account`.`investor_nm` | (Sửa 2026-08-11) Đổi nguồn từ `open_investors.ten_khach_hang` sang `investment_account.investor_nm` — xem ghi chú K_NHNCK_87 |
| K_NHNCK_89 | Mã CK nắm giữ chính | Base | READY | `major_shareholder`.`securities_code` | (Sửa 2026-08-11) Đổi tên cột từ `ma_ck` sang `securities_code` theo DDL Atomic mới — xem ghi chú K_NHNCK_87 |
| K_NHNCK_103 | Số lượng chứng khoán VSDC sở hữu (Cuối kỳ) | Base | READY | `major_shareholder`.`closing_security_quantity` | (Thêm 2026-07-20) BA row 60, cùng nguồn `VSDC_BM 8` với K_NHNCK_89 — BA quên điền tên KPI ở cột "Thông tin", đã bổ sung theo "Trường nguồn". (Sửa 2026-08-11) Đổi tên cột từ `so_luong_ck_cuoi_ky` sang `closing_security_quantity` — xem ghi chú K_NHNCK_87 |
...
erDiagram
    Operational_Practitioner_Listed_Company_Role {
        varchar Practitioner_Code PK
        varchar Securities_Company_Code PK
        varchar Securities_Company_Short_Name
...
> **Ghi chú schema Nhóm 7 (Sửa 2026-08-12):** Đổi toàn bộ nguồn K_NHNCK_81/82/83/84/85 từ `Securities Practitioner Organization Employment Report` (NHNCK.ORGANIZATION_REPORTS) sang chuỗi SCMS — xem O_NHNCK_34. Chuỗi join dùng chung cho mọi KPI của Cụm này: `Securities Practitioner` → CCCD (`Involved Party Alternative Identification.identification_nbr`, nguồn NHNCK.PROFESSIONALS) → CCCD lần 2 (nguồn SCMS.SC_FIRM_SHAREHOLDER) → `Securities Company Shareholder` (filter `Individual Indicator = 1`) → `Securities Company` **trực tiếp qua FK có sẵn `Securities Company Shareholder.Securities Company Id`** (lọc `Listed Indicator=1`). **(Sửa 2026-08-12, lần 2)** BA bỏ điều kiện `Listing Exchange Code='UPCOM'` — phạm vi mở rộng sang mọi DN niêm yết (HOSE/HNX/UPCOM), không còn giới hạn riêng sàn UPCOM. Đổi tên K_NHNCK_81 bỏ chữ "UPCOM". **Không dùng `Securities Company Insider Related Person`** — ban đầu có cân nhắc dùng entity này để bắc cầu sang `Securities Company` (theo đúng SQL BA tham khảo, join `SC_FIRM_INSIDER_RELATION` trước), nhưng xác nhận lại: (1) `sc_shareholder.sc_id` đã là FK trực tiếp tới `securities_company`, không cần entity trung gian; (2) SQL BA tham khảo join `sc_id⟷sc_id` giữa `sc_insider_related_person`/`sc_shareholder` là đúng pattern fan-out đã bị bác bỏ ngày 2026-07-20 cho K_NHNCK_85 (join qua CTCK không xác định đúng người). Loại hẳn entity này khỏi thiết kế Cụm 5/Nhóm 7. `Securities_Company_Short_Name` (K_NHNCK_81) + `Securities_Company_Code` (K_NHNCK_84, đồng thời là BK phần 2) lấy từ `Securities Company` ở cuối chuỗi. `Representative_Position` (K_NHNCK_82) và `Shares_Held` (K_NHNCK_85) lấy trực tiếp từ `Securities Company Shareholder` (không cần join thêm `Securities Company`). `Practice_Status_Code` (K_NHNCK_83) ETL từ `Securities Practitioner.Practice Status Code` (NHNCK.PROFESSIONALS, denormalize STATUS_WORK — không lấy từ `sp_reason_change_history`/PROFESSIONAL_HISTORIES vì Atomic không lưu field này ở đó), không cần JOIN thêm vì đã có sẵn trên driving table. NULL ở `Shares_Held` nếu NHN không phải cổ đông của DN niêm yết đang xét. **(Sửa 2026-08-11)** `Account_Number`/`Account_Holder_Name` (K_NHNCK_87/88) direct từ `investment_account` (`investment_account_nbr`/`investor_nm`), join `investment_account.identification_nbr = practitioner_identity_nbr_at_rpt` filter `investment_account_tp_code IN ('3','4')` (Sửa 2026-08-12, xem O_NHNCK_33); `Main_Held_Securities_Code`/`Vsdc_Held_Securities_Volume` (K_NHNCK_89/103) direct từ `major_shareholder` (`securities_code`/`closing_security_quantity`), join `major_shareholder.identification_nbr = investment_account.identification_nbr`. DDL 2 bảng này do Atomic designer gửi trực tiếp (`atomic.uat_atm.investment_account`, `atomic.uat_atm.major_shareholder`), thay thế hoàn toàn `open_investors`/`major_shareholder` (cột cũ) đã dùng trước đây — xem O_NHNCK_33. Chưa có YAML LLD chính thức nên chưa qua `join_atomic` kiểm chứng FK theo quy trình chuẩn.
...
|---|---|---|
| `Operational Practitioner Listed Company Role` | (Sửa 2026-08-12, lần 2) 1 DN niêm yết per NHN cổ đông cá nhân (join CCCD qua `Involved Party Alternative Identification` tới `sc_shareholder`, filter `Individual_Indicator=1`, rồi `securities_company` qua FK `sc_shareholder.sc_id` filter `Listed_Indicator=1` — BA bỏ điều kiện Listing Exchange Code='UPCOM', mở rộng sang mọi sàn niêm yết) | new |
...
> **(Sửa 2026-08 — BA cần lần tất cả thông tin NHN, kể cả chưa có CCHN):** Driving table đổi từ `Securities Practitioner License Certificate Document` sang `Securities Practitioner` (toàn bộ NHN, kể cả chưa từng có bản ghi CCHN nào) — LEFT JOIN sang Certificate Document, không giới hạn số dòng (fan-out N CCHN/NHN). Grain đổi thành **1 NHN × 1 CCHN, bao gồm cả NHN chưa có CCHN nào** (cột Certificate NULL). PK `License_Certificate_Document_Code` chấp nhận NULL — giữ khai báo `key=PK` mang tính documentation/logical, không enforce constraint vật lý (nhất quán với cách Delta Lake hiện tại không enforce PK constraint). `Practice_Status_Code`/`Full_Name`/`Source_System_Code` chuyển từ JOIN gián tiếp qua Certificate Document sang lấy trực tiếp từ driving table `Securities Practitioner`.
...
| K_NHNCK_69 | Số CCHN | Text | Base | `Securities Practitioner License Certificate Document`.Certificate Number | (Sửa 2026-08) LEFT JOIN — NULL nếu NHN chưa có CCHN nào |
| K_NHNCK_70 | Loại hình hành nghề | Text | Base | `Securities Practitioner License Certificate Document`.Certificate Type Code — ETL denormalize Certificate Type Name khi populate bảng | (Sửa 2026-08) LEFT JOIN — NULL nếu NHN chưa có CCHN nào |
...
| K_NHNCK_72 | Ngày cấp CCHN | Date | Base | `Securities Practitioner License Certificate Document`.Certificate Issue Date | (Sửa 2026-08) LEFT JOIN — NULL nếu NHN chưa có CCHN nào |
| K_NHNCK_73 | Trạng thái NHN | Text | Base | `Securities Practitioner`.Practice Status Code (`securities_practitioner_dim.practice_status_code`) ← `PROFESSIONALS.STATUS_WORK` (scheme: 0=Chưa hành nghề, 1=Đang hành nghề, 2=Thu hồi có cấp lại, 3=Thu hồi không cấp lại) | (Sửa 2026-08) Đổi nguồn sang lấy trực tiếp từ driving table `Securities Practitioner` (trước đây join gián tiếp qua Certificate Document) |
...
erDiagram
    Operational_Practitioner_Data_Explorer {
        varchar Practitioner_Code PK
        varchar License_Certificate_Document_Code "PK, nullable - documentation-only"
        varchar Full_Name
        varchar Certificate_Number
        varchar Certificate_Type_Code
        varchar Certificate_Type_Name
...
> **Ghi chú schema:** (Sửa 2026-08) Grain = 1 NHN × 1 CCHN, bao gồm cả NHN chưa có CCHN nào — Driving `Securities Practitioner`, LEFT JOIN `Securities Practitioner License Certificate Document` không giới hạn (fan-out N CCHN/NHN). `License_Certificate_Document_Code`, `Certificate_Number`, `Certificate_Type_Code/Name`, `Certificate_Issue_Date` đều NULL khi NHN chưa có CCHN nào. Slicer "Loại hình" filter trên `Certificate_Type_Code`; slicer "Trạng thái" filter trên `Practice_Status_Code` — cả hai filter tại query time, không pre-filter khi ETL populate. `Practice_Status_Code`/`Full_Name`/`Source_System_Code` lấy trực tiếp từ driving table `Securities Practitioner` (trước 2026-08 join gián tiếp qua Certificate Document). `Practice_Status_Code` phục vụ đồng thời K_NHNCK_73 (hiển thị) và slicer Trạng thái (filter) — 1 cột duy nhất, không tạo cột riêng cho slicer. `Practice_Status_Name` (K_NHNCK_109, Sửa 2026-07-22) ETL-denormalized từ Classification (scheme PRACTITIONER_PRACTICE_STATUS) khi populate — cặp Code/Name cho cùng 1 cột, không phải slicer riêng. `Certificate_Type_Name` ETL-denormalized từ Classification khi populate — presentation không join ở query time. `Current_Organization_Name` ETL-derived từ `Organization Employment Report` mới nhất có Termination Date = NULL. `Identification_Type_Code` (K_NHNCK_101, Sửa 2026-08) lấy từ `ip_alternative_identification.identification_tp_code` join qua `sp_id` — scheme IP_ALT_ID_TYPE giá trị string thật (NATIONAL_ID/CITIZEN_ID/PASSPORT), xem O_NHNCK_25.
...
| Tên bảng | Grain |
|---|---|
| `Operational Practitioner Data Explorer` | (Sửa 2026-08) 1 NHN × 1 CCHN — bao gồm cả NHN chưa có CCHN nào, toàn bộ trạng thái (slicer filter tại query time) |

> Grain hiển thị thực tế tại bước Chỉ tiêu báo cáo có thể lớn hơn 1 NHN×1 CCHN khi JOIN đồng thời Exam History và Violation History (N đợt thi × M vi phạm) — xem ghi chú JOIN ở đầu Nhóm 13.
...
| `Operational Practitioner Listed Company Role` | 1 vai trò per NHN per DN niêm yết | K_NHNCK_81–85 READY; K_NHNCK_87–89, K_NHNCK_103 READY (Sửa 2026-08-11, nguồn `investment_account`/`major_shareholder` theo DDL Atomic mới — xem O_NHNCK_33) | READY |
| `Operational Practitioner Data Explorer` | (Sửa 2026-08) 1 NHN × 1 CCHN — bao gồm cả NHN chưa có CCHN nào (slicer filter tại query time) | K_NHNCK_68–74 READY; K_NHNCK_101 (Mã định danh) READY | READY |
...
| O_NHNCK_31 | (Yêu cầu người thiết kế 2026-08) BA cần Data Explorer lần được **tất cả** thông tin NHN, kể cả NHN chưa có CCHN — `Operational Practitioner Data Explorer` trước đây driving từ `Securities Practitioner License Certificate Document` (NHNCK.CERTIFICATE_RECORDS), nên NHN chưa từng có bản ghi CCHN nào bị loại hoàn toàn khỏi bảng (driving table không có dòng gốc cho họ). Người thiết kế xác nhận: (1) đổi driving table sang `Securities Practitioner` (toàn bộ NHN, giống cách `Operational Practitioner 360 Profile` — Nhóm 5 — đã làm từ đầu), LEFT JOIN `Securities Practitioner License Certificate Document` không giới hạn số dòng (giữ fan-out N CCHN/NHN theo yêu cầu BA lần đủ thông tin, không rút gọn về 1 dòng/NHN); (2) `License_Certificate_Document_Code` (PK phần 2) chấp nhận NULL khi NHN chưa có CCHN — giữ khai báo `key=PK` mang tính documentation/logical, không enforce constraint vật lý (Delta Lake hiện tại cũng không enforce PK constraint). Đồng thời phát hiện `mapping_dtm_practitioner_data_explorer.yaml` (Mapping/data_mart/NHNCK/) từ trước không khớp cột với LLD CSV chuẩn (mapping có `Certificate Status Code/Name` scheme CERTIFICATE_STATUS không tồn tại ở LLD CSV; LLD CSV có `Practice Status Code/Name` + `Identification Type Code` không có ở mapping) — đồng bộ luôn trong lần sửa này, không tách issue riêng. | Đổi driving table + LEFT JOIN tại: `mapping_dtm_practitioner_data_explorer.yaml` (viết lại toàn bộ, đồng bộ đủ cột với LLD CSV), LLD CSV (đổi tên file `..._NHNCK_CERTIFICATE_RECORDS.csv` → `..._NHNCK_PROFESSIONALS.csv` theo convention driving table mới, viết lại nội dung), `datamart_attributes.csv` (12 dòng), `datamart_model.yaml` (description, source_atomic order, PK/description từng cột, `src_stm_code` source đổi sang `securities_practitioner`), HLD (Cụm 11 mô tả + mermaid note, Nhóm 13 ghi chú grain/KPI K_NHNCK_69/70/72/73/erDiagram/ghi chú schema/bảng grain/bảng tổng hợp status). | K_NHNCK_68–74, K_NHNCK_101, K_NHNCK_109 | Closed |
| O_NHNCK_32 | (Phát hiện ngay sau O_NHNCK_31, người thiết kế tự soát) `Operational Practitioner 360 Profile` (Nhóm 5) driving từ `Securities Practitioner` — toàn bộ NHN kể cả chưa có CCHN — vốn đã đúng cho mục đích Nhóm 13 (Data Explorer JOIN sang lấy field 1-1), nhưng **sai khi dùng riêng cho Dashboard 360° (Nhóm 5)**: mockup danh sách + header (dòng 940-951) đều giả định NHN có Loại CCHN/Số CCHN/Trạng thái hiển thị — không có UI cho case chưa-có-CCHN; bảng KPI K_NHNCK_33-41 không có điều kiện lọc nào loại NHN chưa có CCHN. Bản chất "Hồ sơ hành nghề 360°" chỉ có ý nghĩa với NHN đã/đang có CCHN. Người thiết kế xác nhận: KHÔNG thêm cột indicator mới (Has_Certificate_Indicator) — tận dụng cột đã có sẵn `Active_Certificate_Number` (lấy theo bản ghi Issue Date mới nhất, đã tồn tại từ trước tại Nhóm 5), filter `IS NOT NULL` tại bước Chỉ tiêu báo cáo. Data Explorer (Nhóm 13) KHÔNG áp filter này — giữ đúng mục đích BA (lần được tất cả NHN). | Thêm ghi chú filter bắt buộc `Active_Certificate_Number IS NOT NULL` vào: HLD Nhóm 5 (Source note + Lineage flowchart edge label + Bảng grain), `DTM_NHNCK_Detail_Mapping.csv` (1 dòng FILTER mới gắn K_NHNCK_33). Không đổi schema `opr_practitioner_360_profile`/LLD/datamart_model.yaml — bảng gốc giữ nguyên grain 1 NHN (toàn bộ), chỉ thêm điều kiện filter ở tầng khai thác báo cáo. | K_NHNCK_33–41 | Closed |
| O_NHNCK_33 | (Yêu cầu người thiết kế 2026-08-11) Atomic designer gửi trực tiếp DDL 2 bảng `atomic.uat_atm.investment_account` và `atomic.uat_atm.major_shareholder` (chưa có YAML LLD chính thức trong `DataModel/Atomic/`). So với thiết kế trước (ngoại lệ `open_investors`/`major_shareholder` cột cũ, xem O_NHNCK_6): (1) `open_investors` bị thay thế hoàn toàn bởi `investment_account` — không phải đổi tên, cấu trúc cột khác hẳn; (2) `major_shareholder` giữ tên bảng nhưng đổi toàn bộ tên cột sang chuẩn Atomic tiếng Anh (`ma_ck`→`securities_code`, `so_luong_ck_cuoi_ky`→`closing_security_quantity`); (3) khóa join CCCD đổi từ `so_dang_ky_so_huu` (VSDC-specific) sang `identification_nbr` (chuẩn hóa, cùng pattern `Involved Party Alternative Identification.identification_nbr` đã dùng ở K_NHNCK_85/O_NHNCK_6) trên cả 2 bảng. K_NHNCK_87/88 (Account Number/Holder Name) đổi nguồn sang `investment_account.investment_account_nbr`/`investor_nm`. K_NHNCK_89/103 (Main Held Securities Code/VSDC Held Securities Volume) đổi nguồn sang `major_shareholder.securities_code`/`closing_security_quantity`. **Vấn đề chưa xử lý xong (lúc mở):** filter "chỉ tài khoản cá nhân" trước đây dùng `open_investors.loai_hinh = 'cá nhân'` (giá trị rõ ràng); DDL mới không có cột `loai_hinh` — chỉ có `investment_account_tp_code` (INT, mã phân loại). Người thiết kế xác nhận vẫn cần giữ logic lọc "chỉ cá nhân" theo đúng yêu cầu BA gốc, nhưng giá trị `investment_account_tp_code` nào tương ứng "cá nhân" **chưa được cung cấp** — BA hiện chỉ có giá trị mã filter cá nhân của 1 nguồn khác không liên quan (`Trading_Hose/Trading_HNX.Buy_Client/House Classification Code = '10'`), không áp dụng được cho cột mới này. **(Sửa 2026-08-12)** Người thiết kế xác nhận: dữ liệu VSDC lọc `loai_hinh IN ('3','4')` thay cho giá trị 'cá nhân' — áp dụng trực tiếp thành `investment_account.investment_account_tp_code IN ('3','4')` trên DDL Atomic mới. | Đã cập nhật nguồn/tên cột tại `datamart_attributes.csv`, LLD CSV NHNCK, `DTM_NHNCK_Detail_Mapping.csv`, HLD (ghi chú đầu Nhóm 7, bảng KPI, ghi chú schema, bảng tổng hợp trạng thái). **(Sửa 2026-08-12)** Thay placeholder `<TBD_cá_nhân>` bằng `investment_account_tp_code IN ('3','4')` tại 4 dòng `datamart_attributes.csv` + 4 dòng LLD CSV NHNCK (`opr_practitioner_list_company_role`) + ghi chú HLD (đầu Nhóm 7, ghi chú schema) + Detail_Mapping K_NHNCK_87 (đồng thời sửa tham chiếu sai `O_NHNCK_7` → `O_NHNCK_33`). | K_NHNCK_87, K_NHNCK_88, K_NHNCK_89, K_NHNCK_103 | Closed |
| O_NHNCK_34 | (Phát hiện qua `/datamart-review NHNCK nhóm 7`, 2026-08-12) BA đổi nguồn K_NHNCK_81/82/84 ("Tên DN niêm yết/UPCOM"/"Vai trò tại DN"/"Mã CTCK") từ `Securities Practitioner Organization Employment Report` (NHNCK.ORGANIZATION_REPORTS) sang chuỗi SCMS, theo đúng SQL BA tham khảo mới (dòng 49/50/57 BA_analyst_NHNCK.csv). Phát hiện và xử lý 3 vấn đề trong quá trình thiết kế lại: (1) SQL BA tham khảo join `sc_insider_related_person.sc_id = sc_shareholder.sc_id` (qua CTCK) — đúng pattern fan-out (N insider × N shareholder cùng công ty, không xác định đúng người) đã bị bác bỏ ngày 2026-07-20 cho K_NHNCK_85. (2) Sau khi xác nhận cấu trúc `sc_shareholder` (đã có sẵn FK trực tiếp `sc_id` tới `securities_company`, giống hệt cách SQL BA join `SC_FIRM_INFO_ID` sang `SC_FIRM_SHAREHOLDER`), người thiết kế xác nhận **bỏ hẳn `sc_insider_related_person` khỏi chuỗi join** — không cần entity trung gian: nối `securities_practitioner` → CCCD (`ip_alternative_identification`, 2 hop: NHNCK rồi SCMS.SC_FIRM_SHAREHOLDER) → `sc_shareholder` (filter `individual_ind=1`, tương ứng `s.INDIVIDUAL='1'` SQL BA) → `securities_company` trực tiếp qua `sc_shareholder.sc_id` (filter `listed_ind=1 AND listing_exchange_code='UPCOM'`) — nhất quán hoàn toàn với chuỗi K_NHNCK_85 đã dùng. (3) K_NHNCK_83 "Trạng thái vai trò" — BA ghi nguồn `Professional_Histories.status_work` join `PROFESSIONALS` qua `professional_id`; tra Atomic cho thấy `sp_reason_change_history` (Atomic của PROFESSIONAL_HISTORIES) đã cố tình bỏ `STATUS_WORK` khỏi thiết kế (ghi chú `pending_design.yaml`: "PROFESSIONAL_HISTORIES chỉ lưu grain sự kiện thay đổi, dữ liệu master hiện tại xem entity Securities Practitioner") — cột này đã được denormalize sẵn lên `Securities Practitioner.Practice Status Code` (NHNCK.PROFESSIONALS), giá trị scheme PRACTITIONER_PRACTICE_STATUS (0-4) khớp đúng case-when trong SQL BA — không phải Gap Atomic, chỉ cần đổi entity tra cứu. **(Cập nhật 2026-08-12, lần 2)** BA cập nhật tiếp BA_analyst_NHNCK.csv — bỏ hẳn điều kiện `AND i.LISTING_EXCHANGE = 'UPCOM'` khỏi SQL tham khảo K_NHNCK_81, chỉ còn `i.LISTED = '1'`. Phạm vi dữ liệu mở rộng từ "chỉ DN niêm yết sàn UPCOM" sang "mọi DN niêm yết" (HOSE/HNX/UPCOM). Đổi tên K_NHNCK_81 "Tên DN niêm yết/UPCOM" → "Tên DN niêm yết" (cả bản khai sinh Nhóm 7 và bản reuse Nhóm 6); K_NHNCK_82 (bản reuse Nhóm 6) "Vai trò NHN tại DN niêm yết/UPCOM" → "Vai trò NHN tại DN niêm yết" — phản ánh đúng phạm vi dữ liệu mới, không còn giới hạn riêng UPCOM. | Cập nhật HLD: ghi chú đầu Nhóm 7, bảng KPI (K_NHNCK_81/82/83/84), erDiagram, Cụm 5 flowchart, ghi chú schema, Bảng grain (loại `sc_insider_related_person` khỏi thiết kế). Attributes LLD (`Datamart/lld/NHNCK/DTM_NHNCK_opr_practitioner_list_company_role_NHNCK_PROFESSIONALS.csv`, đổi tên file từ `..._NHNCK_ORGANIZATION_REPORTS.csv`) đã cập nhật theo chuỗi join cuối cùng (không qua insider), merge vào `datamart_attributes.csv`. Detail Mapping (K_NHNCK_81/82/83/84) và `datamart_model.yaml` đã cập nhật theo nguồn mới. **(Lần 2)** Bỏ điều kiện `listing_exchange_code='UPCOM'` khỏi etl_logic Attributes (2 dòng `Securities Company Code`/`Securities Company Short Name`), master `datamart_attributes.csv`, `datamart_model.yaml` (description entity), Detail Mapping (đổi tên KPI K_NHNCK_81/82), flat table SQL comment nếu có nhắc UPCOM. | K_NHNCK_81, K_NHNCK_82, K_NHNCK_83, K_NHNCK_84 | Open (chờ verify đồng bộ toàn bộ LLD sau lần sửa 2) |
