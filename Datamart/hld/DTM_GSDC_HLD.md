...
4. **JOIN `fr_row_template`** ON `fr_row_template.row_code = fr_value.row_code AND fr_row_template.fr_catalog_id = fr_value.fr_catalog_id` — lấy `row_description_reference`. **[SỬA 2026-09-14, dev report qua module GSTT — cùng bug tại nguồn rule GSĐC]** `fr_row_template_code`/`fr_column_template_code` là 2 cột ID nội bộ (hash), không cùng miền giá trị với `row_code`/`column_code` bên `fr_value` (mã nghiệp vụ) — join theo `_code` nội bộ sẽ luôn ra rỗng.
5. **JOIN `fr_column_template`** ON `fr_column_template.column_code = fr_value.column_code AND fr_column_template.fr_catalog_id = fr_value.fr_catalog_id` — lấy `column_description_reference` (`'1'` cuối kỳ, `'2'` đầu kỳ).
6. **Dedup `fr_value`:** `ROW_NUMBER() OVER (PARTITION BY pc_id, rpt_year, rpt_quarter, fr_catalog_id, row_code, column_code ORDER BY ds_rcrd_udt_dt DESC, ds_rcrd_isrt_dt DESC, fr_val_code DESC) = 1` — bản ghi mới nhất mỗi ô. **[SỬA 2026-08-21]** Thêm `fr_catalog_id` vào PARTITION BY — `row_code`/`column_code` là mã tương đối theo từng catalog (biểu mẫu BCTC), thiếu `fr_catalog_id` có rủi ro dedup chéo giữa 2 catalog khác nhau trùng mã. Thêm `fr_val_code DESC` làm tie-breaker cuối, tránh non-determinism khi trùng cả 2 timestamp.
