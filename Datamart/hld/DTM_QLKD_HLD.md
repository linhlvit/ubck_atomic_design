...
> **Mở rộng 2026-07-27 (GSTT reuse, Nhóm 5):** GSTT bổ sung ~15 measure còn thiếu (Open/High/Low/Prior Index, Index Change, Index Percent Change, Advances/Declines/No Change/Ceiling/Floor Count, Advances/Declines/No Change Volume, Odd Lot Total Volume/Value, PT Total Volume/Value/Trade Count) vào cùng `fct_market_index_snpst` — lấy từ đúng row nguồn `rn=1` (MAX index_time per market_code per ngày) đã dùng cho `market_index_val`, không thêm JOIN/logic lọc mới, không đổi grain, không sửa 3 cột hiện có (`snpst_dt_dim_id`, `market_index_dim_id`, `market_index_val`). K_QLKD_88-91/K_NDTNN_34 không bị ảnh hưởng. `modules_using` bổ sung `GSTT`.
