...
## 1. AGENFUNDS — Pure Junction Table (không tạo entity Atomic)

AGENFUNDS = (AgnId, FndId) — chỉ có 2 trường nghiệp vụ: FK đến AGENCIES (Atomic entity Fund Distribution Agent) và FK đến FUNDS (Atomic entity Investment Fund). Không có attribute nghiệp vụ riêng.

→ Áp dụng quy tắc pure junction table giữa 2 Atomic entity: **không tạo Atomic entity**. 1 quỹ có nhiều đại lý phân phối → bên Many là Investment Fund → denormalize thành `ARRAY<STRUCT>` trên Investment Fund.
...
## 2. FNDSBMN — Pure Junction Table (không tạo entity Atomic)

FNDSBMN = (FndId, BmnId) — chỉ có 2 trường nghiệp vụ: FK đến FUNDS (Atomic entity Investment Fund) và FK đến BANKMONI (Atomic entity Custodian Bank). Không có attribute nghiệp vụ riêng.

→ Áp dụng quy tắc pure junction table giữa 2 Atomic entity: **không tạo Atomic entity**. 1 quỹ có nhiều ngân hàng lưu ký giám sát → bên Many là Investment Fund → denormalize thành `ARRAY<STRUCT>` trên Investment Fund.
...
| Hạng mục | Nội dung |
|---|---|
| Atomic Entity | Investment Fund Investor Membership |
...
| Hạng mục | Nội dung |
|---|---|
| Atomic Entity | Investment Fund Representative Board Member |
...
| Hạng mục | Nội dung |
|---|---|
| Atomic Entity | Foreign Fund Management Organization Unit Staff |
| BCV Concept | [Involved Party] Involved Party Role |
| Model Table Type | Fundamental (SCD1) |
...
graph LR
    ...
    RPT["**Member Periodic Report**\nBáo cáo định kỳ thành viên thị trường"]:::atomic
...
| Hạng mục | Nội dung |
|---|---|
| Atomic Entity | Member Periodic Report |
...
graph LR
    ...
    FMC["Fund Management Company (Tier 1)"]:::refnode
    TRF["**Fund Management Company\nShare Transfer**\nGiao dịch chuyển nhượng cổ phần QLQ"]:::atomic
...
| Hạng mục | Nội dung |
|---|---|
| Atomic Entity | Fund Management Company Share Transfer |
...
graph LR
    ...
    FMC["Fund Management Company (Tier 1)"]:::refnode
    FUND["Investment Fund (Tier 2)"]:::refnode
    VIO["**Fund Management Conduct Violation**\nVi phạm của công ty QLQ hoặc quỹ đầu tư"]:::atomic
...
| Hạng mục | Nội dung |
|---|---|
| Atomic Entity | Fund Management Conduct Violation |
...
graph TD
    ...
    STF["**Foreign Fund Management OU Staff**"]:::atomic
    RPT["**Member Periodic Report**"]:::atomic
    TRF["**Fund Management Company Share Transfer**"]:::atomic
...
| Source Table | Junction | Xử lý trên Atomic |
|---|---|---|
