...
**Mô tả:** ECAT cung cấp 46 danh mục nền tảng (quốc gia, vùng miền, địa giới hành chính, tiền tệ, chứng khoán, chức vụ, loại báo cáo, trạng thái...). Scope trong dự án Atomic: ECAT là **source chuẩn (authoritative)** cho danh mục Currency, Security (Code + Name), holiday flag của Calendar Date, và toàn bộ danh mục hành chính Việt Nam trên Geographic Area. Phần lớn các bảng còn lại (36/46) là Classification Value scheme — không tạo Atomic entity riêng.
...
**Tổng: 4 Atomic entities** (3 Tier 1, 1 Tier 2)
...
| D-02 | 35/46 danh mục thuần đi vào Classification Value, không tạo Atomic entity riêng | Áp dụng quy tắc #11 CLAUDE.md (bảng Code + Name không phải Atomic entity) + hướng dẫn skill. Ngoại lệ: Geographic Area (theo ngoại lệ skill), Currency (quy tắc #4 data domain), Security (instance data mã CK), Calendar Date (entity Time Period mới). |
...
> 4 entity ECAT không có FK lẫn nhau ở cấp Atomic. Geographic Area có self-join thể hiện phân cấp hành chính. Security liên kết 2 scheme Classification Value (không vẽ node Classification Value theo quy tắc).
...
*(Không có bảng ngoài scope. Toàn bộ 46 bảng ECAT đều thuộc scope Atomic: 4 Atomic entity + 36 Classification Value scheme.)*

> **Ghi chú:** ECAT là source thuần danh mục — các bảng không có FK inbound trong chính source ECAT là tình huống bình thường, không phải lý do loại khỏi scope. 36 bảng danh mục thuần được thiết kế thành Classification Value scheme (bảng Fundamental SCD4A trong kiến trúc Atomic). Xem chi tiết mục 7c.
