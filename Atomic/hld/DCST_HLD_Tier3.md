...
| 2 | Wide table TCT_BAO_CAO_CHI_TIET có các cột số liệu kiểu VARCHAR(100) — có cần normalize sang Currency Amount + currency code không? | Nếu luôn là VND và không có ngoại lệ → có thể giữ Text, làm sạch ở Gold. Nếu cần tính toán ở Atomic → cần xác nhận đơn vị và chuyển kiểu. |
...
**Lưu ý:** Wide table — giữ nguyên 18 cột số liệu, không pivot. Tất cả giá trị số từ nguồn là VARCHAR → giữ Text trên Atomic.
