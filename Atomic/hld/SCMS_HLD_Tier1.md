...
| DM_QUOC_TICH | Danh mục quốc tịch | [Location] Geographic Area | → **Atomic entity `Geographic Area`** (shared, status=approved). Bổ sung source_table. Type = `COUNTRY`. FK inbound: CTCK_CO_DONG, CTCK_NHAN_SU_CAO_CAP, CTCK_THONG_TIN, CTCK_VP_DAI_DIEN_NN. | — |
...
| DM_CANH_BAO | Danh sách cảnh báo báo cáo | — | **Ngoài scope Atomic.** Bảng định nghĩa rule nhập liệu hệ thống (CONG_THUC, DIEU_KIEN, CAP_DO) — không phải dữ liệu đã được nhập liệu/upload. Chỉ `BC_CANH_BAO` (kết quả cảnh báo thực tế) mới lên Atomic. | — |
...
|---|---|---|---|
| 1 | `DM_CANH_BAO` — Atomic entity hay Classification Value? | ✅ Đã quyết định | Ngoài scope Atomic. Đây là rule nhập liệu hệ thống, không phải dữ liệu được nhập. Chỉ `BC_CANH_BAO` (kết quả thực tế) mới lên Atomic. |
| 2 | `CTCK_THONG_TIN.CTCK_THONG_TIN_ID` — business key hay FK cha? | ✅ Đã quyết định | Là Business Key (BK) — mã nghiệp vụ dùng trong liên hệ với hệ thống khác. Map thành `securities_company_bk` trên Atomic. |
