...
Logic:
  ...
  2. Đọc atomic_attributes.csv → tập hợp cột đã map:
  ...
  6. Với mỗi source_column trong atomic_attributes.csv (đủ 3 phần):
...
ATTRS_FILE    = LLD_DIR / "atomic_attributes.csv"
...
def load_atomic_source_columns(filter_source=None, filter_table=None):
    ...
    cho mọi source_column trong atomic_attributes.csv có đủ 3 phần."""
    rows = []
    if not ATTRS_FILE.exists():
        return rows
    with open(ATTRS_FILE, encoding="utf-8-sig", newline="") as f:
