...
def infer_table_type(row: dict) -> tuple[str, str]:
    ...
    if atomic_entity in SHARED_ENTITIES:
        return "Relative", "Shared entity (IP Address / Electronic / Alt ID)"

    # 2. Snapshot (tên entity chứa "Snapshot") → Snapshot
    if any(kw in atomic_entity for kw in SNAPSHOT_KEYWORDS):
        return "Snapshot", f"atomic_entity chứa '{[kw for kw in SNAPSHOT_KEYWORDS if kw in atomic_entity][0]}'"
    ...
    # 3b. atomic_entity chứa Activity Log / Status Log / Status History → Fact Append
    matched_ek = [kw for kw in FACT_APPEND_ENTITY_KEYWORDS if kw in atomic_entity]
    if matched_ek:
        return "Fact Append", f"atomic_entity chứa '{matched_ek[0]}'"
...
def main():
    ...
    # Xác định vị trí chèn: sau atomic_entity
    insert_after = "atomic_entity"
    ...
    print(f"\n{'source_system':<8} {'atomic_entity':<55} {'table_type':<14} Lý do")
    print("-" * 120)
