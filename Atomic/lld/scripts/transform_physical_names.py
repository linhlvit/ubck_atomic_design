...
Bổ sung cột physical name và data type vào 2 file output LLD:
  - atomic_attributes.csv : thêm atomic_table (sau atomic_entity),
                            atomic_column và data_type (sau atomic_attribute)
  - attr_Classification_Value.csv : thêm atomic_column và data_type (sau attribute_name)
...
ATOMIC_ATTRS    = PROJECT_ROOT / "Atomic" / "lld" / "atomic_attributes.csv"
CLASSVAL_ATTRS  = PROJECT_ROOT / "Atomic" / "lld" / "attr_Classification_Value.csv"

# Tên cột mới trong atomic_attributes.csv
...
COL_CV_ENTITY   = "atomic_entity"
COL_CV_TABLE    = "atomic_table"
COL_CV_PHYS     = "atomic_column"
...
def patch_atomic_attributes(
    ...
    fields, rows = read_csv(ATOMIC_ATTRS)

    # Dọn tên cột cũ
    fields = remove_cols(fields, rows, "atomic_entity_physical_name", "atomic_attribute_physical_name", COL_DATA_TYPE)

    # Chèn cột physical name
    fields = insert_after(fields, "atomic_entity",    COL_ENTITY_PHYS)
    fields = insert_after(fields, "atomic_attribute", COL_ATTR_PHYS)

    # Chèn data_type ngay sau atomic_column
    ...
    for row in rows:
        entity = row.get("atomic_entity", "")
        attr   = row.get("atomic_attribute", "")
    ...
    if dry_run:
        ...
        write_csv(ATOMIC_ATTRS, fields, rows)
...
def patch_classval(
    ...
    # Thêm atomic_entity + atomic_table trước attribute_name (nếu chưa có)
...
def main() -> None:
    ...
    print(f"  atomic_attributes: {n1} dong", file=sys.stderr)
