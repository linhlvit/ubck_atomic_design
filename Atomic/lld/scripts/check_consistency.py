...
def extract_entities_from_hld(md_path: Path) -> set[str]:
    ...
    for m in re.finditer(r"^#{2,4}\s+(?:\d+\.\s+)?(.+)$", text, re.MULTILINE):
        ...
        if not re.match(r"^6[a-f]|^7[a-f]|^Tier\s+\d|^HLD|^Overview|^Source|^Atomic", name, re.IGNORECASE):
            found.add(name)
    ...
    for m in re.finditer(r"^\|\s*([^|]+?)\s*\|", text, re.MULTILINE):
        ...
        if re.match(r"^-+$|^Entity|^Atomic Entity|^Tier|^Bảng|^Source|^BCV|^Loại|^Mục|^Mermaid", cell, re.IGNORECASE):
            continue
        if len(cell) > 5:
            found.add(cell)
...
def check(reviewed: list[dict],
    ...
    # Build status lookup
    entity_status = {r["atomic_entity"]: r.get("status", "draft") for r in reviewed}
