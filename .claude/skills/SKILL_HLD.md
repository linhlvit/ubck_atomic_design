...
| Section | Heading |
|---|---|
| 7a | `#### 7a. Bảng tổng quan Silver entities` |
| 7b | `#### 7b. Diagram Silver tổng (Mermaid)` |
| 7c | `#### 7c. Bảng Classification Value` |
| 7d | `#### 7d. Junction Tables` |
| 7e | `#### 7e. Điểm cần xác nhận` |
| 7f | `#### 7f. Bảng ngoài scope` |

**Cấu trúc bảng:** Số cột và tên header phải đúng (xem chi tiết từng section). Grain mỗi dòng = 1 bảng nguồn — không gộp nhiều bảng vào 1 ô.
...
**Heading bắt buộc:** `#### 7f. Bảng ngoài scope` (chính xác từng ký tự).

**Bảng bắt buộc 4 cột — đúng thứ tự, đúng tên header:**
...
**Grain BẮT BUỘC: 1 dòng = 1 bảng nguồn.**

KHÔNG gộp nhiều bảng vào 1 ô "Source Table" (kể cả khi cùng lý do). Ví dụ SAI:

```markdown
| Cascade drop | data, data_values | ... | ... |
| System | logins, users | ... | ... |
```

Phải tách thành các dòng riêng:
...
Output csv lấy grain `(source_system, source_table)` — gộp nhiều bảng vi phạm key uniqueness.

**Quy tắc viết cột "Nhóm":** Dùng group từ danh sách chuẩn dưới đây:
...
Nếu cần group mới, thêm vào danh sách trên trong skill này để các source sau dùng nhất quán.

**Quy tắc viết cột "Mô tả bảng nguồn":** Mô tả ngắn gọn nội dung bảng (1 câu), KHÔNG nhắc lại lý do ngoài scope.
...
- KHÔNG mô tả nội dung bảng (ví dụ: "Chỉ có Code + Name" không phải lý do — đó là mô tả dữ liệu).
- Các lý do chuẩn:
...
**Encoding bắt buộc per-file:**

| File | Encoding |
|---|---|
| `silver_entities.csv` | UTF-8 with BOM (`utf-8-sig`) |
| `silver_out_of_scope.csv` | UTF-8 không BOM |
| `manifest.csv` (LLD) | UTF-8 không BOM |

**Strip BOM nếu file no-BOM bị thêm BOM (sau Write/Edit):**
...
" Silver/hld/silver_out_of_scope.csv
```

**Source-of-truth:** HLD Overview (mục 7f) và file Tier. `silver_entities.csv` và `silver_out_of_scope.csv` luôn được sinh bằng script aggregate.
...
**Cấu trúc cột:** `source_system,source_table,description,group,reason` (không space sau dấu phẩy trong header).

**Nguồn dữ liệu:** Mục `#### 7f. Bảng ngoài scope` trong tất cả `*_HLD_Overview.md`. Script `aggregate_out_of_scope.py` parse heading bằng regex `^#{2,3}\s+7f\b` — heading phải khớp.

**Workflow BẮT BUỘC sau mỗi lần update HLD Overview:**

1. Sửa nội dung mục 7f trong `<SOURCE>_HLD_Overview.md` theo format BẮT BUỘC ở Bước 7.
2. Chạy aggregate (chỉ source vừa sửa):
   ```bash
   python Silver/lld/scripts/aggregate_out_of_scope.py --source <SOURCE>
   ```
3. Nếu fail với `KeyError: 'source_system'` → strip BOM khỏi `silver_out_of_scope.csv`:
   ```bash
   python -c "
   with open('Silver/hld/silver_out_of_scope.csv', 'rb') as f: data = f.read()
   if data.startswith(b'\xef\xbb\xbf'):
       open('Silver/hld/silver_out_of_scope.csv', 'wb').write(data[3:])
   "
   ```
   Rồi chạy lại bước 2.
4. Verify số dòng source trong output csv = số dòng mục 7f của HLD Overview:
   ```bash
...
Checklist BẮT BUỘC sau khi tạo/sửa `<SOURCE>_HLD_Overview.md`:
...
- [ ] **Group dùng đúng danh sách chuẩn** (xem mục 7f). Nếu phát sinh group mới → bổ sung vào skill này trước.
