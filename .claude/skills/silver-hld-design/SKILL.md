---
name: silver-hld-design
description: |
  Thiết kế High-Level Design (HLD) cho Silver source system trong kiến trúc Medallion.
  ...
  {SOURCE}_HLD_Tier{N}.md hoặc {SOURCE}_HLD_Overview.md trong Silver/hld/.
  Cũng dùng khi cập nhật mục 7f (bảng ngoài scope), điều chỉnh source_table cho
  shared entity, hoặc cần chạy aggregate_silver.py / aggregate_out_of_scope.py.
...
4. **Đọc `Silver/hld/silver_entities.csv`** — lọc theo source system đang thiết kế (cột `source_table`), ghi nhận tất cả dòng có `status=approved`. Tên approved là **LOCKED** — dùng đúng tên đó trong HLD, không tự sinh tên mới. Dòng `status=draft` có thể điều chỉnh khi cần.
...
- Bảng **isolated** (không FK đến và không FK từ bất kỳ bảng nghiệp vụ nào trong scope) → ngoài scope Silver.
...
- Bảng **Audit Log nguồn** (cột PrevValue/ValueChange hoặc OldValue/NewValue + Action + DateChange): cơ chế ghi lịch sử đặc thù source system → ngoài scope Silver. Nếu cần dữ liệu lịch sử trên Silver → thiết kế job parsing riêng để đẩy vào entity Silver có cột tường minh.
  ...
  **Phân biệt Audit Log nguồn vs ETL Pattern — Activity/Status Log trên Silver:**
  ...
  - **ETL Pattern — Activity/Status Log** = Silver entity ghi nhận sự kiện nghiệp vụ có cột tường minh (trạng thái trước/sau, lý do thay đổi, timestamp) → trong scope Silver, tạo entity riêng.
...
- Bảng **Snapshot nguồn** (cột IsBefore + blob data như SecData/TLData): cùng lý do với Audit Log nguồn → ngoài scope Silver.
...
- **Ngoại lệ — Geographic Area**: Bảng lưu danh mục khu vực địa lý (tỉnh, thành phố, quốc gia...) dù chỉ có Code + Name → **Silver entity [Location] Geographic Area**, không phải Classification Value. Lý do: BCV có Data Concept riêng là Location.

**Pure junction table giữa entity và Classification Value:** Bảng chỉ có 2 trường nghiệp vụ — 1 trỏ đến entity chính, 1 trỏ đến bảng danh mục — không có attribute nghiệp vụ riêng → **không tạo Silver entity**. Denormalize thành trường `ARRAY<Classification Value Code>` trên entity chính.

**Pure junction table giữa 2 Silver entity:** Bảng chỉ có 2 trường nghiệp vụ đều là FK đến Silver entity — không có attribute nghiệp vụ riêng → **không tạo Silver entity**. Xác định bên "Many" trong quan hệ → denormalize thành trường `ARRAY<STRUCT>` trên entity bên Many: surrogate key (dùng join) + business code (lưu dư thừa). Ví dụ: `distribution_agents ARRAY<STRUCT<agent_id BIGINT, agent_code STRING>>`.
...
**Tên file:** `Silver/hld/{SOURCE_SYSTEM}_HLD_Tier{N}.md`
...
| 6c | Diagram Silver (Mermaid) — entity từ tầng trước hiện dạng node tham chiếu (chỉ tên), không vẽ Classification Value |
...
- **Sau khi xác định Scheme Code → đăng ký ngay vào `Silver/lld/ref_shared_entity_classifications.csv`**, kể cả khi chưa có giá trị cụ thể (ghi `(source)` hoặc `(to_define)` ở cột code).
...
**Tên file:** `Silver/hld/{SOURCE_SYSTEM}_HLD_Overview.md`

Copy [`templates/HLD_Overview.md`](templates/HLD_Overview.md) làm starting point.

### FORMAT BẮT BUỘC

Format dưới đây là contract giữa HLD Overview và các script aggregate (`aggregate_out_of_scope.py`, `aggregate_silver.py`) — script parse bằng regex trên heading và cấu trúc bảng Markdown.

**Heading bắt buộc** (chính xác từng ký tự, đúng cấp `####`, đúng tiền tố `7a.`...`7f.`):

| Section | Heading | Cấu trúc bảng |
|---|---|---|
| 7a | `#### 7a. Bảng tổng quan Silver entities` | 8 cột: `Tier \| BCV Core Object \| BCV Concept \| Category \| Source Table \| Mô tả bảng nguồn \| Silver Entity \| BCV Term` |
| 7b | `#### 7b. Diagram Silver tổng (Mermaid)` | 1 mermaid diagram |
| 7c | `#### 7c. Bảng Classification Value` | 4 cột: `Source Table \| Mô tả \| BCV Term \| Xử lý Silver` |
| 7d | `#### 7d. Junction Tables` | 4 cột: `Source Table \| Mô tả \| Entity chính \| Xử lý trên Silver` |
...
2. Chạy aggregate (chỉ source vừa sửa):
   ```bash
   python Silver/lld/scripts/aggregate_out_of_scope.py --source {SOURCE}
   ```
3. Verify số dòng source trong output csv = số dòng mục 7f của HLD Overview:
   ```bash
   grep "^{SOURCE}," Silver/hld/silver_out_of_scope.csv | wc -l
   ```
...
- [ ] **Chạy aggregate verify:** `python Silver/lld/scripts/aggregate_out_of_scope.py --source {SOURCE}` không có WARN, output có đủ số dòng.
- [ ] **Chạy aggregate silver entities:** `python Silver/lld/scripts/aggregate_silver.py` thành công, không có entity lock vi phạm.
- [ ] **Cross-check số liệu:** số entity Silver trong mục 7a Overview = số entity trong tất cả mục 6a các Tier.
