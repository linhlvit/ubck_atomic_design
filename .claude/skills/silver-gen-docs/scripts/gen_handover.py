...
def cmd_full(source: str) -> Path:
    ...
    for g in uid_groups:
        uid_entities = [e for e in all_entities if e["silver_entity"] in g["entities"]]
        if not uid_entities:
            continue
...
def _build_master_md(source_filter: str | None = None) -> tuple[Path, list[str]]:
    ...
    master_file = out_dir / f"silver_dbdesign{suffix}.md"
...
def cmd_docx(source_filter: str | None = None) -> Path:
    ...
    out_docx = docx_dir / f"Silver_DBDesign{suffix}.docx"
