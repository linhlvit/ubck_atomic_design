...
> ⛔ **Bài học thực tế (GSTT, 2026-09-11) — bàn giao Kịch bản C không đồng nghĩa đã đồng bộ registry:** Một chuỗi sửa bug join-key/filter theo Kịch bản C (được thực hiện trực tiếp mid-conversation theo yêu cầu tức thời của user, không qua lại toàn bộ flow Phase 0→3 của `datamart-lld-design`) đã sửa đúng file Attributes module (`Datamart/lld/{MODULE}/DTM_{MODULE}_*.csv`) nhưng **quên đồng bộ `datamart_attributes.csv`** (master registry) — không bị phát hiện qua nhiều lượt hội thoại sau vì tên bảng/cột không đổi, chỉ nội dung `etl_logic` khác. Riêng biệt, cùng đợt còn phát hiện 1 Fact đã READY (Attributes + Detail Mapping + HLD đầy đủ) nhưng bị bỏ sót khỏi `Entities.csv`/`.md` và Phase 3 flat table cả tuần.
> **Áp dụng bắt buộc:** Bất kể Kịch bản C được thực hiện qua lời gọi `datamart-lld-design` đầy đủ hay Edit trực tiếp mid-conversation (kể cả khi Reviewer — không phải `datamart-lld-design` — là người ra Action Proposal rồi tự thực thi theo xác nhận của user), NGAY SAU khi sửa xong etl_logic của 1 file Attributes module, PHẢI: (1) đồng bộ dòng tương ứng trong `datamart_attributes.csv`, (2) chạy TC7 Sub-check E (etl_logic content parity — xem `datamart-lld-design/SKILL.md`) để xác nhận khớp, (3) chạy Orphan Check 3 chiều (LLD ↔ Entities ↔ Flat Table) nếu thay đổi liên quan đến 1 Fact/Dim — TRƯỚC khi báo "đã sửa xong" cho user. Đây không phải bước tùy chọn "nếu có thời gian" — thiếu bước này chính là nguyên nhân gốc của cả 2 lỗi kể trên.
