...
| `L3-REFERENCE-SQL-MISALIGNMENT` | Lớp 3 (Detail Mapping) | 🔴 Critical | C | Gate 2, Gate 3 | `python scripts/datamart_ba_cross_checker.py` / Manual Detail Mapping review | Vi phạm Quy tắc L17: Không bám sát Câu lệnh tham khảo & Điều kiện chung BA (lệch số đo khớp lệnh vs thỏa thuận, thiếu dòng FILTER cho WHERE clause, sai logic rolling). |
...
#### 12B. Đặc tả Mã lỗi: `L3-REFERENCE-SQL-MISALIGNMENT`

| Thuộc tính | Chi tiết đặc tả |
|---|---|
| **Mã lỗi (Error Code)** | `L3-REFERENCE-SQL-MISALIGNMENT` |
| **Tên lỗi (Issue Name)** | Không bám sát Câu lệnh tham khảo (Reference SQL) & Điều kiện chung trong file BA (Quy tắc L17) |
| **Phân loại kịch bản** | **Kịch bản C — Lỗi kỹ thuật tầng Detail Mapping (Lớp 3)** |
| **Mức độ (Severity)** | 🔴 **Critical** (Chặn mở Gate 2 và Gate 3, dữ liệu báo cáo sai bản chất nghiệp vụ) |
| **Mô tả (Description)** | Detail Mapping không phản ánh chính xác câu lệnh SQL tham khảo và điều kiện kỹ thuật do BA cung cấp tại file `BA_analyst_{MODULE}.csv`. Bao gồm 3 dạng sai lệch cốt lõi:<br>(1) **Sai lệch số đo (Measure Misalignment):** Chỉ tiêu khớp lệnh thuần (SQL tham khảo loại trừ thỏa thuận: `Board Type NOT IN ('T1'..'R1')`) nhưng lại map vào `Total Volume` / `total_vol` (gộp cả thỏa thuận) thay vì `Total Matched Volume` / `total_matched_vol`. Hoặc chỉ tiêu mua/bán/ròng bị đảo chiều biểu thức tính toán.<br>(2) **Bỏ sót điều kiện lọc tĩnh (Missing Filter Rows):** Trong SQL tham khảo có mệnh đề `WHERE` lọc sàn (`FloorCode IN (...)`), loại trừ chứng khoán (`StockType NOT IN (1,4)`), phân loại lệnh (`Board Type`), cờ hiệu lực (`active_flg = 1`) nhưng Detail Mapping không sinh các dòng `column_role = FILTER` tương ứng và không có ghi chú rõ ràng là đã xử lý ngầm ở ETL Fact.<br>(3) **Sai lệch cấu trúc Rolling Window / Tính toán phái sinh:** Bỏ sót hoặc làm sai số phiên/grain tính toán của các hàm cửa sổ trong SQL tham khảo (như KLGDTB 5 ngày `5 PRECEDING`, TTM 4 quý BCTC). |
| **Phương pháp chẩn đoán & phát hiện** | 1. Mở file `BRD/BA/BA_analyst_{MODULE}.csv`, lọc các dòng có dữ liệu ở cột `Câu lệnh tham khảo` hoặc `Điều kiện chung`.<br>2. Bóc tách 4 thành phần: `SELECT`, `WHERE`, `FROM/JOIN`, `GROUP BY/Window`.<br>3. So khớp với Detail Mapping:<br>  - Kiểm tra xem tên cột và hàm tổng hợp trong `logic` có phản ánh đúng `SELECT` không.<br>  - Kiểm tra xem mọi điều kiện trong `WHERE` đã có dòng `column_role = FILTER` tương ứng trong nhóm chưa (hoặc có note ETL Fact chưa).<br>4. Chạy CLI: `python scripts/datamart_ba_cross_checker.py --module [MODULE] --lint-detail-mapping`. |
| **Remediation Protocol** | **Bước 1:** Đọc toàn bộ câu lệnh SQL tham khảo, điều kiện chung và note của BA.<br>**Bước 2:** Sửa lại `mart_column` và `logic` phản ánh đúng số đo (ví dụ: đổi `total_vol` ➔ `total_matched_vol` cho chỉ tiêu khớp lệnh).<br>**Bước 3:** Bổ sung các dòng `column_role = FILTER` cho các điều kiện lọc tĩnh của sàn, loại chứng khoán, loại bảng lệnh vào nhóm; hoặc nếu Fact đã lọc sẵn thì ghi rõ vào `ghi_chu`: `"Đã lọc sẵn tại ETL Fact theo SQL tham khảo BA: <điều kiện>"`.<br>**Bước 4 (Verify):** Chạy lại `datamart_ba_cross_checker.py --module [MODULE] --lint-detail-mapping` xác nhận 0 vi phạm. |
