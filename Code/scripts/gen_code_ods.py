"""
gen_code_ods — Convert ODS mapping CSV → formatted SQL (dbt source syntax).
...
Reads from:
    Mapping/ods/<SS>/ods_<ss>_<table>.csv
    system/rules/rule_map_schema.csv  (lookup Tên Schema Var DBT STG)

Writes to:
    Code/ods/<SS>/ods_<ss>_<table>.sql

Rules:
- FROM uses dbt ref(): {{ ref('<staging_table>') }}
- Physical Target Column as-is from mapping (already lowercase)
- etl_date mapping pattern → to_date('{{ var("etl_date") }}','yyyy-MM-dd'), no CAST
- CAST applied only when Data Type column is non-empty (and not etl_date pattern)
- No WHERE rendered when filter is empty
- Program name: ODS <SS_UPPER> <TABLE_UPPER>
"""
import csv, os, sys, argparse, glob, re

REPO_ROOT   = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
MAPPING_DIR = os.path.join(REPO_ROOT, 'Mapping', 'ods')
CODE_DIR    = os.path.join(REPO_ROOT, 'Code', 'ods')
...
def build_cte(row):
    """Build CTE for a physical_table input row using dbt ref() syntax."""
    staging_table = get(row, 3)   # e.g. stg_fms_securities
    alias         = get(row, 4)
    sel           = get(row, 5) or '*'
    filt          = get(row, 6)

    dbt_ref = f"{{{{ ref('{staging_table}') }}}}"
    cte = f"{alias} AS (\n    SELECT {sel}\n    FROM {dbt_ref}"
    if filt:
...
def build_ctes(input_rows):
    return [build_cte(r) for r in input_rows if get(r, 1) == 'physical_table']
...
def main():
    parser = argparse.ArgumentParser(description='Generate ODS SQL from mapping CSV.')
    ...
    parser.add_argument('--all',           action='store_true',     help='Process all ODS mapping files')
    ...
    if not args.stdout:
        print(f'[gen_code_ods] Done. {len(generated)} file(s) generated:')
        for p in generated:
