# IDS — Tài liệu khảo sát nguồn và ánh xạ Silver
...
**Mục đích:** Tổng hợp nghiệp vụ nguồn, quan hệ bảng (tree FK), và ánh xạ Silver entity theo từng nhóm chức năng (UID) — phục vụ tham chiếu cho thiết kế Silver layer.
...
Ký hiệu trong cột **Ánh xạ Silver**:

| Ký hiệu | Ý nghĩa |
|---|---|
| 🟢 *Tên entity* | Silver entity được thiết kế, ví dụ `🟢 Public Company` |
...
| 🔴 (Out of scope) *lý do* | Bảng không thiết kế Silver, kèm lý do ngắn gọn |
...
### 4.6 Chức vụ cổ đông *(chờ thiết kế Silver)*

**Nghiệp vụ:** Lưu thông tin chức vụ của cổ đông (vd: Chủ tịch HĐQT, Thành viên BGĐ, Cổ đông lớn…). Bảng này không được đề cập trong HLD Overview hiện tại — cần xác nhận thiết kế Silver.
...
| │   └── `fields_history` | Lịch sử thay đổi fields | 🔴 (Out of scope) *Bảng lịch sử kỹ thuật — SCD2 Silver tự track* |
| ├── `form_fields` | Bảng nối: danh sách fields trong một form | 🔴 (Out of scope) *Field definition metadata — denormalized vào data_values* |
| │   └── `form_fields_history` | Lịch sử thay đổi form_fields | 🔴 (Out of scope) *Bảng lịch sử kỹ thuật — SCD2 Silver tự track* |
...
Các bảng `*_his` trong IDS ghi nhận lịch sử thay đổi của hồ sơ nghiệp vụ theo thời gian. Theo quyết định thiết kế Silver (D-xx trong HLD), các bảng lịch sử này **out-of-scope** vì Silver layer tự triển khai SCD2 (Type 2 Slowly Changing Dimension) trên entity chính — không cần map lại bảng lịch sử kỹ thuật của nguồn.
...
|---|---|---|
| `company_profiles` | Công ty đại chúng (xem 1.1) | 🟢 Public Company |
| ├── `company_profiles_his` | Lịch sử hồ sơ công ty | 🔴 (Out of scope) *Bảng lịch sử kỹ thuật — SCD2 Silver tự track* |
| ├── `company_detail_his` | Lịch sử chi tiết công ty | 🔴 (Out of scope) *Bảng lịch sử kỹ thuật — SCD2 Silver tự track* |
| └── `company_change_role_his` | Lịch sử thay đổi vai trò user (theo công ty) | 🔴 (Out of scope) *Bảng lịch sử kỹ thuật — SCD2 Silver tự track* |
| `stock_holders` | Cổ đông giao dịch (xem 4.1) | 🟢 Stock Holder |
| └── `stockholder_history` | Lịch sử cổ đông | 🔴 (Out of scope) *Bảng lịch sử kỹ thuật — SCD2 Silver tự track* |
...
Các bảng quản trị hệ thống, cấu hình, log, phân quyền, và danh mục tham chiếu chung của IDS. Phần lớn **out-of-scope** với Silver layer vì là metadata hệ thống, trừ hai bảng danh mục lớn (`lookup_values`, `categories`) được ánh xạ thành Classification Value schemes.
...
| Bảng | Ý nghĩa | Ánh xạ Silver |
|---|---|---|
| `logins` | Thông tin đăng nhập | 🔴 (Out of scope) *Bảng hệ thống — không có giá trị nghiệp vụ Silver* |
| `users` | Tài khoản sử dụng hệ thống (phân hệ AF, thiếu thiết kế chi tiết) | 🔴 (Out of scope) *Bảng hệ thống — không có giá trị nghiệp vụ Silver* |

### 9.2 Phân quyền và tham số hệ thống

**Nghiệp vụ:** `data_access_rules` lưu các quy tắc phân quyền dữ liệu theo user (ai được xem dữ liệu của công ty nào). `sys_parameters` lưu cấu hình tham số hệ thống. `departments` lưu danh sách phòng ban của UBCKNN — khi Silver cần FK tới department, sử dụng shared Regulatory Authority Organization Unit.
...
**Nghiệp vụ:** Ghi log thao tác của user (`user_audit_log`) và log gửi SMS (`sms_log`). Cả hai đều là operational log, out-of-scope Silver.
...
Bảng `lookup_values` chứa 30 `lookup_group`, mỗi group tương ứng một Classification Value scheme trong Silver layer. Danh sách đầy đủ (từ `ref_shared_entity_classifications.csv`):

| Scheme code | Mô tả | Sử dụng bởi Silver entity |
|---|---|---|
...
| Scheme code | Nguồn cột | Silver entity |
|---|---|---|
