# Khảo sát nguồn và ánh xạ Silver — Phân hệ Thanh Tra
...
**Tổng quan:** 56 bảng nguồn → 33 Silver entities (30 mới + 2 reuse + 1 Geographic Area shared) · 13 Classification Value · 2 Geographic Area shared · 8 Out-of-scope (SYS_*)
...
**Quan hệ dữ liệu:** Cùng cấu trúc bảng với B.1.2 *(xem B.1.2)* — các bảng `TT_KE_HOACH`, `TT_QUYET_DINH`, `TT_HO_SO` và bảng con phân biệt nghiệp vụ kiểm tra qua cột `LOAI_HINH`/`LOAI_KE_HOACH` = `KIEM_TRA`. Silver entity giữ nguyên — 1 entity phục vụ cả thanh tra và kiểm tra.
...
| `SYS_VAN_BAN_PHAP_QUY` | Văn bản pháp quy tham chiếu | 🔴 (Out of scope) *Regulatory ref. — ngoài scope Silver* |

---

## Phụ lục: Danh mục dùng chung

Các bảng DM_* được tham chiếu xuyên suốt nhiều nhóm chức năng. Phân loại: Silver entity, Classification Value (CV), hoặc Geographic Area shared.

### A. Silver entities — Danh mục đối tượng
