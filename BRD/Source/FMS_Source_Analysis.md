...
| **HLD / Silver mapping** | ✅ Có — FMS_HLD_Overview.md (7a, 7d–7f), ref_shared_entity_classifications.csv (CV) |
| **Tổng số bảng** | 79 |
...
| Ký hiệu | Ý nghĩa |
|---|---|
| 🟢 *Tên entity* | Silver entity được thiết kế (HLD 7a) |
| 🟢 *Tên entity* *(shared — bổ sung source)* | Silver entity dùng chung nhiều source (HLD 7a, shared) |
...
| 🔴 (Out of scope) *lý do* | Ngoài scope Silver (HLD 7f) |
...
Quản trị hạ tầng hệ thống FMS: người dùng, phân quyền chức năng, phiên đăng nhập, lịch làm việc, chứng thư số. Toàn bộ nhóm nằm ngoài scope Silver (hạ tầng IT / isolated).
...
Danh mục master data riêng phân hệ FMS. Phần lớn → Classification Value; riêng NATIONAL → Silver entity Geographic Area (shared).
...
Chức năng hỗ trợ: tham số cấu hình, email, thông báo, hiển thị, vi phạm. Phần lớn ngoài scope Silver trừ VIOLT.
...
| Phân loại | Số bảng |
|---|---|
| 🟢 Silver entity (HLD 7a) | 24 bảng → 21 Silver entities + 3 shared entities |
