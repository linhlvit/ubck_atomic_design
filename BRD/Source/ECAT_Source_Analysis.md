# ECAT — Tài liệu khảo sát nguồn và ánh xạ Silver
...
| **Đặc thù nguồn** | ECAT là source thuần danh mục dùng chung — không có file đặc tả yêu cầu hay thiết kế CSDL riêng. Mặc định mọi bảng schema = `Code + Name` (+ `parent_code` với bảng phân cấp). Không có FK inbound giữa các bảng trong chính source ECAT — đây là tình huống bình thường cho source danh mục. Toàn bộ 46/46 bảng thuộc scope Silver: 4 Silver entity + 36 Classification Value scheme. |

## Quy ước ký hiệu cột Ánh xạ Silver
...
ECAT đồng bộ 7 danh mục hành chính Việt Nam từ HTTT, bao gồm quốc gia, vùng/miền, tỉnh/thành phố, quận/huyện, phường/xã ở cả phiên bản cũ (pre-2025) và mới (post-sáp nhập 2025). Đây là source authoritative cho Geographic Area trong hệ thống Silver.
...
> Không có FK giữa 2 bảng trong source ECAT. Ở Silver, cả hai đều map vào Geographic Area với `geographic_area_type_code` khác nhau (COUNTRY, REGION).
...
> Ở Silver, `ECAT_03_ProvinceOld` dùng `geographic_area_type_code` = PROVINCE_OLD, `ECAT_04_Province` dùng PROVINCE.
...
> Ở Silver, phân biệt qua `geographic_area_type_code`: DISTRICT_OLD, WARD_OLD, WARD. Phân cấp cha-con (quốc gia → vùng → tỉnh → quận → phường) thể hiện qua `parent_geographic_area_id` (self-join). Các bảng phân cấp địa lý có cột `parent_code` thực tế (user đã xác nhận).
...
> Currency là Silver entity riêng (data domain — quy tắc #4), ECAT là source authoritative.
...
> Ở Silver, `ECAT_12_SecurityType` và `ECAT_13_Market` là Classification Value thuộc entity Security (cột `security_type_code`, `market_code`). `ECAT_15_SharePar` là CV dùng chung, không gắn trực tiếp vào entity cụ thể.
...
> Calendar Date là Silver entity mới. Dense calendar do ETL tự generate; ECAT cung cấp holiday flag + holiday name.
...
Bảng dưới đây gom tất cả 36 bảng danh mục ECAT được thiết kế thành Classification Value scheme trong Silver.
