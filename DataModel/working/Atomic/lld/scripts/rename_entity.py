...
# ---------------------------------------------------------------------------
def patch_yaml_file(path: Path, old: str, new: str,
                    string_fields: list[str], dry_run: bool) -> int:
    """Replace old → new trong các string fields của YAML file. Trả về số thay đổi."""
    if not path.exists():
        return 0
    ...
    for a in (data or {}).get("attributes", []):
        for f in string_fields:
            ...
            if isinstance(v, str) and old in v:
                a[f] = v.replace(old, new)
                count += 1
    # Also check metadata fields (atomic_entity, etc.)
    meta = data.get("metadata", {})
    for f in ("atomic_entity",):
        v = meta.get(f)
        if isinstance(v, str) and old in v:
            meta[f] = v.replace(old, new)
            count += 1
    if count and not dry_run:
...
def patch_attr_file(source_system: str, lld_file: str,
    ...
    path = LLD_DIR / lld_file
    if not path.exists():
        path = LLD_DIR / source_system / Path(lld_file).name
    return patch_yaml_file(path, old, new,
                           string_fields=["attribute_name", "description", "comment"],
                           dry_run=dry_run)
...
def propagate_rename(old: str, new: str,
    ...
    cnt = patch_yaml_file(OUT_ATTRS, old, new,
                          string_fields=["atomic_entity", "atomic_attribute",
                                         "description", "source_column"],
                          dry_run=dry_run)
    if cnt:
        print(f"  {prefix}atomic_attributes.yaml       : {cnt} thay thế")
    total_changes += cnt

    # 3. manifest.yaml — cập nhật atomic_entity để giữ sync
    if YAML_MANIFEST.exists():
        mdata = yaml.safe_load(YAML_MANIFEST.read_text(encoding="utf-8"))
        mcnt = 0
        for e in (mdata or {}).get("entries", []):
            if e.get("atomic_entity") == old:
                e["atomic_entity"] = new
                mcnt += 1
        if mcnt and not dry_run:
            with open(YAML_MANIFEST, "w", encoding="utf-8") as fh:
                yaml.dump(mdata, fh, allow_unicode=True, sort_keys=False, default_flow_style=False)
        if mcnt:
            print(f"  {prefix}manifest.yaml                : {mcnt} thay thế")
        total_changes += mcnt

    # 4. lld_*.yaml của entity này
    cnt = patch_attr_file(source_system, lld_file, old, new, dry_run)
    if cnt:
        print(f"  {prefix}{source_system}/{lld_file:<35}: {cnt} thay thế")
    total_changes += cnt

    # 4b. Các lld_*.yaml của entity KHÁC
    for row in manifest_rows:
        other_lld = row["lld_file"]
        other_sys = row["source_system"]
        if other_lld == lld_file and other_sys == source_system:
            continue
        path = LLD_DIR / other_lld
        if not path.exists():
            path = LLD_DIR / other_sys / Path(other_lld).name
        cnt = patch_yaml_file(path, old, new,
                              string_fields=["attribute_name", "description", "comment"],
                              dry_run=dry_run)
        if cnt:
...
def main():
    ...
    print("Đọc manifest.yaml (tên cũ)...", file=sys.stderr)
    ...
    if args.dry_run:
        ...
        print("Chạy aggregate_atomic.py để refresh atomic_attributes.yaml.")
