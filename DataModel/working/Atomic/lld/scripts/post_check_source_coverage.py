...
Logic:
  ...
  6. Với mỗi source_column trong atomic_attributes.yaml (đủ 3 phần):
...
SKIP_COLUMNS = {
    c.upper() for c in [
        ...
        "ISDELETE", "ISDELETED",
        # Binary/file attachment fields — không lưu Atomic
        "FILEDATA", "FILEIMPORT", "DATAFILEIMPORT",
        # File attachment name/data fields — không lưu Atomic
        ...
        "USERID",
        # QLRR audit user fields — không có User entity trên Atomic (HLD decision)
...
def load_manifest(filter_source=None, filter_table=None):
    ...
    for e in (data or {}).get("entries", []):
        if e.get("group", "") == "pending":
            continue
        ss = e.get("source_system", "")
        st = e.get("source_table", "")
        if filter_source and ss != filter_source:
            continue
        if filter_table and st != filter_table:
            continue
        result[(ss, st)] = e.get("group", "")
    return result


def _iter_attrs():
    """Yield dicts từ atomic_attributes.yaml với string normalization."""
    if not ATTRS_FILE.exists():
        return
    data = yaml.safe_load(ATTRS_FILE.read_text(encoding="utf-8"))
    for a in (data or {}).get("attributes", []):
        r = dict(a)
        for k in ("source_column", "source_system", "source_table",
                  "atomic_entity", "atomic_attribute"):
            if r.get(k) is None:
                r[k] = ""
        yield r
...
def load_atomic_source_columns(filter_source=None, filter_table=None):
    """Trả về list (source_system, source_table, col_orig, atomic_entity, atomic_attr)
    cho mọi source_column trong atomic_attributes.yaml có đủ 3 phần."""
    rows = []
    for row in _iter_attrs():
        col = row.get("source_column", "").strip()
        if not col:
...
def main():
    ...
    print(f"\n{'=' * 60}")
    if total_unmapped == 0:
        print("✓  CHECK A: Mọi cột nguồn đã được thiết kế đều có mapping trong Atomic.")
    else:
    ...
    print(f"\n{'=' * 60}")
    if total_ghost == 0:
        print("✓  CHECK B: Mọi source_column trong Atomic đều tồn tại trong nguồn.")
    else:
