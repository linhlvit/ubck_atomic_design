...
**Tổng: 34 Atomic entities** (9 Tier 1, 8 Tier 2, 10 Tier 3, 7 Tier 4)  
...
| Cascade drop | report_approval | Phê duyệt báo cáo (FK company_data) | Cascade drop — quy trình phê duyệt nội bộ hệ thống, không có giá trị nghiệp vụ Atomic |
| Cascade drop | report_extensions | Gia hạn báo cáo (FK company_data) | Cascade drop — gia hạn kỹ thuật, không có giá trị nghiệp vụ Atomic |
...
| Operational / System | logins | Tài khoản đăng nhập hệ thống | Bảng hệ thống — không có giá trị nghiệp vụ Atomic |
| Operational / System | users | Người dùng hệ thống | Bảng hệ thống — không có giá trị nghiệp vụ Atomic |
...
| Audit Log nguồn | company_profiles_his | Lịch sử thay đổi company_profiles | Bảng lịch sử kỹ thuật — SCD2 Atomic tự track |
| Audit Log nguồn | company_detail_his | Lịch sử thay đổi company_detail | Bảng lịch sử kỹ thuật — SCD2 Atomic tự track |
| Audit Log nguồn | company_change_role_his | Lịch sử thay đổi vai trò người dùng | Bảng lịch sử kỹ thuật — SCD2 Atomic tự track |
| Audit Log nguồn | fields_history | Lịch sử thay đổi fields | Bảng lịch sử kỹ thuật — SCD2 Atomic tự track |
| Audit Log nguồn | form_fields_history | Lịch sử thay đổi form_fields | Bảng lịch sử kỹ thuật — SCD2 Atomic tự track |
| Audit Log nguồn | stockholder_history | Lịch sử thay đổi cổ đông | Bảng lịch sử kỹ thuật — SCD2 Atomic tự track |
...
| D-05 | company_data → **Atomic entity `Public Company Report Submission` (Tier 3)** | ✅ Đảo quyết định: Gold cần dữ liệu BCTC thực tế từ `data`. company_data là anchor bắt buộc để biết công ty nào nộp kỳ nào. Filter: news_status_cd = 'APPROVED'. LLD: attr_IDS_company_data.csv. |
| D-06 | data → **Atomic entity `Public Company Financial Report Value` (Tier 4, Fact Append)**; report_approval + report_extensions vẫn out-of-scope; data_values vẫn out-of-scope | ✅ Đảo quyết định cho `data`: Gold requirement mới cần giá trị ô BCTC (data_value, row_cd, col_cd). LLD: attr_IDS_data.csv. `report_approval`, `report_extensions`, `data_values` vẫn out-of-scope. |
| D-07 | af_approval: gộp BTC + SSC vào 1 entity | Cùng grain (1 hồ sơ per công ty KT), cùng nguồn af_approval. |
| D-08 | categories là Classification Value, không phải Atomic entity | Chỉ có code + name + parent_id. Scheme IDS_INDUSTRY_CATEGORY. |
| D-09 | Không dùng prefix "IDS" trong tên Atomic entity | Entity name theo pattern [Domain BCV Term], không theo source system. |
