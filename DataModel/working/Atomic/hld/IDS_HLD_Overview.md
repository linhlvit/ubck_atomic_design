...
- [IDS_HLD_Tier1.md](IDS_HLD_Tier1.md) — Public Company, Legal Entity, Audit Firm, Financial Report Template, Financial Report Catalog, Unstructured Financial Report Template, Public Company Evaluation Group, Public Company Evaluation Period, Classification Business Line (shared với ECAT)
...
| T1 | Documentation | [Documentation] Form Document | Documentation | `FORMS` | Update | Định nghĩa template form CBTT — tên form, phiên bản, self-ref qua parent_form_id. | Financial Report Template | Fundamental | (1) [Documentation] Form Document (BCV id 9348) — Documentation Item theo bố cục mẫu chuẩn, yêu cầu cung cấp thêm thông tin. (2) Cấu trúc trường: FORM_CODE, FORM_NAME, FORM_VERSION, PARENT_FORM_ID (self-ref), STATUS_CD — chỉ metadata layout, dữ liệu thực nằm ở Financial Report Value. (3) Chọn [Documentation] Form Document, thay `[Condition] Form Definition` (term không tồn tại trong knowledge/terms.csv). Đổi theo yêu cầu tường minh Data Modeler (2026-08-13) — xem 7e #21. Đổi tên từ `Disclosure Form Definition` (2026-08-05) — xem 7e. |
...
| T1 | Documentation | [Documentation] Form Document | Documentation | `REP_FORMS` | Update | Template báo cáo định kỳ (tháng/quý/năm/bán niên): tên form, kỳ báo cáo, loại. | Unstructured Financial Report Template | Fundamental | (1) [Documentation] Form Document (BCV id 9348) — Documentation Item theo bố cục mẫu chuẩn cho báo cáo định kỳ. (2) Cấu trúc trường: FORM_CODE, FORM_NAME, PERIOD_TYPE_CD, STATUS_CD — chỉ metadata layout, không có dữ liệu nghiệp vụ thực. (3) Chọn [Documentation] Form Document, thay `[Condition] Form Definition` (term không tồn tại trong knowledge/terms.csv). Đổi theo yêu cầu tường minh Data Modeler (2026-08-13) — xem 7e #21. Đổi tên từ `Periodic Report Form` (2026-08-05) — xem 7e. Đổi Domain Prefix từ `Financial Report` sang `Unstructured Financial Report` (`ufr`) (2026-08-27) — xem 7e #22. Bỏ chữ "Sub" khỏi tên entity (2026-08-27) — xem 7e #23. |
...
| T2 | Documentation | [Documentation] Form Document | Documentation | `RROW` | Update | Định nghĩa hàng của template báo cáo tài chính: mã hàng, tên, cấp, công thức tổng hợp. | Financial Report Row Template | Fundamental | (1) [Documentation] Form Document (BCV id 9348) — thành phần cấu trúc (detail) của bản mẫu Financial Report Catalog. (2) FK → REPORT_CATALOG (T1, vẫn `[Condition] Form Definition` — chưa đổi trong lần này, xem 7e #21). (3) Chọn [Documentation] Form Document, thay `[Condition] Form Definition` (term không tồn tại trong knowledge/terms.csv). Đổi theo yêu cầu tường minh Data Modeler (2026-08-13) — xem 7e #21. Tên entity bỏ chữ "Form" để khớp physical_name `fnc_rpt_row_tpl` và diagram 7b (đồng nhất bản LinhLV). Khôi phục scope 2026-08-05 — xem 7e. |
| T2 | Documentation | [Documentation] Form Document | Documentation | `RCOL` | Update | Định nghĩa cột của template báo cáo tài chính: mã cột, tên, kỳ tham chiếu. | Financial Report Column Template | Fundamental | (1) [Documentation] Form Document (BCV id 9348) — thành phần cấu trúc (detail) của bản mẫu Financial Report Catalog. (2) FK → REPORT_CATALOG (T1, vẫn `[Condition] Form Definition` — chưa đổi trong lần này, xem 7e #21). (3) Chọn [Documentation] Form Document, thay `[Condition] Form Definition` (term không tồn tại trong knowledge/terms.csv). Đổi theo yêu cầu tường minh Data Modeler (2026-08-13) — xem 7e #21. Tên entity bỏ chữ "Form" để khớp physical_name `fnc_rpt_clmn_tpl` và diagram 7b (đồng nhất bản LinhLV). Khôi phục scope 2026-08-05 — xem 7e. |
...
| T2 | Documentation | [Documentation] Form Document | Documentation | `REP_ROW` | Update | Định nghĩa hàng của template báo cáo định kỳ. | Unstructured Financial Report Row Template | Fundamental | (1) [Documentation] Form Document (BCV id 9348) — thành phần cấu trúc (detail) của bản mẫu Unstructured Financial Report Template. (2) FK → REP_FORMS (nay cũng Documentation — không lệch BCO cha/con). (3) Chọn [Documentation] Form Document, thay `[Condition] Form Definition` (term không tồn tại trong knowledge/terms.csv). Đổi theo yêu cầu tường minh Data Modeler (2026-08-13) — xem 7e #21. Đổi tên từ `Periodic Report Form Row Template` (2026-08-05) — xem 7e. Đổi Domain Prefix từ `Financial Report` sang `Unstructured Financial Report` (`ufr`) (2026-08-27) — xem 7e #22. Bỏ chữ "Sub" khỏi tên entity (2026-08-27) — xem 7e #23. |
| T2 | Documentation | [Documentation] Form Document | Documentation | `REP_COLUMN` | Update | Định nghĩa cột của template báo cáo định kỳ. | Unstructured Financial Report Column Template | Fundamental | (1) [Documentation] Form Document (BCV id 9348) — thành phần cấu trúc (detail) của bản mẫu Unstructured Financial Report Template. (2) FK → REP_FORMS (nay cũng Documentation — không lệch BCO cha/con). (3) Chọn [Documentation] Form Document, thay `[Condition] Form Definition` (term không tồn tại trong knowledge/terms.csv). Đổi theo yêu cầu tường minh Data Modeler (2026-08-13) — xem 7e #21. Đổi tên từ `Periodic Report Form Column Template` (2026-08-05) — xem 7e. Đổi Domain Prefix từ `Financial Report` sang `Unstructured Financial Report` (`ufr`) (2026-08-27) — xem 7e #22. Bỏ chữ "Sub" khỏi tên entity (2026-08-27) — xem 7e #23. |
...
| 21 | T1/T2 | Theo yêu cầu tường minh Data Modeler (2026-08-13): đổi BCV Core Object/Concept cho 6 entity `FORMS`, `REP_FORMS` (T1), `RROW`, `RCOL`, `REP_ROW`, `REP_COLUMN` (T2) từ `[Condition] Form Definition` sang `[Documentation] Form Document`. Rà soát `knowledge/terms.csv` phát hiện term "Form Definition" **không tồn tại** (0 kết quả grep) — term cũ được gán khi thiết kế trước đây (2026-08-05 trở về trước) không qua tra cứu BCV đúng quy trình rule#6, mà tự đặt phỏng theo tên bảng nguồn. Term "Form Document" (BCV id 9348, category Documentation, part_of `Documentation Item`) là term thật, mô tả "Documentation Item trình bày theo bố cục mẫu chuẩn, yêu cầu cung cấp thêm thông tin" — khớp chính xác cấu trúc cả 6 bảng (chỉ có metadata layout: mã/tên/loại/công thức/thứ tự hiển thị, không có dữ liệu nghiệp vụ thực; dữ liệu thực nằm ở `Financial Report Value`/`DATA`, đã đúng `[Documentation] Regulatory Report`). Nhân dịp sửa, đồng bộ luôn Table Type `REP_ROW`/`REP_COLUMN`: `Tier2.md` 6a ghi `Relative` trong khi Overview 7a/Entities/LLD/manifest đã thống nhất `Fundamental` từ trước — chọn `Fundamental` (đa số nguồn, không phải thay đổi phát sinh từ việc đổi BCO). | Đã xử lý: `IDS_HLD_Tier1.md` (FORMS, REP_FORMS), `IDS_HLD_Tier2.md` (RROW, RCOL, REP_ROW, REP_COLUMN — kèm sửa Table Type), mục 7a/Entities Overview này, `atomic_entities.yaml` (6 dòng, status vẫn draft — không bị khóa), `lld_IDS_FORMS/REP_FORMS/RROW/RCOL/REP_ROW/REP_COLUMN.yaml` (bcv_core_object/bcv_concept). **Lưu ý phát sinh — chưa xử lý:** (a) `Financial Report Catalog` (`REPORT_CATALOG`, T1) là bảng cha của RROW/RCOL, cấu trúc giống hệt (cũng chỉ có mã/tên/loại catalog, không dữ liệu thực) nhưng KHÔNG thuộc phạm vi yêu cầu lần này — vẫn giữ `[Condition] Form Definition` (term không tồn tại), tạo lệch BCO cha (Condition)/con (Documentation, RROW/RCOL). Cần Data Modeler xác nhận có mở rộng đổi luôn `REPORT_CATALOG` sang `[Documentation] Form Document` hay không. (b) `FORMS`/`RCOL`/`RROW` có `design_status: approved` trong LLD — nội dung BCV đã đổi, cần Data Modeler re-review trong App (mirror pattern 7e #19). (c) Docx handover (`atomic-gen-docs`) chưa regen — cần chạy lại nếu đã publish trước đó. |
| 22 | T1/T2 | Theo yêu cầu tường minh Data Modeler (2026-08-27): tách Domain Prefix `Financial Report` cho đúng 3 entity thật sự có tên bảng nguồn `REP_*` — `REP_FORMS` (`Financial Report Sub Template`), `REP_ROW` (`Financial Report Sub Row Template`), `REP_COLUMN` (`Financial Report Sub Column Template`) — sang Domain Prefix mới `Unstructured Financial Report` (viết tắt `ufr`, đăng ký vào `rule_domain_prefix_abbreviations.csv`, giữ nguyên dòng `Financial Report,fr` vì 5 entity còn lại của họ — `FORMS`, `REPORT_CATALOG`, `RROW`, `RCOL`, `DATA` — không đổi). Tên entity đổi thành `Unstructured Financial Report Sub Template`/`Sub Row Template`/`Sub Column Template`; toàn bộ attribute PK/BK/FK có tiền tố cũ (`fr_sub_*`) đổi sang `ufr_sub_*`, bao gồm cả FK `Sub Template Id/Code` trên REP_ROW/REP_COLUMN trỏ tới REP_FORMS. Không entity nào khác FK tới REP_FORMS/REP_ROW/REP_COLUMN (đã grep xác nhận toàn bộ `DataModel/working/Atomic/lld/`) nên phạm vi ảnh hưởng gói gọn trong 3 entity này. | Đã xử lý: `rule_domain_prefix_abbreviations.csv`, `atomic_entities.yaml` (3 dòng), `manifest.yaml` (3 dòng), `classification_schemes.yaml` (4 `used_in_entities`: `IDS_PERIODIC_REPORT_FREQUENCY`, `IDS_PERIODIC_FORM_ROW_DATA_TYPE`, `IDS_PERIODIC_FORM_COLUMN_DATA_TYPE`, `IDS_SYSTEM_CD`), `lld_IDS_REP_FORMS/REP_ROW/REP_COLUMN.yaml`, mục 7a/Entities Overview này + `IDS_HLD_Tier1.md`/`Tier2.md` (6a). **Chưa xử lý**: `aggregate/atomic_attributes.yaml` (build artifact — script `aggregate_atomic.py` hiện bị lỗi crash do 1 entity không liên quan có `domain_prefix` sai định dạng, cần fix riêng trước khi regen toàn cục; không ảnh hưởng tính đúng đắn của các file source-of-truth đã liệt kê ở trên). |
| 23 | T1/T2 | Theo yêu cầu tường minh Data Modeler (2026-08-27): bỏ chữ "Sub" khỏi tên và physical_name của 3 entity vừa đổi Domain Prefix ở #22 — `Unstructured Financial Report Sub Template` → `Unstructured Financial Report Template` (`ufr_sub_template` → `ufr_template`), `Unstructured Financial Report Sub Row Template` → `Unstructured Financial Report Row Template` (`ufr_sub_row_template` → `ufr_row_template`), `Unstructured Financial Report Sub Column Template` → `Unstructured Financial Report Column Template` (`ufr_sub_column_template` → `ufr_column_template`). Toàn bộ attribute PK/BK/FK có tiền tố `ufr_sub_*` đổi sang `ufr_*` tương ứng, bao gồm FK `Template Id/Code` trên REP_ROW/REP_COLUMN trỏ tới REP_FORMS. Không phát sinh trùng tên với 5 entity còn lại của họ `Financial Report` (khác domain prefix: `fr_*` vs `ufr_*`). | Đã xử lý: `atomic_entities.yaml` (3 dòng), `manifest.yaml` (3 dòng), `classification_schemes.yaml` (4 `used_in_entities`), `lld_IDS_REP_FORMS/REP_ROW/REP_COLUMN.yaml`, mục 7a/Entities Overview này + `IDS_HLD_Tier1.md`/`Tier2.md` (6a). **Chưa xử lý** (kế thừa từ #22): `aggregate/atomic_attributes.yaml`. |
...
### 4. Unstructured Financial Report Template
**Tier:** 1 | **Source:** `REP_FORMS` | **BCV Concept:** [Documentation] Form Document | **BCO:** Documentation | **Table Type:** Fundamental
**Domain Prefix:** Unstructured Financial Report
**Description:** Template báo cáo định kỳ (tháng/quý/năm/bán niên) của CTĐC: xác định cấu trúc form cho từng loại kỳ báo cáo. Đổi BCV Concept từ "[Condition] Form Definition" (term không tồn tại trong knowledge/terms.csv) sang "[Documentation] Form Document" (2026-08-13, theo yêu cầu tường minh Data Modeler) — xem 7e #21. Đổi tên từ "Periodic Report Form" (2026-08-05) — xem 7e. Đổi Domain Prefix từ "Financial Report" sang "Unstructured Financial Report" (2026-08-27, theo yêu cầu tường minh Data Modeler — chỉ áp dụng 3 entity REP_FORMS/REP_ROW/REP_COLUMN) — xem 7e #22. Bỏ chữ "Sub" khỏi tên entity/field (2026-08-27, theo yêu cầu tường minh Data Modeler) — xem 7e #23.

### 5. Financial Report Template
**Tier:** 1 | **Source:** `FORMS` | **BCV Concept:** [Documentation] Form Document | **BCO:** Documentation | **Table Type:** Fundamental
**Domain Prefix:** Financial Report
**Description:** Template/form dùng để công bố thông tin (CBTT): xác định cấu trúc, phiên bản, và hệ thống phân cấp form qua self-reference. Đổi BCV Concept từ "[Condition] Form Definition" (term không tồn tại trong knowledge/terms.csv) sang "[Documentation] Form Document" (2026-08-13, theo yêu cầu tường minh Data Modeler) — xem 7e #21. Đổi tên từ "Disclosure Form Definition" (2026-08-05) — xem 7e.
...
**Tier:** 2 | **Source:** `RCOL` | **BCV Concept:** [Documentation] Form Document | **BCO:** Documentation | **Table Type:** Fundamental
**Domain Prefix:** Financial Report
**Description:** Định nghĩa từng cột trong template báo cáo tài chính: mã cột, tên, kỳ tham chiếu (hiện tại/kỳ trước). Đổi BCV Concept từ "[Condition] Form Definition" (term không tồn tại trong knowledge/terms.csv) sang "[Documentation] Form Document" (2026-08-13, theo yêu cầu tường minh Data Modeler) — xem 7e #21. Tên bỏ chữ "Form" (2026-08-05) để khớp physical_name `fr_column_template` và diagram — xem 7e #15.

### 16. Financial Report Row Template
**Tier:** 2 | **Source:** `RROW` | **BCV Concept:** [Documentation] Form Document | **BCO:** Documentation | **Table Type:** Fundamental
**Domain Prefix:** Financial Report
**Description:** Định nghĩa từng hàng trong template báo cáo tài chính: mã hàng, tên, cấp phân cấp, công thức tổng hợp. Đổi BCV Concept từ "[Condition] Form Definition" (term không tồn tại trong knowledge/terms.csv) sang "[Documentation] Form Document" (2026-08-13, theo yêu cầu tường minh Data Modeler) — xem 7e #21. Tên bỏ chữ "Form" (2026-08-05) để khớp physical_name `fr_row_template` và diagram — xem 7e #15.

### 17. Unstructured Financial Report Column Template
**Tier:** 2 | **Source:** `REP_COLUMN` | **BCV Concept:** [Documentation] Form Document | **BCO:** Documentation | **Table Type:** Fundamental
**Domain Prefix:** Unstructured Financial Report
**Description:** Định nghĩa từng cột trong template báo cáo định kỳ. Đổi BCV Concept từ "[Condition] Form Definition" (term không tồn tại trong knowledge/terms.csv) sang "[Documentation] Form Document" (2026-08-13, theo yêu cầu tường minh Data Modeler) — xem 7e #21. Đổi tên từ "Periodic Report Form Column Template" (2026-08-05) — xem 7e #15. Đổi Domain Prefix từ "Financial Report" sang "Unstructured Financial Report" (2026-08-27, theo yêu cầu tường minh Data Modeler — chỉ áp dụng 3 entity REP_FORMS/REP_ROW/REP_COLUMN) — xem 7e #22. Bỏ chữ "Sub" khỏi tên entity/field (2026-08-27, theo yêu cầu tường minh Data Modeler) — xem 7e #23.

### 18. Unstructured Financial Report Row Template
**Tier:** 2 | **Source:** `REP_ROW` | **BCV Concept:** [Documentation] Form Document | **BCO:** Documentation | **Table Type:** Fundamental
**Domain Prefix:** Unstructured Financial Report
**Description:** Định nghĩa từng hàng trong template báo cáo định kỳ. Đổi BCV Concept từ "[Condition] Form Definition" (term không tồn tại trong knowledge/terms.csv) sang "[Documentation] Form Document" (2026-08-13, theo yêu cầu tường minh Data Modeler) — xem 7e #21. Đổi tên từ "Periodic Report Form Row Template" (2026-08-05) — xem 7e #15. Đổi Domain Prefix từ "Financial Report" sang "Unstructured Financial Report" (2026-08-27, theo yêu cầu tường minh Data Modeler — chỉ áp dụng 3 entity REP_FORMS/REP_ROW/REP_COLUMN) — xem 7e #22. Bỏ chữ "Sub" khỏi tên entity/field (2026-08-27, theo yêu cầu tường minh Data Modeler) — xem 7e #23.
...
**Tier:** 2 | **Source:** `LEGAL_ENTITIES`, `AF_LEGAL_REPRESENTATIVE`, `IDENTITY`, `AF_PROFILES`, `COMPANY_PROFILES` | **BCV Concept:** Shared Entity | **BCO:** Involved Party | **Table Type:** Fundamental
**Domain Prefix:** Involved Party
**Description:** Giấy tờ định danh (CMND/CCCD/hộ chiếu) của thực thể pháp lý và người đại diện trong hệ thống IDS. Shared entity — extend source_table IDS. Bao gồm giấy tờ định danh của cổ đông/người nội bộ/người liên quan (nguồn IDENTITY). Bao gồm giấy chứng nhận ĐKKD/đủ điều kiện kinh doanh của công ty kiểm toán (nguồn AF_PROFILES, hardcode type BUSINESS_LICENSE/BUSINESS_ELIGIBILITY_LICENSE). Bao gồm mã số thuế của công ty đại chúng (nguồn COMPANY_PROFILES.BUSINESS_REG_NO, hardcode type TAX_ID) — trước đây map denormalized Text trên Public Company, tách ra shared entity theo yêu cầu Data Modeler (2026-07-15). `LEGAL_REPRESENTATIVE.IDENTITY_NO/ISSUE_DATE/ISSUE_PLACE` KHÔNG còn extend vào shared entity này — giữ denormalized trực tiếp trên `Public Company Legal Representative` do nguồn không đủ thông tin phân biệt loại giấy tờ (quyết định Data Modeler 2026-09-04).
