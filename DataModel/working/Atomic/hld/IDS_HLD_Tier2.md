...
- Con của **Unstructured Financial Report Template** (T1, đổi tên từ Periodic Report Form 2026-08-05, Domain Prefix đổi sang Unstructured Financial Report 2026-08-27, bỏ chữ "Sub" 2026-08-27): Unstructured Financial Report Row/Column Template
...
| Documentation | [Documentation] Form Document | Documentation | `REP_ROW` | Update | Hàng của template báo cáo định kỳ với DATA_TYPE_CD phân biệt loại dữ liệu. | Unstructured Financial Report Row Template | Fundamental | (1) Term candidate: `[Documentation] Form Document` (BCV id 9348) — thành phần cấu trúc (detail) của bản mẫu Form Document `Unstructured Financial Report Template`. (2) Cấu trúc trường: ROW_CODE, ROW_NAME, DATA_TYPE_CD, display_order — chỉ metadata layout, không có dữ liệu nghiệp vụ thực. (3) Chọn `[Documentation] Form Document`, thay cho `[Condition] Form Definition` — term "Form Definition" không tồn tại trong `knowledge/terms.csv`; "Form Document" là term thật khớp chính xác. Đổi theo yêu cầu tường minh Data Modeler (2026-08-13) — xem Overview 7e #21. FK → Unstructured Financial Report Template (nay cũng Documentation — không phát sinh lệch BCO cha/con). Table Type sửa `Relative` → `Fundamental` để đồng bộ với Overview 7a/Entities/LLD/manifest (đã là Fundamental ở các nguồn đó — lệch tài liệu tồn tại từ trước, không liên quan đổi BCO). Đổi tên entity từ `Periodic Report Form Row Template` (2026-08-05). Domain Prefix đổi từ `Financial Report` sang `Unstructured Financial Report` (`ufr`) (2026-08-27) — xem Overview 7e #22. Bỏ chữ "Sub" khỏi tên entity (2026-08-27) — xem Overview 7e #23. |
| Documentation | [Documentation] Form Document | Documentation | `REP_COLUMN` | Update | Cột của template báo cáo định kỳ với DATA_TYPE_CD phân biệt loại dữ liệu. | Unstructured Financial Report Column Template | Fundamental | (1) Term candidate: `[Documentation] Form Document` (BCV id 9348) — thành phần cấu trúc (detail) của bản mẫu Form Document `Unstructured Financial Report Template`. (2) Cấu trúc trường: COL_CODE, COL_NAME, DATA_TYPE_CD, display_order — chỉ metadata layout, không có dữ liệu nghiệp vụ thực. (3) Chọn `[Documentation] Form Document` — lý do tương tự REP_ROW (xem ghi chú dòng trên). FK → Unstructured Financial Report Template (nay cũng Documentation — không phát sinh lệch BCO cha/con). Table Type sửa `Relative` → `Fundamental` để đồng bộ với Overview 7a/Entities/LLD/manifest (lệch tài liệu tồn tại từ trước, không liên quan đổi BCO). Đổi tên entity từ `Periodic Report Form Column Template` (2026-08-05). Domain Prefix đổi từ `Financial Report` sang `Unstructured Financial Report` (`ufr`) (2026-08-27) — xem Overview 7e #22. Bỏ chữ "Sub" khỏi tên entity (2026-08-27) — xem Overview 7e #23. |
...
| Involved Party | Shared Entity | Shared Entity | `LEGAL_ENTITIES`, `AF_LEGAL_REPRESENTATIVE` | Update | Giấy tờ định danh của thực thể pháp lý và người đại diện (CMND/CCCD/Hộ chiếu/ĐKKD). | Involved Party Alternative Identification | Fundamental | Shared entity đã approved từ NHNCK — bổ sung source_table IDS. Giấy tờ: IDENTITY_TYPE_CD trên LEGAL_ENTITIES; ID_NO trên AF_LEGAL_REPRESENTATIVE. Chi tiết giấy tờ từ bảng IDENTITY (Tier 3). `LEGAL_REPRESENTATIVE.IDENTITY_NO/ISSUE_DATE/ISSUE_PLACE` KHÔNG extend vào shared entity này — giữ denormalized trên `Public Company Legal Representative` (xem T2-08). |
...
| T2-08 | `LEGAL_REPRESENTATIVE.IDENTITY_NO` và `AF_LEGAL_REPRESENTATIVE.IDENTITY_NO` không có cột phân biệt loại giấy tờ (CMND/CCCD/Hộ chiếu). | Đã xử lý (2026-09-04) cho `LEGAL_REPRESENTATIVE` — theo yêu cầu tường minh Data Modeler, KHÔNG tách shared entity IP Alt Identification (không đủ thông tin để xác định type). `IDENTITY_NO/ISSUE_DATE/ISSUE_PLACE` map denormalized trực tiếp trên `Public Company Legal Representative` (`lld_IDS_LEGAL_REPRESENTATIVE.yaml`); file shared entity riêng đã xóa. `AF_LEGAL_REPRESENTATIVE.IDENTITY_NO` giữ nguyên hiện trạng — vẫn tạm dùng default `NATIONAL_ID` theo quy tắc trường hợp đặc biệt (`reference/shared_entity_schemas.md`), status = pending trên attribute `Identification Type Code`, cần Data Modeler xác nhận loại giấy tờ thực tế riêng. |
