...
| Condition | [Condition] Form Definition | Condition | `RROW` | Update | Hàng của template báo cáo tài chính: loại hàng (value/formula/description), mã hàng, tên, công thức. | Financial Report Row Template | Fundamental | (1) Term candidate: `[Condition] Form Definition` — thành phần cấu trúc của template BCTC. (2) Cấu trúc trường: ROW_CODE, ROW_NAME, ROW_TYPE_CD, formula, display_order. (3) Chọn `[Condition] Form Definition`. Table Type = Fundamental theo quyết định chủ đích của Data Modeler (2026-08-05, đồng bộ với pattern override đã áp dụng cho ~30 entity Tier 2/3/4 khác — xem 7e #7) dù còn FK → Financial Report Catalog (T1); tên entity bỏ chữ "Form" để khớp diagram 6c/Overview 7b và physical_name `fnc_rpt_row_tpl` (đồng nhất với bản LinhLV tham chiếu). |
| Condition | [Condition] Form Definition | Condition | `RCOL` | Update | Cột của template báo cáo tài chính: thường là kỳ báo cáo (năm hiện tại, năm trước). | Financial Report Column Template | Fundamental | (1) Term candidate: `[Condition] Form Definition` — thành phần cấu trúc của template BCTC. (2) Cấu trúc trường: COL_CODE, COL_NAME, COL_TYPE_CD, display_order. (3) Chọn `[Condition] Form Definition`. Table Type = Fundamental — lý do tương tự RROW (xem ghi chú dòng trên). Tên entity bỏ chữ "Form" để khớp diagram 6c/Overview 7b và physical_name `fnc_rpt_clmn_tpl`. |
| Condition | [Condition] Form Definition | Condition | `REP_ROW` | Update | Hàng của template báo cáo định kỳ với DATA_TYPE_CD phân biệt loại dữ liệu. | Financial Report Sub Row Template | Relative | (1) Term candidate: `[Condition] Form Definition` — thành phần cấu trúc của template báo cáo định kỳ. (2) Cấu trúc trường: ROW_CODE, ROW_NAME, DATA_TYPE_CD, display_order. (3) Chọn `[Condition] Form Definition`. Relative (FK → Financial Report Sub Template). Đổi tên từ `Periodic Report Form Row Template` (2026-08-05). |
| Condition | [Condition] Form Definition | Condition | `REP_COLUMN` | Update | Cột của template báo cáo định kỳ với DATA_TYPE_CD phân biệt loại dữ liệu. | Financial Report Sub Column Template | Relative | (1) Term candidate: `[Condition] Form Definition` — thành phần cấu trúc của template báo cáo định kỳ. (2) Cấu trúc trường: COL_CODE, COL_NAME, DATA_TYPE_CD, display_order. (3) Chọn `[Condition] Form Definition`. Relative (FK → Financial Report Sub Template). Đổi tên từ `Periodic Report Form Column Template` (2026-08-05). |
