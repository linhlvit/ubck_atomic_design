...
|---|---|---|---|---|---|---|---|
| Involved Party | [Involved Party] Branch | Involved Party | SC_FIRM_BRANCH | Chi nhánh của CTCK trong nước (có địa chỉ, giấy phép, trạng thái riêng) | Securities Company Branch | Relative | (1) BCV có `Branch` trong Involved Party — đơn vị thành viên có địa chỉ pháp lý riêng của một Involved Party lớn hơn. (2) SC_FIRM_BRANCH lưu chi nhánh CTCK: địa chỉ, tỉnh/huyện/xã, giấy phép, ngày hoạt động, trạng thái — đây là Involved Party con của Securities Company. (3) Chọn `[Involved Party] Branch`. |
| Involved Party | [Involved Party] Branch | Involved Party | SC_FIRM_REP_OFFICE | Văn phòng đại diện trong nước của CTCK (VPDD nội địa) | Securities Company Representative Office | Relative | (1) BCV có `Representative Office` trong Involved Party. (2) SC_FIRM_REP_OFFICE có FK đến cả SC_FIRM_INFO và SC_FIRM_BRANCH — VPDD có thể trực thuộc hội sở hoặc chi nhánh. (3) Chọn `[Involved Party] Representative Office`. |
| Involved Party | [Involved Party] Branch | Involved Party | SC_FIRM_TRANSACTION_OFFICE | Phòng giao dịch của CTCK (PGD — đơn vị phụ thuộc chi nhánh hoặc hội sở) | Securities Company Transaction Office | Relative | (1) BCV có `Branch` hoặc `Transaction Office` trong Involved Party. (2) SC_FIRM_TRANSACTION_OFFICE có FK đến SC_FIRM_INFO và SC_FIRM_BRANCH — PGD là đơn vị nhỏ nhất trực tiếp giao dịch với KH. (3) Chọn `[Involved Party] Branch` vì không có term Transaction Office riêng trong BCV; entity name phân biệt bằng từ Transaction Office. |
| Involved Party | [Involved Party] Branch | Involved Party | SC_FIRM_DOMESTIC_REP_OFFICE | Văn phòng đại diện trong nước (bảng thứ hai — cấu trúc khác SC_FIRM_REP_OFFICE) | Securities Company Domestic Representative Office | Relative | (1) SCMS có 2 bảng VPDD nội địa: SC_FIRM_REP_OFFICE và SC_FIRM_DOMESTIC_REP_OFFICE. (2) SC_FIRM_DOMESTIC_REP_OFFICE có cấu trúc khác (không có FK SC_FIRM_BRANCH_ID, chỉ FK SC_FIRM_INFO_ID + CAT_NATIONALITY_ID) — có thể là VPDD của tổ chức nước ngoài tại VN. (3) Giữ là entity riêng để tránh nhầm lẫn dữ liệu. |
...
| Involved Party | [Involved Party] Representative Office | Involved Party | SC_FIRM_FOREIGN_REP_OFFICE | Văn phòng đại diện CTCK nước ngoài tại Việt Nam | Securities Company Foreign Representative Office | Relative | (1) BCV có `Representative Office` Involved Party. (2) SC_FIRM_FOREIGN_REP_OFFICE lưu VPDD của CTCK nước ngoài: công ty mẹ, giấy phép, thời hạn hoạt động, phạm vi hoạt động. (3) Chọn `[Involved Party] Representative Office`. |
| Involved Party | [Involved Party] Representative Office | Involved Party | SC_FIRM_FOREIGN_REP_OFFICE_VN | Văn phòng đại diện CTCK nước ngoài — phiên bản tại Việt Nam (cấu trúc khác SC_FIRM_FOREIGN_REP_OFFICE) | Securities Company Foreign Representative Office VN | Relative | (1) Tương tự SC_FIRM_FOREIGN_REP_OFFICE nhưng có thêm BUSINESS_LICENSE (giấy phép VN) và REP_OFFICE_NAME. Có thể là VPDD đã đăng ký tại VN (khác với VPDD đang đăng ký). (2) Giữ entity riêng — cấu trúc khác và FK khác (không có SC_FIRM_INFO_ID trực tiếp). |
...
| Involved Party | [Involved Party] Registered Securities Practitioner | Involved Party | SC_FIRM_LICENSED_PRACTITIONER | Người hành nghề chứng khoán đang công tác tại CTCK (từ hệ thống SCMS) | Securities Practitioner | Relative | (1) BCV có `Registered Securities Practitioner` trong Involved Party — đây là entity đã LOCKED tại NHNCK với tên `Securities Practitioner`. (2) SC_FIRM_LICENSED_PRACTITIONER lưu thông tin người HNCK tại CTCK: số CCHN, ngày cấp, trạng thái làm việc, FK đến SC_FIRM_INFO. (3) Extend source_table của entity `Securities Practitioner` (LOCKED); không tạo entity mới. |
| Involved Party | [Involved Party] Auditor | Involved Party | AUDITOR | Kiểm toán viên được giao kiểm toán CTCK (cá nhân, trực thuộc AUDIT_FIRM) | Securities Company Auditor | Relative | (1) BCV có `Auditor` trong Involved Party — cá nhân có chứng chỉ kiểm toán. (2) AUDITOR có FK AUDIT_FIRM_ID → AUDIT_FIRM.ID — là Involved Party cá nhân gắn với công ty kiểm toán. (3) Chọn `[Involved Party] Auditor`. |
| Arrangement | [Arrangement] Securities Service Agreement | Arrangement | CUSTODIAN_BANK | Ngân hàng lưu ký được CTCK chỉ định (quan hệ CTCK—Ngân hàng lưu ký) | Securities Company Custodian Bank | Relative | (1) BCV có `Securities Service Agreement` hoặc `Custodian Arrangement` trong Arrangement. (2) CUSTODIAN_BANK không phải thực thể Ngân hàng — mà là quan hệ giữa CTCK và Ngân hàng lưu ký (SC_FIRM_INFO_ID + BANK_ID). Đây là arrangement/thỏa thuận lưu ký. (3) Chọn `[Arrangement] Securities Service Agreement` — mô tả quan hệ lưu ký. |
| Arrangement | [Arrangement] Service License | Arrangement | LNK_SC_FIRM_SERVICE | Dịch vụ chứng khoán được CTCK cấp phép (có LICENSE_NUMBER, LICENSE_DATE — không phải pure junction) | Securities Company Licensed Service | Relative | (1) BCV có `Service License` hoặc `Authorization` trong Arrangement — thỏa thuận/giấy phép dịch vụ. (2) LNK_SC_FIRM_SERVICE có SC_FIRM_INFO_ID + CAT_SERVICE_ID + LICENSE_NUMBER + LICENSE_DATE — không phải pure junction. Đây là giấy phép/thỏa thuận dịch vụ. (3) Chọn `[Arrangement] Service License`. |
| Event | [Event] Transaction | Event | SC_FIRM_PERIODIC_REPORT | Báo cáo định kỳ CTCK nộp lên UBCKNN (mỗi kỳ = 1 event nộp báo cáo) | Securities Company Periodic Report | Relative | (1) BCV có `Transaction` hoặc `Submission` trong Event — sự kiện nộp tài liệu. (2) SC_FIRM_PERIODIC_REPORT ghi nhận từng lần nộp báo cáo định kỳ: REPORT_YEAR, PERIOD, RECORD_STATUS (0=chưa gửi,1=đã gửi,2=đã duyệt). Đây là event nộp báo cáo, có trạng thái lifecycle. (3) Chọn `[Event] Transaction`. |
| Event | [Event] Transaction | Event | SC_FIRM_ADHOC_REPORT | Báo cáo đột xuất/bất thường CTCK nộp lên UBCKNN | Securities Company Adhoc Report | Relative | (1) Tương tự SC_FIRM_PERIODIC_REPORT — event nộp báo cáo đột xuất theo yêu cầu hoặc bất thường. (2) SC_FIRM_ADHOC_REPORT có RECORD_STATUS, EVENT_TYPE_ID, DETAIL_TYPE — mô tả event nghiệp vụ. (3) Chọn `[Event] Transaction`. |
| Event | [Event] Communication | Event | DISCLOSURE_REPORT | Báo cáo công bố thông tin (CBTT) của CTCK | Securities Company Disclosure Report | Relative | (1) BCV có `Communication` trong Event — sự kiện truyền đạt thông tin ra bên ngoài. (2) DISCLOSURE_REPORT = sự kiện CBTT: gắn SC_FIRM_INFO_ID, loại CBTT, nội dung. (3) Chọn `[Event] Communication`. |
...
| Event | [Event] Business Activity | Event | ALERT_RUN | Lần chạy cảnh báo của hệ thống (batch run kiểm tra ngưỡng) | Securities Company Alert Run | Fact Append | (1) BCV có `Business Activity` cho hoạt động batch/run của hệ thống. (2) ALERT_RUN ghi nhận từng lần chạy cảnh báo: thời điểm chạy, chỉ tiêu, trạng thái. Insert-only. (3) Chọn `[Event] Business Activity`, Fact Append. |
| Arrangement | [Arrangement] Assessment Assignment | Arrangement | RISK_REPORTING_PERIOD_SC_FIRM | Gán kỳ đánh giá rủi ro cho từng CTCK cụ thể (CTCK nào tham gia kỳ nào) | Securities Company Risk Reporting Period Assignment | Relative | (1) BCV có `Assessment Assignment` hoặc `Reporting Assignment` trong Arrangement — quan hệ giao kỳ báo cáo cho Involved Party. (2) RISK_REPORTING_PERIOD_SC_FIRM có RISK_REPORTING_PERIOD_ID + SC_FIRM_INFO_ID — quan hệ N:N giữa CTCK và kỳ báo cáo, có thể có thêm attribute. (3) Chọn `[Arrangement] Assessment Assignment`. |
...
erDiagram
    ...
    SC_FIRM_INFO ||--o{ RISK_REPORTING_PERIOD_SC_FIRM : "SC_FIRM_INFO_ID"
    ...
    RISK_REPORTING_PERIOD ||--o{ RISK_REPORTING_PERIOD_SC_FIRM : "RISK_REPORTING_PERIOD_ID"
```
...
erDiagram
    ...
    Securities_Company_Audit_Firm {
        bigint ds_audit_firm_id PK
        string audit_firm_code
    }
    Securities_Company_Settlement_Bank {
    ...
    Securities_Company_Branch {
        bigint ds_sc_firm_branch_id PK
        bigint securities_company_id FK
        string securities_company_code
        string branch_code
        date license_date
        string ds_branch_status_code
    }
    Securities_Company_Representative_Office {
        bigint ds_sc_firm_rep_office_id PK
        bigint securities_company_id FK
        string securities_company_code
        bigint securities_company_branch_id FK
    }
    Securities_Company_Transaction_Office {
        bigint ds_sc_firm_transaction_office_id PK
        bigint securities_company_id FK
        string securities_company_code
        bigint securities_company_branch_id FK
    }
    ...
    Securities_Company_Foreign_Representative_Office {
        bigint ds_sc_firm_foreign_rep_office_id PK
        bigint securities_company_id FK
        string securities_company_code
    }
    ...
    Securities_Company_Auditor {
        bigint ds_auditor_id PK
        bigint securities_company_audit_firm_id FK
        string securities_company_audit_firm_code
    }
    Securities_Company_Custodian_Bank {
        bigint ds_custodian_bank_id PK
        bigint securities_company_id FK
        string securities_company_code
        bigint securities_company_settlement_bank_id FK
    }
    Securities_Company_Licensed_Service {
        bigint ds_licensed_service_id PK
        bigint securities_company_id FK
        string securities_company_code
        string ds_service_type_code
        string license_number
        date license_date
    }
    ...
    Securities_Company_Risk_Reporting_Period_Assignment {
        bigint ds_id PK
        bigint securities_company_id FK
        bigint securities_company_risk_reporting_period_id FK
    }

    Securities_Company ||--o{ Securities_Company_Branch : "securities_company_id"
    Securities_Company ||--o{ Securities_Company_Representative_Office : "securities_company_id"
    Securities_Company ||--o{ Securities_Company_Transaction_Office : "securities_company_id"
    Securities_Company ||--o{ Securities_Company_Foreign_Branch : "securities_company_id"
    Securities_Company ||--o{ Securities_Company_Foreign_Representative_Office : "securities_company_id"
    Securities_Company ||--o{ Securities_Company_Senior_Personnel : "securities_company_id"
    Securities_Company_Audit_Firm ||--o{ Securities_Company_Auditor : "securities_company_audit_firm_id"
    ...
    Securities_Company_Risk_Reporting_Period ||--o{ Securities_Company_Risk_Reporting_Period_Assignment : "securities_company_risk_reporting_period_id"
```
