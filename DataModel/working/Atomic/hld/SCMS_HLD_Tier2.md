...
| 4 | `BM_BAO_CAO_LS` — Audit Log nguồn hay ETL thành Status History? | ✅ **Audit Log nguồn, ngoài scope Atomic.** Cơ chế ghi lịch sử đặc thù source system, không phải sự kiện nghiệp vụ. |
...
| Junction / derivative | BM_TIEUDE_HANG_COT | Danh sách tiêu đề thiết kế động | Metadata layout báo cáo (FIRST_ROW, LAST_ROW, COL_SPAN...) — cấu hình hiển thị UI, không có giá trị nghiệp vụ Atomic |
