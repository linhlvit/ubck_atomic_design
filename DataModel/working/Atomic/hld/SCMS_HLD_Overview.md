...
> - [SCMS_HLD_Tier1.md](SCMS_HLD_Tier1.md) — Entity độc lập: Securities Company, Securities Company Audit Firm, Securities Company Settlement Bank, Risk/Alert Indicators & Period, Classification SCMS Firm Status/Service/Nationality/Event Type, Securities Company Form Report (Geographic Area đã chuyển sang nguồn ECAT — xem mục 7f)
...
| 1 | Common | [Common] Firm Status | — | CAT_SC_FIRM_STATUS | Append | Danh mục trạng thái pháp lý CTCK/Chi nhánh/VPĐD/PGD/Ngân hàng | Classification SCMS Firm Status | Classification | (1) Term gần nhất trong BCV: `Organization Life Cycle Status` (id 10930, Involved Party) / `Organization Registration Status` (id 11478) — mô tả vòng đời/đăng ký của 1 Organization. (2) Bảng: SC_FIRM_STATUS_CODE/NAME, REPORT_SUBMISSION_ENABLED, DISCLOSURE_ENABLED, APPLICABLE_ENTITY (CTCK/CN/VPĐD/NH/Cả hai) — danh mục dùng chung nhiều loại đối tượng. (3) Theo chỉ đạo Data Modeler: gán Common (không dùng Involved Party dù match khá tốt) để nhất quán pattern NHNCK (Classification NHNCK Application Status/Document/Specialization). Table Type = Classification theo chỉ đạo. Xem 7e #11. |
| 1 | Common | [Common] Service | — | CAT_SERVICE | Append | Danh mục dịch vụ chứng khoán được cấp phép | Classification SCMS Service | Classification | (1) Term BCV: `Service` (id 11846, Product) / `Service Type` (id 11852). (2) Bảng: SERVICE_CODE/NAME, NOTE, RECORD_STATUS — danh mục Code+Name+ghi chú. (3) Theo chỉ đạo Data Modeler: gán Common (không dùng Product dù match tốt). Table Type = Classification. Xem 7e #11. |
| 1 | Common | [Common] Nationality | — | CAT_NATIONALITY | Append | Danh mục quốc tịch | Classification SCMS Nationality | Classification | (1) Không có term BCV chính xác "Nationality"; gần nhất `Citizenship` (Involved Party) / `Country` (Location). (2) Bảng: NATIONALITY_CODE/NAME, NOTE, RECORD_STATUS — Code+Name thuần. (3) Gán Common theo chỉ đạo — match tự nhiên hơn vì không có term khớp sẵn. Table Type = Classification. Xem 7e #11. |
| 1 | Common | [Common] Event Type | — | CAT_EVENT_TYPE | Update | Danh mục loại sự kiện/sự vụ nghiệp vụ làm cơ sở xác định nghĩa vụ báo cáo/CBTT | Classification SCMS Event Type | Relative | (1) Term BCV khớp: `Event Type` (id 9924, Event). (2) Bảng: EVENT_TYPE_CODE/NAME, REQUIRES_LICENSE, REQUIRES_DISCLOSURE, OBLIGATION_TYPE, EVENT_CATEGORY, CYCLE, FREQUENCY. (3) Theo chỉ đạo Data Modeler: gán Common (không dùng Event dù match mạnh). Table Type = Relative (khác 3 bảng CAT_ trên) theo chỉ đạo — không FK rõ ràng đến Fundamental khác, xem 7e #11/#12. |
| 1 | Common | [Common] Business Line | — | CAT_BUSINESS_LINE | Append | Danh mục nghiệp vụ kinh doanh chứng khoán được UBCKNN cấp phép cho CTCK | Classification SCMS Business Line | Classification | (1) Term BCV gần nhất `Line Of Business` (id 11422, Involved Party) không khớp hoàn toàn (phân loại theo khách hàng, không theo nghiệp vụ cấp phép); `Industry Classification` (id 8291, Common) đã dùng cho entity `Classification ECAT Business Line` tại ECAT — không tái sử dụng vì khác cấu trúc/ý nghĩa. (2) Bảng: BUSINESS_LINE_CODE/NAME, NOTE, RECORD_STATUS, LEGAL_CAPITAL (vốn pháp định tối thiểu theo nghiệp vụ) — gần giống CAT_SERVICE nhưng thêm LEGAL_CAPITAL. (3) Theo chỉ đạo Data Modeler (tiền lệ T1-05): gán Common, đặt tên `Classification SCMS Business Line` theo convention đặt tên hệ thống nguồn ngay sau "Classification" — áp dụng nhất quán cho mọi Classification entity đa nguồn (không chỉ riêng để phân biệt với `Classification ECAT Business Line`, entity này cũng đã đổi tên theo cùng convention). Table Type = Classification. Xem 7e #16. |
...
**Tổng: 54 Atomic entities** (T1: 15 entities, T2: 34 entities, T3: 7 entities — bao gồm 5 entity bổ sung ở lượt trước: Classification SCMS Firm Status, Classification SCMS Service, Classification SCMS Nationality, Classification SCMS Event Type, Securities Company Form Report (Tier 1); 1 entity bổ sung lượt trước đó: Securities Company Alert Run (Tier 2, đảo ngược quyết định loại-scope ALERT_RUN); và 1 entity mới bổ sung ở lượt này: Classification SCMS Business Line (Tier 1, nâng cấp từ Classification Value CAT_BUSINESS_LINE, theo tiền lệ T1-05))
...
graph TD
    ...
    CLS_STATUS["**Classification SCMS Firm Status**\n(CAT_SC_FIRM_STATUS)"]:::atomic
    CLS_SERVICE["**Classification SCMS Service**\n(CAT_SERVICE)"]:::atomic
    CLS_NAT["**Classification SCMS Nationality**\n(CAT_NATIONALITY)"]:::atomic
    CLS_EVT["**Classification SCMS Event Type**\n(CAT_EVENT_TYPE)"]:::atomic
    CLS_BIZ_LINE["**Classification SCMS Business Line**\n(CAT_BUSINESS_LINE)"]:::atomic
...
| CAT_SC_FIRM_STATUS | Danh mục trạng thái pháp lý CTCK/CN/VPĐD/PGD | **Đã nâng cấp lên Atomic entity thật** | ~~Scheme: SCMS_SC_FIRM_STATUS (deprecated)~~ → entity `Classification SCMS Firm Status` (Tier 1, xem 7a + Entities). |
| CAT_SERVICE | Danh mục dịch vụ chứng khoán được cấp phép | **Đã nâng cấp lên Atomic entity thật** | ~~Scheme: SCMS_SERVICE_TYPE (deprecated)~~ → entity `Classification SCMS Service` (Tier 1, xem 7a + Entities). |
| CAT_BUSINESS_LINE | Danh mục nghiệp vụ kinh doanh chứng khoán | **Đã nâng cấp lên Atomic entity thật** | ~~Scheme: SCMS_BUSINESS_LINE (deprecated)~~ → entity `Classification SCMS Business Line` (Tier 1, xem 7a + Entities). |
| CAT_NATIONALITY | Danh mục quốc tịch | **Đã nâng cấp lên Atomic entity thật** | ~~Scheme: SCMS_NATIONALITY (deprecated)~~ → entity `Classification SCMS Nationality` (Tier 1, xem 7a + Entities). |
...
| CAT_EVENT_TYPE | Danh mục loại sự kiện/loại văn bản | **Đã nâng cấp lên Atomic entity thật** | ~~Scheme: SCMS_EVENT_TYPE (deprecated)~~ → entity `Classification SCMS Event Type` (Tier 1, table_type Relative, xem 7a + Entities). |
...
|---|---|---|---|
| LNK_SC_FIRM_BUSINESS_LINE | Liên kết CTCK với nghiệp vụ kinh doanh (SC_FIRM_INFO_ID + CAT_BUSINESS_LINE_ID + RECORD_STATUS) | Securities Company | Pure junction giữa 2 Atomic entity (CAT_BUSINESS_LINE nay là entity thật `Classification SCMS Business Line` — xem 7c/7e #16). Denormalize thành `business_lines ARRAY<STRUCT<business_line_id BIGINT, business_line_code STRING>>` trên Securities Company (bên Many). ETL filter RECORD_STATUS = 1 (active only) trước khi denormalize. |
...
| LNK_PRACTITIONER_BUSINESS_LINE | Liên kết người HNCK (SCMS) với nghiệp vụ kinh doanh CK (LICENSED_PRACTITIONER_ID + CAT_BUSINESS_LINE_ID — không có attribute) | Securities Company Practitioner | **Sửa lại lượt này** — entity tiêu thụ trước đây ghi nhầm "Securities Practitioner" (NHNCK), không khớp quyết định tách 2 entity Practitioner riêng biệt đã chốt 2026-07-09 (xem Tier2 T2-06); LICENSED_PRACTITIONER_ID thực chất trỏ đến `SCMS.SC_FIRM_LICENSED_PRACTITIONER`. Pure junction giữa 2 Atomic entity (CAT_BUSINESS_LINE nay là `Classification SCMS Business Line`). Denormalize thành `business_lines ARRAY<STRUCT<business_line_id BIGINT, business_line_code STRING>>` trên Securities Company Practitioner. |
...
| 14 | T2 | `SERVICE_ID` trên `Securities Company Licensed Service` (entity #48/SC_FIRM_SERVICE) dùng pattern Classification Value đơn (scheme `SCMS_SERVICE_TYPE`, nay deprecated) — cần đổi sang FK Id+Code khi `Classification SCMS Service` được thiết kế. | **Đã thực hiện trong lượt này** — `lld_SCMS_SC_FIRM_SERVICE.yaml` đã sửa: attribute `Service Code` → cặp `Classification SCMS Service Id` + `Classification SCMS Service Code`. Các entity tiêu thụ khác của 4 scheme deprecated (Securities Company, entity chi nhánh/VPĐD/PGD, Securities Company Custodian Bank, 7 bảng dùng Nationality, Securities Company Profile Change, Securities Company Disclosure Report) — CHƯA sửa, để LLD hóa sau (xem SCMS_HLD_Tier1.md 6f T1-08). |
...
| 16 | T1 | `CAT_BUSINESS_LINE` — nâng cấp từ Classification Value (scheme `SCMS_BUSINESS_LINE`) lên Atomic entity thật, theo đúng tiền lệ #11 (4 entity Classification khác). Đặt tên `Classification SCMS Business Line` theo convention chèn tên hệ thống nguồn ngay sau "Classification" — áp dụng chung cho mọi Classification entity đa nguồn, không riêng để phân biệt với entity `Classification ECAT Business Line` đã có tại nguồn **ECAT** (khác BCV Concept — `Industry Classification` tổng quát, self-referencing — xem ECAT_HLD_Tier1.md 6a; entity này cũng đã đổi tên theo cùng convention). | **Quyết định Data Modeler (task này) — theo đúng tiền lệ #11/T1-05.** BCV Core Object = Common, Table Type = Classification. Xem SCMS_HLD_Tier1.md 6f T1-09. |
| 17 | T1/T2 | Mục 7d ghi nhận `LNK_PRACTITIONER_BUSINESS_LINE` với entity tiêu thụ sai tên "Securities Practitioner" (NHNCK) — không khớp quyết định tách 2 entity Practitioner riêng biệt (SCMS vs NHNCK) đã chốt 2026-07-09 (xem Tier2 T2-06). `LICENSED_PRACTITIONER_ID` thực chất trỏ đến `SCMS.SC_FIRM_LICENSED_PRACTITIONER`. | **Đã sửa trong lượt này** — entity tiêu thụ đổi thành `Securities Company Practitioner`. Đồng thời cả 2 junction (`LNK_SC_FIRM_BUSINESS_LINE`, `LNK_PRACTITIONER_BUSINESS_LINE`) đổi denormalize pattern từ `ARRAY<STRING>` sang `ARRAY<STRUCT>` Id+Code, do `CAT_BUSINESS_LINE` nay là Atomic entity thật, không còn Classification Value đơn — xem 7d. |
...
### 49. Classification SCMS Firm Status
...
### 50. Classification SCMS Service
**Tier:** 1 | **Source:** `SCMS.CAT_SERVICE` | **BCV Concept:** [Common] Service | **BCO:** Common | **Table Type:** Classification
**Domain Prefix:** Classification
**Description:** Danh mục dịch vụ chứng khoán được UBCKNN cấp phép cho CTCK. Nâng cấp từ Classification Value (scheme SCMS_SERVICE_TYPE) lên entity thật. FK target cho Classification SCMS Service Id trên Securities Company Licensed Service. Xem 7e #11/#14.


### 51. Classification SCMS Nationality
...
### 52. Classification SCMS Event Type
...
### 55. Classification SCMS Business Line
**Tier:** 1 | **Source:** `SCMS.CAT_BUSINESS_LINE` | **BCV Concept:** [Common] Business Line | **BCO:** Common | **Table Type:** Classification
**Domain Prefix:** Classification
**Description:** Danh mục nghiệp vụ kinh doanh chứng khoán được UBCKNN cấp phép cho CTCK (môi giới, tự doanh, bảo lãnh phát hành, tư vấn đầu tư, quản lý quỹ...) — mã, tên, vốn pháp định tối thiểu theo quy định pháp luật cho từng nghiệp vụ. Nâng cấp từ Classification Value (scheme SCMS_BUSINESS_LINE) lên entity thật; đặt tên có "SCMS" theo convention chèn tên hệ thống nguồn ngay sau "Classification", áp dụng chung cho mọi Classification entity đa nguồn — không riêng để phân biệt với entity `Classification ECAT Business Line` tại nguồn ECAT (Industry Classification tổng quát, self-referencing). Xem 7e #16.
