...
> **Phụ thuộc Tier 1/2/3:** Fund Management Company, Investment Fund, Foreign Fund Management Organization Unit, Fund Management Company Employee, Member Rating, Foreign Fund Management Organization Unit Staff, Custodian Bank, Member Warning Parameter, Member Warning Condition
...
graph LR
    ...
    EVENT_TYPE["**EVENT_TYPE** (Tier 1, Classification FMS Event Type entity)"]:::outscope
...
graph TD
    ...
    MDA -->|"Fund Management Company Employee FK (nullable)"| KP
...
| ~~EVENT_TYPE (bảng danh mục)~~ | ~~Danh mục loại sự vụ báo cáo/CBTT~~ | ~~`FMS_EVENT_TYPE`~~ | **[GIẢI QUYẾT 2026-07-19]** Promote lên entity riêng `Classification FMS Event Type` (Tier 1) — xem T6-06. FK từ Member Disclosure Announcement (ANNOUNCE.EVENT_TYPE_ID) đổi sang cặp Id/Code trỏ entity mới. |
...
| T6-06 | **[GIẢI QUYẾT 2026-07-19]** ~~EVENT_TYPE (bảng danh mục loại sự vụ) có nhiều cột cấu hình nghiệp vụ hơn danh mục thuần (CATEGORY, OBLIGATION_TYPE, DEADLINE_CALC_METHOD, METADATA JSON) — không chỉ Code+Name. Xác nhận: xử lý Classification Value mở rộng (scheme FMS_EVENT_TYPE, các cột cấu hình để LLD lưu thêm ngoài Code+Name) có đủ hay cần entity Fundamental riêng?~~ Đã promote lên entity riêng `Classification FMS Event Type` (Tier 1, Relative) — xem FMS_HLD_Overview.md 7a/7c/Entities #46, FMS_HLD_Tier1.md 6a. Scheme `FMS_EVENT_TYPE` đánh dấu deprecated. FK từ Member Disclosure Announcement (ANNOUNCE.EVENT_TYPE_ID, entity này) đổi từ Classification Value đơn sang cặp FK Id/Code — xem T6-05 nếu liên quan. | Đã cập nhật — không còn là điểm cần xác nhận. LLD ANNOUNCE cần dùng FK Id/Code mới. |
