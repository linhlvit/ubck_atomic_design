...
> - [FMS_HLD_Tier3.md](FMS_HLD_Tier3.md) — FK đến Tier 2: Foreign FM Org Unit Staff, Investment Fund Representative Board Member, Investment Fund Investor Membership, Discretionary Investment Account, Fund Management Company Employee X Classification FMS Position Relationship, Investment Fund X Fund Management Company Employee Relationship, Investment Fund X Fund Distribution Agent Relationship
...
| 3 | Arrangement | [Arrangement] Investment Fund | Investment Fund | FUND_TL_PRO | Update | Người hành nghề liên quan đến quỹ đầu tư (junction) | Investment Fund X Fund Management Company Employee Relationship | Relative | Investment Fund — **[MỚI 2026-07-19]** BCV Concept tạm dùng (tái sử dụng concept của MB_FUND), cần tra lại trước LLD. Trước đây ở 7f nhóm Junction. FK đến Investment Fund (Tier 2) + Fund Management Company Employee (Tier 2). |
| 3 | Arrangement | [Arrangement] Investment Fund | Investment Fund | AGEN_FUNDS | Update | Bảng trung gian đại lý và quỹ đầu tư (junction) | Investment Fund X Fund Distribution Agent Relationship | Relative | Investment Fund — **[MỚI 2026-07-19]** BCV Concept tạm dùng, cần tra lại trước LLD. **Đảo ngược quyết định denormalize ARRAY cũ (7d)** — theo quyết định Data Modeler, tạo entity Relative riêng thay vì `distribution_agent_ids ARRAY<STRUCT>` trên Investment Fund. FK đến Investment Fund (Tier 2) + Fund Distribution Agent (Tier 1). Cột `DATA_MIGRATION` (cờ migration kỹ thuật) không map — xem 7e#6. |
...
| ~~AGEN_FUNDS~~ | ~~Bảng trung gian đại lý và quỹ đầu tư~~ | — | **[ĐẢO NGƯỢC 2026-07-19]** Theo quyết định Data Modeler, không còn denormalize — chuyển vào scope thành entity Relative riêng `Investment Fund X Fund Distribution Agent Relationship` (Tier 3, xem 7a). Attribute `distribution_agent_ids` ARRAY<STRUCT> xóa khỏi Investment Fund (xem LLD FUNDS). **FUD_AG_AGT (xem dưới) cần rà soát lại** — quyết định cũ nói "mở rộng STRUCT của distribution_agents" nhưng STRUCT đó không còn tồn tại; cần xác nhận FUD_AG_AGT có gộp vào entity Relationship mới này (bổ sung `agent_type_code`/`parent_agent_id`) hay vẫn tách riêng — xem 7e. |
| FUD_AG_AGT | Bảng trung gian quỹ, loại đại lý và đại lý (kèm đại lý cấp trên) | Investment Fund X Fund Distribution Agent Relationship (đang rà soát) | Junction 4-FK (FUD_ID + AG_TY_ID + AGEN_ID + AGEN_ID_PARENT), không có attribute nghiệp vụ khác. **[CẦN RÀ SOÁT 2026-07-19]** Trước đây "mở rộng STRUCT distribution_agents" — nay AGEN_FUNDS đã có entity riêng, cần xác nhận gộp FUD_AG_AGT vào entity đó (thêm `agent_type_code` + `parent_agent_id`) hay thiết kế riêng trước khi LLD. Chưa quyết — xem 7e. |
...
| 19 | 3 | **[MỚI 2026-07-19]** FUD_AG_AGT (4-FK: FUD_ID + AG_TY_ID + AGEN_ID + AGEN_ID_PARENT) trước đây "mở rộng STRUCT distribution_agents" trên Investment Fund (7d cũ) — nay AGEN_FUNDS đã có entity Relative riêng `Investment Fund X Fund Distribution Agent Relationship` (Tier 3). Xác nhận: gộp FUD_AG_AGT vào entity đó (bổ sung `agent_type_code`/`parent_agent_id`) hay thiết kế Relative riêng? | Ảnh hưởng thiết kế LLD cho AGEN_FUNDS/FUD_AG_AGT — chưa quyết, cần xác nhận trước khi thiết kế LLD Tier 3. |
| 20 | 3 | **[MỚI 2026-07-19]** BCV Concept của 3 entity Relationship mới (Fund Management Company Employee X Classification FMS Position Relationship, Investment Fund X Fund Management Company Employee Relationship, Investment Fund X Fund Distribution Agent Relationship) đang tạm dùng (tái sử dụng `[Involved Party] Individual Employment Status` / `[Arrangement] Investment Fund`) — chưa tra cứu BCV chính thức. | Cần tra `knowledge/` trước khi thiết kế LLD, theo quy tắc bắt buộc tra BCV. |
...
### 49. Investment Fund X Fund Management Company Employee Relationship
**Tier:** 3 | **Source:** `FMS.FUND_TL_PRO` | **BCV Concept:** [Arrangement] Investment Fund | **BCO:** Arrangement | **Table Type:** Relative
**Description:** Quan hệ gán người hành nghề (nhân sự chủ chốt CTQLQ) liên quan đến quỹ đầu tư. **[MỚI 2026-07-19]** Trước đây ở 7f nhóm Junction. FK đến Investment Fund (Tier 2) + Fund Management Company Employee (Tier 2). BCV Concept tạm dùng (tái sử dụng concept của Investment Fund Investor Membership/MB_FUND) — xem 7e#20.


### 50. Investment Fund X Fund Distribution Agent Relationship
**Tier:** 3 | **Source:** `FMS.AGEN_FUNDS` | **BCV Concept:** [Arrangement] Investment Fund | **BCO:** Arrangement | **Table Type:** Relative
**Description:** Quan hệ đại lý phân phối CCQ được ủy quyền cho quỹ đầu tư. **[ĐẢO NGƯỢC 2026-07-19]** Trước đây denormalize thành `distribution_agent_ids` ARRAY<STRUCT> trên Investment Fund (7d cũ) — theo quyết định Data Modeler, chuyển thành entity Relative riêng. FK đến Investment Fund (Tier 2) + Fund Distribution Agent (Tier 1). Cột `DATA_MIGRATION` (cờ migration kỹ thuật) không map — xem 7e#6. FUD_AG_AGT (mở rộng 4-FK, thêm agent_type/parent_agent) cần rà soát gộp hay tách riêng — xem 7e#19. BCV Concept tạm dùng — xem 7e#20.
