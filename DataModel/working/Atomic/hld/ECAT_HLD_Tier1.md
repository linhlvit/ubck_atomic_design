...
**Scope Tier 1:** 3 Atomic entity (đều shared/Fundamental) + 35 Classification Value scheme. Không có entity FK lẫn nhau trong cùng Tier.
...
graph TD
    ...
    GEO["**Geographic Area**\n[Location] Geographic Area\nECAT (7 danh mục hành chính) + shared nguồn khác"]:::atomic
    CUR["**Currency**\n[Common] Currency\nECAT_11_Currency (source chuẩn)"]:::atomic
    CAL["**Calendar Date**\n[Common] Time Period (Calendar Day)\nECAT_29_HolidayInfo"]:::atomic
...
**Shared entity:** Đã approved trong `atomic_entities.csv` (source hiện có: `FIMS.NATIONAL, SCMS.DM_TINH_THANH, SCMS.DM_QUOC_TICH`). ECAT mở rộng source_table để thành master của toàn bộ danh mục hành chính VN (country, region, province old/new, district old, ward old/new).
...
- **ECAT_29_HolidayInfo chỉ cung cấp 2 trường map vào Atomic**: `holiday_flag` và `holiday_name`. ETL merge ngày ECAT gửi với Calendar Date dense: nếu match date → set `holiday_flag = 1` + load `holiday_name`.
...
- Toàn bộ scheme được đăng ký vào `Atomic/lld/ref_shared_entity_classifications.csv` — source_type = `source_table`, values load từ ECAT.
