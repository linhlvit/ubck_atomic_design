...
erDiagram
    ...
    ClassificationECATBusinessLine {
    ...
    ClassificationECATBusinessLine ||--o{ ClassificationECATBusinessLine : "prn_biz_line_id (self-join: LEVEL_2→LEVEL_1)"
```

> `GeographicArea` là entity **đã approved** (nguồn hiện có: NHNCK.COUNTRIES/PROVINCES/DISTRICTS, FMS.NATIONAL) — tier này chỉ bổ sung ECAT làm source mới cho 4 loại COUNTRY/REGION/PROVINCE/WARD hiện hành, không đổi tên/cấu trúc entity. `GeographicAreaOld` là entity **mới** theo yêu cầu Data Modeler. `ClassificationECATBusinessLine` cũng là entity **mới** — gộp `BUSINESS_LINE_LEVEL_1`/`BUSINESS_LINE_LEVEL_2` thành 1 entity self-referencing, độc lập với nhóm Geographic Area.
...
| ETL derive từ tên bảng nguồn (BUSINESS_LINE_LEVEL_1/BUSINESS_LINE_LEVEL_2) | Phân biệt cấp ngành nghề (1/2) trên entity Classification ECAT Business Line | `ECAT_BUSINESS_LINE_LEVEL` | etl_derived | Scheme mới. Values: `LEVEL_1` (từ `ECAT.BUSINESS_LINE_LEVEL_1`), `LEVEL_2` (từ `ECAT.BUSINESS_LINE_LEVEL_2`). |
