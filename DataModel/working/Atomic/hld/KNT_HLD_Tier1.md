...
| Communication | [Communication] Risk Assessment | Risk Assessment | HIGH_RISK_ENTERPRISE | Danh sách doanh nghiệp rủi ro cao về thuế do Cục Thuế cung cấp: MST, tên, địa chỉ, cơ quan thuế quản lý, năm đánh giá | Tax High Risk Enterprise | Fact Snapshot | (1) Term "Risk Assessment" (Communication, id 8765): "Identifies an Assessment that identifies and analyzes the impact on the business of an unexpected financial event" — khớp việc Cục Thuế đánh giá/công bố 1 DN thuộc diện rủi ro cao. Đã tra thêm "Involved Party Credit Risk Rating" (Involved Party, id 10812) nhưng đây chỉ là property (thuộc tính gắn lên Organization), không phải khái niệm 1 lần thông báo/công bố đánh giá theo kỳ. (2) Cấu trúc trường: ENTERPRISE_CODE + ENTERPRISE_NAME + HQ_ADDRESS (thông tin DN, denormalized) + MANAGING_TAX_AUTHORITY (cơ quan ban hành đánh giá) + RISK_ASSESSMENT_YEAR (kỳ đánh giá) — đúng bản chất 1 lần Cục Thuế công bố đánh giá 1 DN rủi ro cao trong 1 năm, không phải hồ sơ DN (đã có Tax Registration) hay danh mục thuần Code+Name. (3) Chọn `[Communication] Risk Assessment` — nhất quán category Communication với Tax Information Request (cùng nhóm dữ liệu Cục Thuế gửi/công bố cho UBCKNN); Table Type = Fact Snapshot vì grain là "danh mục theo kỳ" (RISK_ASSESSMENT_YEAR) — 1 DN có thể xuất hiện lại ở nhiều năm khác nhau, giữ lịch sử qua các kỳ đánh giá thay vì 1 event rời rạc duy nhất. |
...
| HIGH_RISK_ENTERPRISE | N/A (chưa định nghĩa `ingestion` trong `brd_KNT.yaml`) | Cần bổ sung/profile trước LLD — xem 6f T1-04. Tạm xếp Fact Snapshot theo bản chất "danh mục theo kỳ" của RISK_ASSESSMENT_YEAR. |
...
erDiagram
    ...
    Tax_High_Risk_Enterprise {
        bigint ds_tax_high_risk_enterprise_id PK
...
| T1-04 | `HIGH_RISK_ENTERPRISE` không có `ingestion.data_change_mode` khai báo trong `brd_KNT.yaml` (khác với 6 bảng còn lại của Tier 1). `MANAGING_TAX_AUTHORITY` là free text hay có danh mục cố định (có thể trùng `CAT_ORG_UNIT` — KNT-REF, đang pending)? | Chưa xác nhận — cần bổ sung `ingestion` vào `brd_KNT.yaml` và profile dữ liệu thực tế trước khi thiết kế LLD. Tạm xếp Table Type = Fact Snapshot dựa trên bản chất "danh mục theo kỳ" (RISK_ASSESSMENT_YEAR). |
| T1-05 | `HIGH_RISK_ENTERPRISE.ENTERPRISE_CODE` ("Mã số doanh nghiệp đăng ký kinh doanh") có phải cùng giá trị với `TAX_REGISTRATION_INFO.TAX_CODE` không (ở Việt Nam mã số DN thường trùng MST)? Nếu đúng → có thể dựng liên kết logic (business key) với `Tax Registration`. | Chưa xác nhận — hiện thiết kế `Tax High Risk Enterprise` là entity độc lập ở Tier 1, không dựng FK/liên kết logic với `Tax Registration`. |
