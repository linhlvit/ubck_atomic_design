...
| # | Câu hỏi | Ảnh hưởng |
|---|---|---|
| 1 | `analysis_define_report_template` chỉ có 2 FK: attribute_id + report_id (kỹ thuật bỏ qua). Đây là pure junction hay cần entity riêng? | Nếu pure junction 2 entity Atomic → denormalize thành ARRAY `report_template_ids` trên entity `Market Surveillance Analysis Criterion`. Tuy nhiên report_id trỏ đến bảng `report_template` — bảng này ngoài scope Atomic (operational). Cần xác nhận: có cần lưu quan hệ này trên Atomic không? Đề xuất: **ngoài scope Atomic** — thông tin cấu hình chạy báo cáo, không phải dữ liệu nghiệp vụ. |
| 2 | ~~`securities_group_member` chỉ có 2 FK: group_id + securities_code_id. `securities_code` ngoài scope → denormalize thành `ARRAY<Text>` trên `Securities Watchlist Group`.~~ **✅ RESOLVED:** Thêm `securities_codes ARRAY<Text>` vào `Securities Watchlist Group` (T1). Không tạo Atomic entity riêng cho `securities_group_member`. | — |
